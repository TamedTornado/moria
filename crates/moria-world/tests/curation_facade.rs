use moria_world::{
    ...
    derive_manifest, validate_manifest_with_stamp,
};
...
fn facade_validation_returns_a_deterministic_typed_report() {
    ...
    let mut first: CurationReport =
        validate_manifest_with_stamp(&config, &manifest, &stamp).unwrap();
    let mut second = validate_manifest_with_stamp(&config, &manifest, &stamp).unwrap();
