...
const STUMP_PATH: &str = "props/stump.glb";
...
#[test]
fn stump_placeholder_is_a_shared_material_support_centered_glb() {
    let loader = AssetLoader::new();
    let declaration = loader.declaration(AssetId::Stump);
    assert_eq!(declaration.path, STUMP_PATH);
    assert_eq!(loader.resolve_runtime_path(STUMP_PATH), Ok(declaration));
    assert!(matches!(
        loader.missing_asset_action(AssetId::Stump, RuntimeAssetProfile::Development),
        AssetMissingAction::DevelopmentFallback { warning } if warning == AssetId::Stump.stable_id()
    ));

    let asset_path = concat!(env!("CARGO_MANIFEST_DIR"), "/../../assets/props/stump.glb");
    let bytes =
        fs::read(asset_path).expect("stump placeholder exists at its declared runtime path");
    ...
    assert_eq!(meshes.len(), 2, "placeholder supplies named stump variants");
    for mesh in meshes {
        assert!(
            mesh["name"]
                .as_str()
                .is_some_and(|name| name.starts_with("StumpVariant"))
        );
    ...
    for mesh in meshes {
        for primitive in mesh["primitives"].as_array().expect("mesh primitives") {
            ...
            assert!(min[0].as_f64().unwrap() >= -0.55 && max[0].as_f64().unwrap() <= 0.55);
            assert_eq!(min[1].as_f64(), Some(0.0));
            assert!(max[1].as_f64().unwrap() <= 0.75);
            assert!(min[2].as_f64().unwrap() >= -0.55 && max[2].as_f64().unwrap() <= 0.55);
        }
    }
}
