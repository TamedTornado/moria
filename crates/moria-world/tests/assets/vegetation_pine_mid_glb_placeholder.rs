...
#[test]
fn pine_mid_placeholder_is_a_shared_material_support_centered_glb() {
    ...
    assert!(matches!(
        ...
        AssetMissingAction::DevelopmentFallback { warning }
            if warning == AssetId::PineMid.stable_id()
    ));
    assert_eq!(
    ...
    let bytes = fs::read(asset_path()).expect("pine-mid placeholder exists at its declared path");
    ...
    assert!(
        index_count / 3 <= 3_000,
        "mid pine remains within its budget"
    );
