...
fn birch_near_placeholder_is_a_shared_material_support_centered_glb() {
    ...
    assert!(matches!(
        ...
        AssetMissingAction::DevelopmentFallback { warning }
            if warning == AssetId::BirchNear.stable_id()
    ));
    assert_eq!(
    ...
    let bytes = fs::read(asset_path()).expect("birch-near placeholder exists at its declared path");
    ...
    assert_eq!(
        materials.len(),
        1,
        "the near birch uses one shared material"
    );
    ...
    assert!(
        ...
        "near birch remains within its budget"
    );
