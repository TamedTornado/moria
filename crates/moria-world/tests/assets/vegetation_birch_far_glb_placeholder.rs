...
#[test]
fn birch_far_placeholder_is_a_shared_material_support_centered_glb() {
    let loader = AssetLoader::new();
    let declaration = loader.declaration(AssetId::BirchFar);
    assert_eq!(declaration.id.stable_id(), "moria.vegetation.birch_far");
    ...
    let bytes = fs::read(asset_path()).expect("birch-far placeholder exists at its declared path");
    ...
    assert!(
        index_count / 3 <= 500,
        "far birch remains within its budget"
    );
