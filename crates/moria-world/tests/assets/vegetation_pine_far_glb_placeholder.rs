...
#[test]
fn pine_far_placeholder_is_a_shared_material_binary_gltf_at_its_declared_path() {
    ...
    assert_eq!(loader.resolve_runtime_path(PINE_FAR_PATH), Ok(declaration));
    assert_eq!(
        loader.validation_fixture(AssetId::PineFar).asset_id,
        AssetId::PineFar
    );
    assert_eq!(
        loader.missing_asset_action(AssetId::PineFar, RuntimeAssetProfile::Development),
        AssetMissingAction::DevelopmentFallback {
            warning: "moria.vegetation.pine_far",
        }
    );
    assert_eq!(
    ...
    let asset = include_bytes!("../../../../assets/vegetation/pine_far.glb");
    ...
    let positions = primitives
        ...
        .collect::<Vec<_>>();
    let position_accessor = &document["accessors"][positions[0]];
    ...
    let triangles: u64 = primitives
        ...
        .sum();
    assert!(triangles <= 500, "far LOD has {triangles} triangles");
