...
#[test]
fn bush_far_placeholder_is_a_shared_material_support_centered_glb() {
    ...
    assert_eq!(
        loader.resolve_runtime_path(BUSH_FAR_PATH),
        Ok(declaration),
        "the fixture must exercise the immutable shared loader path"
    );
    assert_eq!(
        loader.validation_fixture(AssetId::BushFar).key,
        AssetId::BushFar.stable_id()
    );
    assert!(matches!(
        ...
        AssetMissingAction::DevelopmentFallback { warning }
            if warning == AssetId::BushFar.stable_id()
    ));
    assert_eq!(
    ...
    let bytes = fs::read(asset_path()).expect("bush-far placeholder exists at its declared path");
    ...
    assert!(index_count / 3 <= 250, "far bush remains within its budget");
    ...
    assert_eq!(
        position["max"]
            ...
            .map(Value::as_f64)
            .collect::<Option<Vec<_>>>(),
        Some(vec![1.2, 2.4, 1.2])
    );
}

fn asset_path() -> String {
    format!(
        "{}/../../assets/{BUSH_FAR_PATH}",
