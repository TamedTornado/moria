...
#[test]
fn birch_mid_placeholder_uses_its_declared_path_and_observable_fallback() {
...
fn birch_mid_placeholder_is_a_shared_material_support_centered_glb() {
    ...
    let mesh = document["meshes"]
        ...
        .expect("one birch mesh");
    assert_eq!(mesh["name"], "BirchMid");
    let primitive = mesh["primitives"]
        .as_array()
        .and_then(|primitives| primitives.first())
        .expect("one indexed birch primitive");
    ...
    assert!(index_count / 3 <= MAX_TRIANGLES);
