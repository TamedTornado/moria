...
#[test]
fn bush_near_placeholder_is_a_shared_material_support_centered_ellipsoid_glb() {
    ...
    assert_eq!(materials.len(), 1, "all bush instances share one material");

    let meshes = document["meshes"].as_array().expect("meshes array");
    assert_eq!(meshes.len(), 1, "one shared near bush mesh");
    let mesh = &meshes[0];
    assert_eq!(mesh["name"], "BushNear");
    let primitives = mesh["primitives"].as_array().expect("mesh primitives");
    assert_eq!(primitives.len(), 1, "one ellipsoid primitive");
    ...
    assert!(index_count / 3 <= 2_000, "near bush triangle budget");
    ...
    assert_eq!(position["min"], serde_json::json!([-1.2, 0, -1.2]));
    assert_eq!(position["max"], serde_json::json!([1.2, 2.4, 1.2]));
}
