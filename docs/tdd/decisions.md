...
from one pinned source into pre-reserved dynamic child identities under one
prepared `directory_generation` swap. It cannot invent samples, attach a
...
binding 6 before the adapter dispatch. Provisional ordinary `VolumeKey`s are
reserved at preflight and become lifetime records only when published. The
adapter selects which labeled pieces become persistent children or explicit
removal; Moria does not define connectivity significance or transient-debris
policy.

T35 keeps activity-region bytes as opaque current input and supplies bulk
placement through one GPU `PlacementStream` that reuses ordinary per-volume
move gates after GPU validation. Multi-fidelity body tables, fidelity-class
lists, compaction schedules, and exact proof dispatch/body capacities are
validation-evidence properties of a conforming proof adapter, not Moria
substrate vocabulary. Fixed maximum dispatch remains; P11 is blocking for the
proof class.

T36 adds optional opaque CPU egress as a handoff-shaped terminal on binding 7.
An adapter declares a byte maximum (zero disables). Moria reuses the existing
initialized-prefix, staging, map, and receipt lifecycle with explicit empty,
overflow, and unavailable outcomes. Publication may complete before
asynchronous egress mapping. GPU-to-GPU handoffs remain on the existing GPU
path.
...
## T34. Source-bound component extraction uses one directory-generation swap

**Decision.** Scheduled ABI v2 pre-reserves every possible child identity,
directory/lifetime-candidate slot, transfer/page/brick/scar/provenance record,
proposal, receipt, and byte before GPU execution. An extract-components
proposal labels samples from exactly one pinned source. The adapter sees a
dense piece-handle-to-final-`VolumeId` map before dispatch. Moria builds a new
source and children in unreferenced storage and installs the complete prepared
directory entry set with one checked `directory_generation` swap.
...
Ordinary unique `VolumeKey`s are provisional until publication; only published
keys consume lifetime records. Persisted derived provenance and a complete
sparse derived base replace a consumer content source. Uncheckpointed derived
content remains dirty and subject to the existing unrecoverable-device-loss
rule.
...
content and a source object. Source-bound transfer plus the existing
directory-generation snapshot already used by queries gives atomic membership
without replacing per-volume publication, inventing a global epoch type, or
mandating a four-level radix directory for every commit path.
...
individually, implicit deletion of unassigned cells, GPU transport of
`BaseContentSource`, a world-wide `WorldDirectoryEpoch` authority for ordinary
queries, and a separate derived-key namespace/UUIDv5 ordinal allocator.

## T35. Multi-fidelity uses opaque regions, counted GPU work, and a placement stream

**Decision.** The CPU supplies region definitions through ordinary opaque
participant input. The substrate boundary is that input, counted GPU-adapter
execution, and one bounded `PlacementStream` proposal per participant per
tick. The stream validates on GPU and publishes through ordinary per-volume
placement/revision gates in one ordered pass. Multi-fidelity classification,
body-table shape, halo policy, and list-compaction algorithms remain
adapter-owned and are proven by validation, not mandated as Moria types.

**Dispatch.** The portable baseline keeps fixed maximum dispatch charged to
each adapter's declared maxima. P11 measures the multi-fidelity proof at empty,
sparse, half, and full active sets and is blocking for that proof class.

**Reason.** Geographic adapters/worlds would require accidental state
migration and duplicate overlapping-region processing. Ordinary host move
admission per coarse object would force host enumeration and queue overhead.
Opaque input plus bulk placement already supplies the substrate hooks; making
one proof's body table, class list, or dispatch count normative would freeze
adapter implementation detail into the substrate contract. Per-volume move
gates suffice because placement updates need not publish as one cross-volume
membership transaction.

**Rejected.** GPU-selected activity regions, freezing objects outside all
regions, one Moria adapter per bubble, silent stale placement, an unproven
per-object host command loop, raw indirect dispatch buffers, and elevating
`Full | Halo | Coarse`, hierarchical compaction, exact proof dispatch counts,
or a fixed body capacity into public substrate law.

## T36. Adapter egress is handoff-shaped opaque transport independent of publication

**Decision.** A GPU adapter may declare zero egress or one maximum opaque byte
capacity. Scheduled ABI v2 binding 7 is a Moria-initialized handoff-shaped
CPU terminal: capacity, `written_bytes`, and closed status. Moria validates
the header, copies only a valid initialized prefix, maps asynchronously with
the existing handoff staging lifecycle, and returns a
tick/participant/correlation-bearing receipt in adapter tick order.

**Outcome and lifetime.** Zero written bytes is ready-empty. Exact capacity
succeeds when status is ready. Overflow fails with no delivered prefix.
Publication does not depend on CPU decoding and may be reported while egress
is pending. Working, staging, and host bytes release only after their
respective GPU-use, map/view-drop/unmap, and result-drop milestones.
Cancellation, shutdown, mapping, decoding, and device loss are distinct
terminal outcomes.

**Reason.** Adapters need generic CPU-visible facts without teaching Moria an
event vocabulary or reading back full solver state. Extending the existing
opaque handoff/readback primitive with a CPU terminal and explicit empty,
overflow, and unavailable outcomes is sufficient; a separate fixed-stride
record protocol, schema wire field, atomic record allocator, and independent
five-pool family are not required at the substrate boundary.
...
authority resources, routing GPU-to-GPU consumers through CPU egress, and
mandating fixed-stride records or an atomic record helper as substrate law.
