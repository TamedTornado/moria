...
extraction-scoped publication commit. Pre-reserved child directory entries
stay hidden until that commit installs source remainder (or retirement) and
every selected child together under the ordinary confirmed-publication
boundary. It cannot invent samples, attach a consumer content source, or
perform arbitrary scheduled creation. Proposal-local piece handles map to
final `VolumeId`s in read-only Scheduled ABI v2 binding 6 before the adapter
dispatch. The adapter selects which labeled pieces become persistent children
or explicit removal; Moria does not define connectivity significance or
transient-debris policy.
...
and publishes changed poses through one GPU placement stream that validates
once and advances ordinary per-volume placement/revision gates in one ordered
pass rather than per-object host move commands. The fixed-dispatch baseline
is retained with a declared 65,536-body, 16-dispatch/8,192-workgroup bound
and blocking P11 evidence. Cross-volume atomic placement visibility is not
required and is not provided.

T36 extends the T28 opaque handoff transport with an optional terminal
consumer egress endpoint on binding 7. Every enabled adapter declares exact
record/byte maxima; tick admission reserves map, device, staging, host, and
receipt capacity from the existing handoff pools before execution.
Publication may complete before asynchronous egress mapping. Zero records is
a successful empty result, while overflow and transport failure (map, decode,
shutdown, cancellation, device loss) are explicit terminal results with no
truncated prefix. GPU-to-GPU handoffs remain on the existing GPU path.
...
## T34. Source-bound component extraction uses one extraction-scoped commit
...
and children in unreferenced storage; child candidates remain hidden until one
ordered publication pass installs the source update and every selected child
under the ordinary confirmed-publication boundary.
...
`VolumeKey`/`VolumeId` use the ordinary live/lifetime reservation path.
A small extraction-provenance record and the existing sparse full-brick scar
mechanism over the internal empty base replace a consumer content source.
Uncheckpointed transferred scars remain dirty and subject to the existing
...
content and a source object. Source-bound transfer plus an extraction-scoped
hidden-until-published commit gives the required atomic ownership without
replacing every query, collision, presentation, checkpoint, and ordinary
commit path with a world-wide directory-epoch radix.
...
individually so readers can observe partial ownership, implicit deletion of
unassigned cells, GPU transport of `BaseContentSource`, a four-level immutable
world directory radix, and migrating all authority capture onto root slots.
...
validates on GPU once, then advances each addressed volume's ordinary
placement/revision gate in one ordered publication pass.
...
One table, deterministic union, halo, and compact ordered gate updates preserve
continuous ownership and fresh Moria placements. The authority forbids stale
placements and requires a bounded GPU placement mechanism; it does not require
cross-volume atomic placement visibility, so an alternate world-directory root
is unnecessary.
...
stale placement, an unproven per-object host command loop, whole-stream
cross-volume atomic visibility, and raw indirect dispatch buffers.

## T36. Adapter egress extends handoff transport and is independent of publication

**Decision.** A GPU adapter may declare zero egress or one fixed-stride schema
and exact record/byte maximum. Scheduled ABI v2 binding 7 is the terminal
consumer write target of the T28 opaque handoff lifecycle. The adapter
reserves records with the supplied atomic helper; Moria validates the complete
required count, copies only a valid initialized prefix through the ordinary
GPU-to-CPU handoff copy/map path, and returns a
tick/participant/correlation/schema-bearing receipt in adapter tick order.
Map, device, staging, and host bytes charge the existing handoff pools.
...
map/view-drop/unmap, and result-drop milestones. Overflow and transport
failure (map, decode, cancellation, shutdown, device loss) are closed terminal
outcomes without a parallel subsystem-only error taxonomy.
...
pre-reserved handoff transport with a terminal consumer endpoint, fixed-stride
prefix, overflow result, and correlation distinguishes no-events from
unavailable/lost events while leaving GPU-to-GPU edges unchanged.
...
independent egress-only map/device/staging/host capacity pools.
