...
multi-entry directory-generation install. It cannot invent samples, attach a
...
T35 keeps multi-fidelity simulation in the adapter. Region bytes remain
opaque current input. One persistent adapter/body table classifies bodies,
continues coarse work outside every region, and publishes changed poses
through one GPU placement stream rather than per-object host move commands.
The fixed-dispatch baseline is retained with declared maxima and blocking P11
evidence. Moria does not own fidelity classes or classification algorithms.
...
## T34. Source-bound component extraction uses one multi-entry directory install
...
and children in unreferenced storage and installs the complete multi-entry
directory set under one advanced `directory_generation`, reusing the product's
existing generation/publication model rather than a parallel directory ABI.
...
content and a source object. Source-bound transfer plus multi-entry directory
publication gives the required capability without inventing a second directory
substrate.
...
individually, implicit deletion of unassigned cells, a parallel
`WorldDirectoryEpoch`/radix-root public ABI, and GPU transport of
`BaseContentSource`.
...
regions, classifies bodies under adapter-owned fidelity policy, continues
coarse work outside all regions, and compacts changed Moria placements into one
bounded placement stream. The stream validates on GPU and publishes one
multi-entry directory install while each updated volume advances its revision.
Fidelity classes, classification algorithms, and kernel schedules are adapter
internals proven by validation, not public Moria types.

**Dispatch.** The portable baseline keeps fixed maximum dispatch for the
65,536-body proof capacity. The proof adapter declares dispatch/workgroup
maxima that cover its internal schedule; P11 at empty, 1%, 50%, and 100%
active lists is blocking.
...
One table, deterministic adapter union, transition path, and compact placement
update preserve continuous ownership and fresh Moria placements without
teaching Moria a simulation vocabulary.
...
dispatch buffers, and baking a specific hierarchical-scan recipe into the
substrate contract.
