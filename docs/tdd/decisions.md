...
from one pinned source into pre-reserved dynamic child identities under an
extraction-local commit-group gate referenced only by the prepared source and
hidden child entries. Existing directory structure and ordinary per-volume
revision gates remain intact. It cannot invent samples, attach a consumer
content source, or perform arbitrary scheduled creation. Proposal-local piece
handles map to final `VolumeId`s in read-only Scheduled ABI v2 binding 6
before the adapter dispatch. Child stable keys are generated per extraction and
reserved against the existing lifetime registry; there is no separate
persisted derived-key namespace or never-reused candidate-ordinal lifecycle.
The adapter selects which labeled pieces become persistent children or
explicit removal; Moria does not define connectivity significance or
...
and publishes changed poses through one GPU placement batch validated as a
compact payload and published through the existing per-volume move/revision
path. Cross-volume atomicity is not required for placement. The fixed-dispatch
baseline is retained with a declared 65,536-body, 16-dispatch/8,192-workgroup
bound and blocking P11 evidence.

T36 adds optional fixed-stride opaque CPU egress in binding 7. Every enabled
adapter declares exact record/byte maxima; tick admission reserves capacity
before execution. Egress reuses the existing bounded GPU-to-CPU
handoff/readback lifecycle and shared staging accounting while retaining a
distinct public receipt and exact overflow/failure semantics. Publication may
...
scheduled ABI accepted by the initial implementation. The reduced public
surface is limited to capability descriptors, piece-to-child results,
placement-batch results, and opaque egress outcomes. Conservation digests,
directory layouts, and proof-adapter kernel details remain internal or
validation-only. Moria does not acquire a timestep, region, velocity,
physics, collision-response, damage, weapon, debris, scoring, audio, or
...
## T34. Source-bound component extraction uses an extraction-local commit-group
...
and children in unreferenced storage, installs them as hidden entries
referenced only by an extraction-local commit-group, and publishes that group
so old snapshots retain the source state while new readers resolve the
committed source/children set. Ordinary directory structure and per-volume
revision gates remain; no public world-directory epoch is introduced.
Child `VolumeKey`s are generated per extraction and reserved against the
existing lifetime registry.
...
content and a source object. Source-bound transfer plus a narrow commit-group
gate gives the required capability without a second global directory-epoch
scheme or a never-reused derived-key ordinal lifecycle.
...
individually without a commit-group, implicit deletion of unassigned cells,
GPU transport of `BaseContentSource`, a public `WorldDirectoryEpoch` root
gate, and a persisted derived-key namespace with never-reused candidate
ordinals.

## T35. Persistent multi-fidelity adapters use opaque CPU regions and a placement batch
...
changed Moria placements into one bounded placement batch. The batch validates
as one compact GPU payload and publishes its entries through the existing
per-volume move/revision path in a bounded GPU pass. Cross-volume atomicity is
not required.
...
1%, 50%, and 100% active lists is blocking. Proof-adapter kernel detail is
validation-only.
...
One table, deterministic union, halo, and compact per-volume publication
preserve continuous ownership and fresh Moria placements without a multi-volume
directory-epoch root for placement.
...
dispatch buffers, and cross-volume atomic placement publication.
...
through the existing GPU-to-CPU handoff/readback lifecycle and shared staging
accounting, maps asynchronously, and returns a distinct
...
event vocabulary or reading back full solver state. A distinct public receipt
makes no-events distinguishable from unavailable/lost events while reusing
existing readback/staging machinery and keeping GPU handoffs on the GPU path.
...
separate egress staging authority path that duplicates handoff readback.
