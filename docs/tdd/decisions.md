...
from one pinned source into pre-reserved dynamic child identities and publishes
them through the existing offline-prepare plus confirmed world-directory
generation-swap path. It cannot invent samples, attach a consumer content
source, or perform arbitrary scheduled creation. Proposal-local piece handles
map to final `VolumeId`s in read-only Scheduled ABI v2 binding 6 before the
adapter dispatch. The adapter selects which labeled pieces become persistent
children or explicit removal; Moria does not define connectivity significance
or transient-debris policy. Ordinary single-volume revision gates are not
rewritten.

T35 keeps activity-region definitions as ordinary opaque current input. Moria
does not classify bodies or own fidelity classes. Substrate support is one
bounded GPU placement stream for compact pose publication plus validation that
a single persistent multi-fidelity adapter can continue coarse work outside
CPU-authored regions without freezing. The fixed-dispatch baseline is retained
with a declared 65,536-body, 16-dispatch/8,192-workgroup bound and blocking P11
evidence. Classification, halo, and coarse/full kernels remain adapter-owned
proof obligations, not Moria public vocabulary.
...
## T34. Source-bound component extraction reuses multi-volume directory publication
...
and children in unreferenced storage and installs them with one confirmed
world-directory generation swap, reusing the existing offline-prepare model
rather than inventing a second directory ABI.
...
content and a source object. Source-bound transfer plus the existing directory-
generation publish path gives the required capability without rewriting ordinary
single-volume gates.
...
`BaseContentSource`, and a new radix/authority directory architecture for all
commits.

## T35. Multi-fidelity needs opaque regions and a placement stream, not Moria fidelity classes

**Decision.** The CPU supplies region definitions through ordinary opaque
participant input. Moria does not classify bodies or define full/halo/coarse
classes. Substrate support is one bounded placement stream that validates on
GPU and publishes many pose updates under one multi-volume directory generation
while each updated volume advances its revision. Proof adapters must still use
one persistent body table, process each body once across overlapping regions,
preserve continuous ownership across transitions, and continue coarse work
outside every region.

**Dispatch.** The portable baseline keeps fixed maximum dispatch:
65,536 bodies at width 128 against declared maxima 16 dispatches / 8,192
workgroups. P11 at empty, 1%, 50%, and 100% active lists is blocking.
...
Opaque input plus one compact placement stream is the smallest substrate
surface that keeps Moria free of simulation vocabulary while meeting the
required multi-fidelity behavior.

**Rejected.** GPU-selected activity regions, freezing objects outside all
regions, one adapter per bubble, Moria-owned fidelity classes or region schemas,
silent stale placement, an unproven per-object host command loop, raw indirect
dispatch buffers, and rewriting ordinary publication solely to serve bulk
placement.
