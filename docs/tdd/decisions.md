...
multi-entry directory-generation publication. It cannot invent samples, attach
a consumer content source, or perform arbitrary scheduled creation.
Proposal-local piece handles map to final `VolumeId`s in read-only Scheduled
ABI v2 binding 6 before the adapter dispatch. Child keys are preallocated
directly; children persist as ordinary full-brick scars over an internal empty
base with minimal extraction provenance. The adapter selects which labeled
...
T35 keeps activity-region bytes as opaque current input and publishes changed
poses through one bounded GPU placement stream. That stream validates once and
publishes through the existing independent per-volume placement/revision gates
rather than a multi-entry directory transaction. Fidelity classes, body
classification, and proof-adapter dispatch counts live in validation, not as
substrate architecture. Fixed-dispatch proof capacity remains a blocking P11
evidence obligation for the selected proof adapter.
...
staging, host, map, and receipt capacity before execution using the existing
opaque handoff/readback and generic staging patterns. Publication may
...
## T34. Source-bound component extraction uses one multi-entry directory generation
...
piece-handle-to-final-identity map before dispatch. Child `VolumeKey`s are
preallocated directly from the ordinary stable-key allocator. Moria builds a
new source and children in unreferenced storage and installs them with one
multi-entry directory-generation publication.
...
The child uses an internal empty base plus ordinary full-brick scars for every
nonempty brick, with minimal extraction provenance. Uncheckpointed scars remain
dirty under the existing unrecoverable-device-loss rule.
...
content and a source object. Source-bound transfer plus one multi-entry
directory generation gives the required capability without a second persistence
hierarchy or derived-key namespace.
...
`BaseContentSource`, UUIDv5 derived-key namespaces with candidate ordinals, and
a second derived-base-plus-scar store.

## T35. Opaque CPU regions plus bounded placement batches

**Decision.** The CPU supplies region definitions through ordinary opaque
participant input. Moria does not define fidelity classes or classification.
A multi-fidelity adapter owns one persistent body table and region policy
across the world. Changed poses publish through one bounded GPU placement
stream that validates once and then uses the existing independent per-volume
placement/revision gates. Fixed-dispatch measurement for the proof adapter is
a validation/P11 obligation, not substrate vocabulary.
...
Opaque region transport plus independent-volume placement batches preserve
continuous ownership and fresh Moria placements without a multi-entry directory
transaction for placement.

**Rejected.** GPU-selected activity regions, freezing objects outside all
regions, one adapter per bubble, multi-entry directory epochs for placement
streams, silent stale placement, an unproven per-object host command loop, and
normative substrate Full/Halo/Coarse types.
