...
from one pinned source into pre-reserved dynamic child identities through the
existing internal directory-generation/MVCC path: prebuilt invisible child
entries plus one extraction-only generation swap. It cannot invent samples,
attach a consumer content source, or perform arbitrary scheduled creation.
Proposal-local piece handles map to final `VolumeId`s in the read-only
binding-0 reservation section before the adapter dispatch. Child keys are
derived from parent key, pinned source revision, proposal slot, and piece
handle. Child matter is complete full-brick scars against the canonical empty
internal base plus minimal provenance. The adapter selects which labeled
...
T35 keeps activity-region bytes as opaque current input. One persistent
adapter/body table and continued coarse simulation outside every region are
adapter obligations proven by the multi-fidelity proof fixture; they are not
Moria region, fidelity, or fixed-dispatch architecture. Changed poses publish
through one GPU placement stream that validates as a batch and uses existing
per-volume revision/placement gates rather than an alternate directory root
for unrelated placement updates.

T36 adds optional fixed-stride opaque CPU egress as a trailing section of
binding 1. Every enabled adapter declares exact record/byte maxima; tick
admission reserves staging/map capacity through existing pools and retained
host-result capacity for decoded deliveries. Publication may complete before
asynchronous egress mapping. Zero records is a successful empty result, while
overflow, mapping, decode, shutdown, cancellation, and device loss are
explicit terminal results with no truncated prefix. GPU-to-GPU handoffs remain
on the existing GPU path.

Scheduled ABI v2 therefore still has exactly six group-0 bindings and is the
only scheduled ABI accepted by the initial implementation. The new types remain
...
or gameplay-event model, and it does not replace ordinary commits with a
public copy-on-write world-directory protocol.
...
## T34. Source-bound component extraction extends directory generation, not public COW roots
...
receipt, and byte before GPU execution. Extraction records charge existing
proposal/payload/cell/brick/directory-effect pools; only child-identity
headroom is a distinct capacity. An extract-components proposal labels samples
from exactly one pinned source. The adapter sees a dense
piece-handle-to-final-identity map in the binding-0 view section before
dispatch. Moria prebuilds invisible child directory entries, constructs the
source residual and children in unreferenced storage, and publishes with one
extraction-only directory generation swap.
...
Stable keys derive from parent key, pinned source revision, proposal slot, and
piece handle. Persisted minimal provenance plus complete full-brick scars
against the canonical empty internal base replace a consumer content source.
Uncheckpointed dirty scars remain subject to the existing
unrecoverable-device-loss rule.
...
content and a source object. Source-bound transfer plus the baseline directory
generation mechanism gives the required capability without widening ordinary
commit publication into a public multi-root world-directory protocol.
...
`BaseContentSource`, a persisted global derived-key namespace/candidate
ordinal, a complete second sparse derived-base chunk class, and replacing
every ordinary volume commit with alternate directory roots.

## T35. Opaque CPU regions stay adapter input; placement uses ordinary volume gates

**Decision.** The CPU supplies region definitions through ordinary opaque
participant input. Substrate obligations are transport of that input, a
bounded placement-stream proposal, and the extraction/egress transports above.
One GPU adapter owning one persistent body table, classifying bodies once
against the deterministic region union, continuing coarse work outside all
regions, and using a transition halo are proof-adapter obligations validated
by C12/P11 rather than public Moria fidelity classes or dispatch schedules.
The placement stream validates on GPU and publishes through existing
per-volume revision/placement gates while each updated volume advances its
revision.

**Dispatch.** Fixed maximum dispatch remains the portable scheduled baseline.
Whether a particular multi-fidelity proof adapter's declared dispatch and
workgroup maxima fit empty/sparse/half/full active lists is a blocking P11
fixture question, not a second substrate architecture selection.
...
Opaque input plus a compact placement batch preserve continuous adapter
ownership and fresh Moria placements without inventing alternate directory
roots for unrelated placement updates.
...
stale placement, an unproven per-object host command loop, making Full/Halo/
Coarse Moria types, and a public alternate directory root for bulk placement.
...
and exact record/byte maximum. Scheduled ABI v2 places the egress section at
the end of binding 1. The adapter reserves records with the supplied atomic
helper; Moria validates the complete required count, copies only a valid
initialized prefix through existing staging/map machinery, and returns a
tick/participant/correlation/schema-bearing receipt in adapter tick order.
Decoded retained host results charge the distinct egress host-byte capacity.
...
GPU handoffs on the GPU path without adding a seventh group-0 binding.
...
authority resources, routing GPU-to-GPU consumers through CPU egress, and a
dedicated binding-7 transport.
