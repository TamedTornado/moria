...
`directory_generation` publication gate. It cannot invent samples, attach a
...
T35 keeps activity regions as opaque current input and requires one persistent
adapter/body table rather than one adapter or world per geographic region.
Moria does not classify bodies or own fidelity classes. Changed poses publish
through one compact GPU placement stream that reuses ordinary independent
per-volume placement/revision gates, rather than per-object host move
commands. The fixed-dispatch baseline is retained; declared dispatch and
workgroup maxima are charged at admission, and blocking P11 evidence is
required.
...
## T34. Source-bound component extraction uses one directory-generation gate
...
and children in unreferenced storage and installs those directory entries under
one `directory_generation` publication gate.
...
directory-generation publication gives the required capability without
redesigning ordinary single-volume commits.
...
`BaseContentSource`, and a general multi-volume transaction system for
unrelated operations.

## T35. Multi-fidelity uses opaque CPU regions and a placement stream
...
regions. Classification, fidelity classes, halo policy, and coarse/full
kernels are adapter-owned. Moria transports the region bytes and accepts
changed poses through one bounded placement stream. The stream validates on
GPU and publishes each update through ordinary independent per-volume
placement/revision gates—without host body enumeration or a second multi-volume
epoch.

**Dispatch.** The portable baseline keeps fixed maximum dispatch. Adapters
declare dispatch and workgroup maxima charged at tick admission. P11 at empty,
sparse, half, and full active lists is blocking for the multi-fidelity proof
adapter.
...
Opaque input plus one compact placement path preserves continuous ownership
and fresh Moria placements without teaching Moria region semantics.
...
dispatch buffers, and Moria-owned fidelity classes or region schemas.
