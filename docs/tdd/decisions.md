...
fracture-scoped prepared directory delta and one `directory_generation`
advance. It cannot invent samples, attach a consumer content source, or
perform arbitrary scheduled creation. Proposal-local piece handles are the
only in-dispatch child labels; final `VolumeId`/key mappings return through
ordinary prior feedback (binding 4) after confirmed publication. The adapter
selects which labeled pieces become persistent children or explicit removal;
Moria does not define connectivity significance or transient-debris policy.
...
and publishes changed poses through one GPU placement stream validated once
and applied through existing per-volume placement/revision gates rather than
per-object host move commands. The fixed-dispatch baseline is retained with a
declared 65,536-body, 16-dispatch/8,192-workgroup bound and blocking P11
evidence.

T36 adds optional opaque CPU egress by reusing the existing outgoing-handoff
buffer and GPU-to-CPU staging/map lifecycle as a terminal consumer edge.
Every enabled adapter declares a maximum egress byte capacity; tick admission
reserves that capacity from handoff/staging budgets before execution.
Publication may complete before asynchronous egress mapping. Zero bytes is a
successful empty result, while overflow, mapping, decode, shutdown,
cancellation, and device loss are explicit terminal results with no truncated
prefix. GPU-to-GPU peer handoffs remain on the existing GPU path.

Scheduled ABI v2 therefore retains six group-0 bindings, adds two proposal
kinds, and is the only scheduled ABI accepted by the initial implementation.
The new types remain generic substrate records. Moria does not acquire a
timestep, region, velocity, physics, collision-response, damage, weapon,
debris, scoring, audio, or gameplay-event model.
...
## T34. Source-bound component extraction uses a fracture-scoped directory delta

**Decision.** Scheduled ABI v2 pre-reserves every possible child identity,
directory/lifetime entry, transfer/page/brick/scar/provenance record, fracture
directory delta, proposal, receipt, and byte before GPU execution. An
extract-components proposal labels samples from exactly one pinned source.
Piece handles remain proposal-local during dispatch. Moria builds a new source
and children in unreferenced storage, installs them through a fracture-scoped
prepared directory delta, and advances `directory_generation` once so readers
observe either the complete old source or the complete new source/children set.
Final child IDs appear in prior feedback after confirmed publication.
...
initial placement preserves every transferred world-space cell box. Child keys
derive deterministically from the world's `derived_key_namespace`, source key,
pinned source revision, proposal slot, and piece handle—no world-wide ordinal
or collision-retry allocator. Publication installs an internal empty base plus
ordinary T8 full-brick scars for transferred content. Uncheckpointed dirty
scars remain subject to the existing unrecoverable-device-loss rule.
...
content and a source object. Source-bound transfer plus a fracture-scoped
delta reuses the baseline directory-generation and per-volume gate machinery
without replacing directory authority for every query, collision, scheduled
view, checkpoint, presentation job, or ordinary commit.

**Rejected.** A four-level immutable world-directory radix and
`WorldDirectoryEpoch` gate for every reader path; CPU component readback;
host-authored follow-up create; copying source cells into both parent and
child; publishing directory entries without a generation barrier; pre-dispatch
binding-6 final-identity tables; global candidate ordinals; implicit deletion
of unassigned cells; GPU transport of `BaseContentSource`; and a parallel
checkpoint-v2 complete sparse derived-base store when empty base plus T8 scars
already rematerialize the child.
...
validates on GPU as one all-or-none payload, then publishes each entry through
the existing per-volume placement/revision gates while each updated volume
advances its own revision.
...
One table, deterministic union, halo, and GPU-compacted per-volume publication
preserve continuous ownership and fresh Moria placements. The multi-fidelity
requirement asks for bounded scalable updates, not cross-volume atomicity of
an entire stream; T4's independent-volume revision streams are preserved.
...
stale placement, an unproven per-object host command loop, raw indirect
dispatch buffers, and all-or-none multi-volume directory-root publication for
the stream.

## T36. Adapter egress reuses handoff staging as terminal host delivery

**Decision.** A GPU adapter may declare zero egress or one maximum opaque byte
capacity. When enabled, Moria reserves a terminal consumer edge in the existing
outgoing-handoff buffer and delivers the initialized prefix through the
existing GPU-to-CPU staging/map lifecycle as `Receipt<Box<[u8]>>` in adapter
tick order. No binding 7, fixed-stride schema/count ABI, or independent egress
pool family is added.

**Outcome and lifetime.** Zero bytes is ready-empty. Exact capacity succeeds.
Overflow fails with no delivered prefix. Publication does not depend on CPU
decoding and may be reported while egress is pending. Handoff working ranges,
staging maps, and host result bytes release only after their respective
GPU-use, map/view-drop/unmap, and result-drop milestones. Cancellation,
shutdown, mapping, decoding, and device loss are distinct terminal outcomes.

**Reason.** Adapters need generic CPU-visible facts without teaching Moria an
event vocabulary or reading back full solver state. The handoff header already
bounds initialized prefixes; the GPU-to-CPU handoff path already defines
staging, map, overflow, and reuse contracts. A terminal host receipt over that
primitive distinguishes no-events from unavailable/lost events without a
parallel transport stack, and keeps GPU peer handoffs on the GPU path.
...
authority resources, routing GPU-to-GPU consumers through CPU egress, binding
7, fixed-stride egress schema ABIs, and independent egress
map/receipt/record/device/staging/host pools.
