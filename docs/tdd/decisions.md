...
extraction-local publication transaction that reuses existing directory-
generation and per-volume gate machinery. It cannot invent samples, attach a
...
connectivity significance or transient-debris policy. Children persist through
ordinary full-brick scars against a minimal internal empty base plus
provenance; no parallel derived-base or key-namespace lifecycle is introduced.

T35 leaves activity-region bytes on the existing opaque participant-input path.
Multi-fidelity body tables, region-union classification, halo continuity,
coarse/full dispatch, and related simulation policy remain entirely in the
external proof adapter. Moria adds only a bounded GPU placement-update batch
whose entries publish through ordinary per-volume move gates into the existing
tick revision vector. Cross-volume placement atomicity is not required.

T36 adds optional bounded opaque CPU egress in binding 7. Every enabled
adapter declares only record and byte maxima; tick admission reserves capacity
from the existing handoff, staging, map, and host-result infrastructure.
Publication may complete before asynchronous egress mapping. Zero records is a
...
prefix. A fixed-stride schema protocol and dedicated egress pools are not
justified. GPU-to-GPU handoffs remain on the existing GPU path.
...
## T34. Source-bound component extraction reuses ordinary publication and scars
...
remainder and children in unreferenced storage and installs them as one
extraction transaction so readers never observe partial source/child state.
Ordinary single-volume matter, move, and retire publication paths remain
unchanged; no public `WorldDirectoryEpoch` is exposed.
...
Persisted provenance plus a minimal internal empty base replace a consumer
content source; transferred samples become ordinary full-brick scars against
that base. Uncheckpointed dirty scars remain subject to the existing
...
content and a source object. Source-bound transfer plus one extraction-local
install gives the required capability without a second directory-root or
key-namespace lifecycle.

**Rejected.** CPU component readback, host-authored follow-up create, copying
source cells into both parent and child, publishing source and children as
independently visible commits, implicit deletion of unassigned cells, GPU
transport of `BaseContentSource`, a global world-directory epoch API, and a
parallel complete sparse derived-base store.

## T35. Multi-fidelity policy stays in the adapter; Moria adds a placement batch

**Decision.** The CPU supplies region definitions through ordinary opaque
participant input. The external proof adapter owns body-table organization,
region-union classification, halo/full/coarse dispatch, and continuity policy.
Moria supplies only the existing input binding, stable views, and one bounded
GPU placement-update batch. Each batch entry publishes through the ordinary
per-volume move gate and contributes to the existing tick revision vector.
Cross-volume placement atomicity is not required.

**Reason.** Geographic adapters/worlds would require accidental state
migration and duplicate overlapping-region processing. Encoding classification,
halo, and dispatch strategy in Moria would pull simulation policy into the
substrate. A compact placement batch removes per-object host move admission
without inventing a second multi-volume publication plane.

**Rejected.** GPU-selected activity regions, freezing objects outside all
regions, one adapter per bubble, Moria-owned fidelity classes or body tables,
cross-volume placement atomicity, silent stale placement, and baking
fixed-dispatch qualification into the substrate contract.

## T36. Adapter egress is bounded opaque transport on existing staging paths

**Decision.** A GPU adapter may declare zero egress or nonzero record and byte
maxima. Scheduled ABI v2 binding 7 is initialized by Moria. The adapter
reserves records/bytes with the supplied atomic helper; Moria validates the
complete required counts, copies only a valid initialized prefix, maps
asynchronously through the existing handoff/staging/map infrastructure, and
returns a tick/participant/correlation-bearing receipt in adapter tick order.
No fixed-stride schema field and no dedicated egress pools are introduced.
...
GPU handoffs on the GPU path. Existing staging and map infrastructure already
provide the required lifetime milestones.
...
authority resources, routing GPU-to-GPU consumers through CPU egress, fixed-
stride schema protocol fields, and dedicated egress device/staging/host pools.
