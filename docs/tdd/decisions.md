...
extraction-scoped `directory_generation` transaction that publishes only the
source entry and reserved children. It cannot invent samples, attach a
consumer content source, or perform arbitrary scheduled creation. Proposal-
local piece handles map to final `VolumeId`s and ordinary lifetime `VolumeKey`s
in read-only Scheduled ABI v2 binding 6 before the adapter dispatch. The
...
and publishes changed poses through one GPU placement stream of independent
per-volume placement/revision gates rather than per-object host move commands.
The fixed-dispatch baseline is retained with a declared 65,536-body,
16-dispatch/8,192-workgroup bound and blocking P11 evidence.
...
through the existing handoff and generic staging/readback/typed-receipt paths
before execution. Publication may complete before asynchronous egress mapping.
Zero records is a successful empty result, while overflow, mapping, decode,
...
piece-handle-to-final-identity map before dispatch. Child `VolumeId` and
`VolumeKey` values come from ordinary live-slot and lifetime-key reservation;
Moria mints the key and persists it by value with extraction provenance. Moria
builds the new source and children in unreferenced storage and publishes only
those prepared directory entries by advancing the existing
`directory_generation` already captured in `SnapshotToken`.
...
directory-generation transaction gives old-or-new membership visibility without
replacing ordinary per-volume gates or inventing a global radix epoch system.
...
`BaseContentSource`, a global four-level directory epoch that rewires every
reader path, derived-key namespaces, candidate ordinals, and UUIDv5/restore-time
key recomputation.
...
validates on GPU once; each entry then publishes through the ordinary
per-volume placement and revision gate. Cross-volume placement atomicity and
alternate directory roots are not required.
...
One table, deterministic union, halo, and compact per-volume gate batch preserve
continuous ownership, independent-volume semantics, and fresh Moria placements.
...
stale placement, an unproven per-object host command loop, alternate-root
all-at-once placement publication, root-chain/repair semantics for placement,
and raw indirect dispatch buffers.
...
onto the existing GPU-to-CPU handoff/staging/readback path, and returns a
tick/participant/correlation/schema-bearing typed receipt in adapter tick
order. Capacity charges existing handoff and generic staging pools rather than
a parallel egress-only transport stack.
...
GPU handoffs on the GPU path. Distinct outcomes do not require inventing a
second transport and accounting stack beside handoff and generic staging.
...
independent non-aliasing egress-only map/device/staging/host pools.
