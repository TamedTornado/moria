...
source-bound multi-volume publication that reuses ordinary per-volume
revision/placement gates for only the parent and candidate children. It cannot
invent samples, attach a consumer content source, or perform arbitrary
scheduled creation. Proposal-local piece handles map to final `VolumeId`s in
read-only Scheduled ABI v2 binding 6 before the adapter dispatch. The adapter
...
T35 keeps region bytes as opaque current input and requires adapters to keep
coarse simulation running outside every full-fidelity region, process each body
once across a deterministic region union, preserve continuous ownership across
region crossings, and publish changed poses through a compact GPU placement
stream rather than a host-enumerated move loop. Fixed maximum dispatch remains
the substrate default; P11 proves it for a multi-fidelity qualification
harness. Moria does not mandate that every conforming multi-fidelity adapter
use one particular body table, class enum, hierarchical scan, body-capacity
constant, or private dispatch graph.
...
## T34. Source-bound component extraction uses a multi-volume gate on existing volume authorities
...
and children in unreferenced storage and publishes only that source-bound set
through the existing per-volume revision/placement gates as one all-or-none
confirmed transaction. Queries, collision, scheduled views, checkpoints,
presentation, and ordinary commits continue to use the baseline
`directory_generation` snapshot pin and independent volume gates; they are not
rewired through a world-wide directory epoch or four-level radix forest.
...
content and a source object. Source-bound transfer plus a publication group
limited to the parent and pre-reserved children gives the required atomic
visibility without inventing a world-wide root/version system or new directory
resource pools used by every reader path.
...
individually without a shared confirmed gate, implicit deletion of unassigned
cells, GPU transport of `BaseContentSource`, and a four-level immutable
`WorldDirectoryEpoch` root used by all substrate readers solely to serve
extraction atomicity.

## T35. Multi-fidelity substrate hooks stay transport-bound; placement is a compact move batch

**Decision.** The CPU supplies region definitions through ordinary opaque
participant input. Moria requires one persistent GPU participant (not one
adapter/world per bubble), counted encoder limits, continued coarse work
outside all full-fidelity regions, one-time processing across overlapping
regions, continuous body/volume ownership across crossings, and non-stale
placements. Changed poses publish through one bounded placement stream: a
compact GPU batch of the existing scheduled move effect, validated as a unit
and installed through ordinary per-volume placement/revision gates. The stream
does not publish an alternate world-directory root or force atomic visibility
across unrelated moved volumes.

**Dispatch and harness.** Fixed maximum dispatch remains the substrate default.
The multi-fidelity proof harness used by C12/P11 may use a single body table,
Full/Halo/Coarse compact lists, hierarchical scans, a 65,536-body fixture, and
an exact 11-dispatch/3,604-workgroup private pipeline against declared maxima
16/8,192. Those internals qualify one portable adapter; they are not normative
requirements on every multi-fidelity consumer. P11 at empty, 1%, 50%, and 100%
active lists is blocking for the fixed-dispatch claim of that harness.
...
A compact GPU move batch reuses the baseline independent-publication rule while
removing that host loop. Baking one solver layout and dispatch graph into the
substrate would over-constrain adapters without improving transport bounds.
...
dispatch buffers, all-or-none alternate-directory-root publication for
unrelated volumes, and treating the qualification harness's body table /
class lists / exact dispatch count as the only legal multi-fidelity shape.
