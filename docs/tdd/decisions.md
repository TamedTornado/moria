...
T35 reuses ordinary opaque participant input for CPU-authored activity regions
and adds one compact placement stream so a single persistent GPU adapter can
keep Moria placements current while it runs multi-fidelity simulation in
adapter-owned state. Moria does not acquire region schemas, fidelity classes,
or classification algorithms; those remain adapter contracts proved by
validation. Fixed maximum dispatch is retained and P11 is blocking evidence
for that selection under multi-object placement load.

T36 adds optional fixed-stride opaque CPU egress in binding 7. Every enabled
adapter declares exact record/byte maxima; tick admission reserves map,
receipt, and byte capacity before execution. Publication may complete before
...
scheduled ABI accepted by the initial implementation. Ordinary single-volume
commits keep per-volume authority gates; only extraction and placement streams
publish multi-entry directory roots. The new types remain generic substrate
records. Moria does not acquire a timestep, region, velocity, physics,
collision-response, damage, weapon, debris, scoring, audio, or gameplay-event
model.
...
**Decision.** Activity regions arrive only as ordinary opaque participant
input. Moria does not declare region or fidelity types. One GPU adapter owns
one persistent body table across all regions and publishes changed Moria
placements through one bounded placement stream under a directory-epoch gate,
rather than per-object host move commands. Fixed maximum dispatch is retained;
P11 proves it under multi-object placement load for the proof adapter.

**Reason.** Geographic adapters/worlds would require accidental state
migration. Ordinary move admission per coarse object would force host
enumeration and queue overhead. The substrate needs only opaque region
transport and bulk placement authority; classification, coarse/full solvers,
and halo policy are adapter-owned and are validated rather than re-specified as
Moria architecture.

**Rejected.** GPU-selected activity regions, freezing objects outside all
regions, one adapter per bubble, Moria-owned fidelity class vocabularies,
silent stale placement, an unproven per-object host command loop, and raw
