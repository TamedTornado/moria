...
source-and-children directory-generation transaction that reuses the existing
snapshot pin/directory-generation mechanism. It cannot invent samples, attach a
...
regions. Region bytes remain opaque current input. The multi-fidelity
algorithm and exact dispatch topology remain inside the proof adapter; Moria
only transports region bytes, enforces declared dispatch/workgroup bounds, and
publishes changed poses through one GPU placement stream that fully
prevalidates then uses ordinary per-volume revision gates. Unrelated poses do
not share a world transaction.

T36 adds optional fixed-stride opaque CPU egress as a terminal CPU-consumer
edge on the existing outgoing handoff/readback transport. Every enabled adapter
declares exact record/byte maxima and schema; tick admission reserves edge
capacity from the handoff pools and an egress receipt before execution.
...
prefix. GPU-to-GPU handoffs remain on the existing GPU path.

Scheduled ABI v2 therefore has exactly seven group-0 bindings and is the only
...
## T34. Source-bound component extraction uses one directory-generation transaction
...
and children in unreferenced storage and publishes them together by advancing
the existing world `directory_generation` so readers pin either the complete
old source or the complete new source/children set.
...
Each child receives a pre-reserved ordinary durable `VolumeKey`. Minimal
parent/piece provenance and a complete sparse derived base replace a consumer
content source and persist through the existing scar/chunk machinery.
Uncheckpointed derived content remains dirty and subject to the existing
...
directory-generation/snapshot pin gives the required atomicity without
replacing every ordinary volume commit with a public world-epoch type or a
four-level copy-on-write directory.
...
individually without a shared generation pin, implicit deletion of unassigned
cells, GPU transport of `BaseContentSource`, a public `WorldDirectoryEpoch`
gate for all commits, global UUIDv5 derived-key namespaces, and monotonic
candidate-ordinal identity lifecycles.
...
fidelity classes, continues coarse work outside all regions, and compacts
changed Moria placements into one bounded placement stream. The stream fully
prevalidates on GPU and then publishes each pose through the existing
per-volume revision gate.

**Dispatch.** The substrate already enforces declared
`maximum_gpu_dispatches` / `maximum_gpu_workgroups`. The multi-fidelity
algorithm and exact proof-adapter dispatch topology are adapter-owned and are
proven by blocking P11 evidence; they are not additional Moria ABI types.
...
One table, deterministic union, halo, and compact placement batch preserve
continuous ownership and fresh Moria placements without forcing unrelated
poses into one world transaction.
...
stale placement, an unproven per-object host command loop, encoding fidelity
class names as Moria substrate types, and raw indirect dispatch buffers.
...
and exact record/byte maximum. Scheduled ABI v2 exposes that lane as a terminal
CPU-consumer edge on binding 3's existing outgoing handoff table. The adapter
...
GPU-to-CPU handoff readback path, and returns a
...
Working, staging, and host bytes release only after their respective handoff
milestones. Cancellation, shutdown, mapping, decoding, and device loss are
distinct terminal outcomes. Capacity is charged to existing handoff pools plus
egress receipt/record counts; no parallel egress device/staging/host/map family
and no binding 7 are introduced.

**Reason.** Adapters need generic CPU-visible facts without teaching Moria an
event vocabulary or reading back full solver state. Reusing the handoff
readback path preserves one accounting model while a separately bounded
...
7, and parallel egress-only device/staging/host/map accounting.
