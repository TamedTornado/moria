...
from one pinned source into pre-reserved dynamic child identities and
publishes them with one multi-volume `directory_generation` advance (receipt
token `WorldDirectoryEpoch`). It reuses the existing volume directory rather
than introducing a radix root/authority ABI for ordinary commits. It cannot
...
T35 keeps activity-region definitions on the existing opaque current-input
path. One persistent adapter owns multi-fidelity classification and body
state; Moria does not. That adapter continues coarse work outside every region
and publishes changed poses through one GPU placement stream rather than
...
## T34. Source-bound component extraction uses multi-volume directory publication
...
and children in unreferenced storage and installs every prepared
volume-directory fact with one multi-volume `directory_generation` advance.
Ordinary single-volume commits keep the baseline per-volume revision gate.
...
content and a source object. Source-bound transfer plus one multi-volume
publication of the existing directory gives the required capability without
widening that authority or inventing a parallel directory architecture.
...
`BaseContentSource`, and a world-wide radix directory/root-pool redesign for
ordinary commits.

## T35. Multi-fidelity uses opaque input plus a placement stream

**Decision.** The CPU supplies region definitions through ordinary opaque
participant input. Moria does not own region schemas, fidelity classes, or
body tables. One GPU adapter owns multi-fidelity classification and continues
coarse work outside all regions. Changed Moria placements publish through one
bounded placement stream and one multi-volume `directory_generation` advance
while each updated volume advances its revision.

**Dispatch.** The portable baseline keeps fixed maximum dispatch.
A proof adapter for 65,536 bodies declares maxima of 16 dispatches and 8,192
workgroups. P11 at empty, 1%, 50%, and 100% active lists is blocking.
Classification and fidelity kernels remain adapter-owned proof detail, not
Moria public vocabulary.
...
Opaque regions, one adapter, and a compact placement stream preserve continuous
ownership and fresh Moria placements with the smallest substrate surface.
