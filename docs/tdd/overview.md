...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Atomic component extraction, directory-generation swaps, placement streams, multi-fidelity adapter integration, opaque CPU egress, and Scheduled ABI v2 |
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   The closed exception that combines membership across volumes is atomic
   component extraction, published by one prepared directory-generation swap.
   A pre-reserved placement stream bulk-validates GPU updates and publishes
   them through ordinary per-volume placement gates.
...
16. Activity regions and fidelity classes are consumer/adapter data.
    ...
    full simulation.
17. Optional GPU-to-CPU egress is bounded opaque transport that reuses the
    handoff-shaped initialized-prefix and async map lifecycle.
    Empty, ready, pending, overflow, mapping failure, decode failure, and
    device loss are distinct, and publication never depends on CPU
...
- Component extraction installs its prepared source/child membership with one
  `directory_generation` swap. Placement streams bulk-validate on GPU and
  publish through ordinary per-volume placement gates. Per-volume revisions
  remain consumer freshness identities; unrelated proposals keep independent
  ordered receipts.
- GPU adapters may declare an optional handoff-shaped opaque CPU-egress lane.
  Moria stages and delivers initialized prefixes asynchronously without
  interpreting bytes or routing GPU-to-GPU handoffs through the CPU.
...
- Extraction records/bytes, live and lifetime volume records, versioned
  prepared directory-entry swap capacity, presentation
  ...
  updates/bytes, opaque-egress bytes/maps/receipts,
  and
...
- Public query/interest/result/dressing records, scheduled Behavior ABI v2,
  ...
  pre-reserved component-extraction identity map, and binding 7 is the optional
  handoff-shaped opaque CPU-egress terminal. None may be replaced by a shared-state side
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, prepared directory-generation swap, failure reclamation, and restore/rematerialization |
| Persistent multi-fidelity adapter integration | `behavior`, `gpu`, `volume` | Opaque CPU-authored regions, proof of one-time classification and halo continuity via adapter-owned state, fixed-dispatch bound, and placement stream |
| Opaque GPU-to-CPU adapter egress | `behavior`, `gpu`, `bevy` | Unknown opaque-byte round trip, empty/exact/overflow outcomes, ordered async readback, and failure reclamation |
...
multi-fidelity proof adapter. P12 qualifies atomic component extraction and
directory-generation publication. P13 qualifies bounded opaque egress including
readback latency and zero/exact/overflow honesty.
