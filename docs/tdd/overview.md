...
5. A placement change advances the same per-volume revision sequence as a
   ...
   The closed exceptions are a pre-reserved placement stream (per-volume
   gates after one validation) and atomic component extraction (fracture-scoped
   directory delta plus one directory-generation advance).
...
- Scheduled behavior uses one active tick per world, a builder-validated
  ...
  source-owned samples into pre-reserved child volumes. Arbitrary scheduled
  create and `BaseContentSource` transport remain forbidden.
- Component extraction publishes through a fracture-scoped prepared directory
  delta and one `directory_generation` advance so readers see the complete old
  source or the complete new source/children set. Placement streams validate
  once and publish through existing per-volume placement/revision gates.
  Per-volume revisions remain consumer freshness identities (T4).
- GPU adapters may declare an optional opaque CPU-egress byte maximum.
  Moria delivers initialized prefixes through the existing terminal handoff
  staging/map path without interpreting records or routing GPU-to-GPU peer
...
- Checkpoints store stable identities, placements, and sparse full-brick
  ...
  fingerprint; derived-extraction children store provenance, an internal empty
  base, and ordinary scars for transferred content. Checkpoints never store
...
- Extraction records/bytes, live and lifetime volume records, presentation
  ...
  placement updates/bytes, terminal-egress byte charge against handoff/staging,
  and
...
- Public query/interest/result/dressing records, scheduled Behavior ABI v2,
  ...
  blobs. Scheduled ABI v2 group 0 retains exactly six bindings: binding 5 is the
  participant's current read-only opaque input; binding 3 may carry the
  terminal consumer-egress edge; binding 4 carries prior feedback including
  piece-handle-to-child identity outcomes after extraction. None may be
  replaced by a shared-state side channel. Scheduled ABI logical 64-bit fields
  are always explicit low/high `u32` pairs, and next-tick feedback must preserve
  the complete documented terminal disposition/cause rather than a reduced
  category.
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, prior-feedback child-ID mapping, fracture directory-generation gate, failure reclamation, and empty-base-plus-scar restore |
...
directory-root publication. P13 qualifies bounded opaque egress including
