...
Scheduled ABI v2 also exposes closed source-bound component extraction,
bounded placement-stream publication, and optional opaque CPU egress on the
existing handoff/readback path; see
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   atomically. The sole scheduled exception is closed source-bound
   `ExtractComponents`, which publishes one source remainder-or-tombstone and
   its children as a single directory-generation transaction.
...
- Scheduled behavior uses one active tick per world, a builder-validated
  ...
  related substrate effects: a compact placement stream that still publishes
  through ordinary per-volume revision gates after complete prevalidation, and
  bounded extraction of source-owned samples into pre-identified child volumes
  as one source-and-children directory-generation transaction. Arbitrary
  scheduled create and `BaseContentSource` transport remain forbidden.
- GPU adapters may declare an optional fixed-stride opaque CPU-egress edge on
  the existing outgoing handoff/readback transport. Moria delivers initialized
  prefixes asynchronously with schema, overflow, correlation, and receipt
  semantics without interpreting records or routing GPU-to-GPU handoffs through
  the CPU.
...
- Checkpoints store stable identities, placements, and sparse full-brick
  ...
  fingerprint; derived-extraction children instead store minimal parent/piece
  provenance and their complete sparse derived base through the existing
  scar/chunk machinery. Checkpoints never store meshes or untouched external
  base bricks.
...
- Extraction records/bytes, live and lifetime volume records, presentation
  ...
  placement updates/bytes, egress receipts/records (bytes charged to handoff
  pools), and
...
- Public query/interest/result/dressing records, scheduled Behavior ABI v2,
  and asynchronous Extension ABI v1 layouts in `public-api.md`,
  `behavior-scheduling.md`, and `adapter-substrate-contracts.md` are normative.
  Do not replace closed variants, mandatory revision binding, or fixed offsets
  with implementation-defined blobs. Scheduled ABI v2 group 0 has exactly seven
  bindings: binding 5 is the participant's current read-only opaque input,
  binding 6 is the read-only pre-reserved component-extraction identity map,
  and optional CPU egress reuses binding 3 as a terminal handoff edge. None may
  be replaced by a shared-state side channel. Scheduled ABI logical 64-bit
  fields are always explicit low/high
...
- Every state transition and error variant needs a unit or headless-app test.
  Atomic publication, component-extraction matter conservation, child-ID GPU
  feedback, stale preconditions, observation gaps, restore mismatch, egress
  overflow, queue pressure, behavior first-participant input and upload
  failures, activity-region overlap/boundary continuity, continued coarse
  placement, tick ordering/composition, adapter-state nonownership, and device
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, one directory-generation transaction, failure reclamation, and restore/rematerialization |
| Persistent multi-fidelity adapter integration | `behavior`, `gpu`, `volume` | CPU-authored disconnected/overlapping regions, one-time classification, halo continuity, declared dispatch bounds, and coarse placement stream |
| Opaque GPU-to-CPU adapter egress | `behavior`, `gpu`, `bevy` | Unknown-schema round trip on the handoff path, zero/exact/overflow outcomes, ordered async readback, and failure reclamation |
...
P11 qualifies the multi-fidelity proof adapter's declared dispatch bounds and
compact placement publication. P12 qualifies atomic component extraction and
source-and-children directory publication. P13 qualifies bounded opaque egress
