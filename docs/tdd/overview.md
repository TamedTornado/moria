...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Atomic component extraction, placement batches, multi-fidelity adapter integration, opaque CPU egress, and Scheduled ABI v2 |
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   The closed exceptions are a pre-reserved placement batch, published through
   ordinary per-volume move/revision gates, and atomic component extraction,
   published by an extraction-local commit-group gate.
...
15. Atomic component extraction can publish only samples already owned by one
    ...
    child, or is explicitly reported as removed; pre-gate readers retain the
    source state and post-gate readers resolve the committed source/children
    group.
...
- Scheduled behavior uses one active tick per world, a builder-validated
  ...
  substrate effects: a compact placement batch and bounded extraction of
...
- Component extraction publishes through an extraction-local commit-group
  gate over prepared source and hidden child entries; placement batches
  publish through existing per-volume move/revision gates. Per-volume
  revisions remain consumer freshness identities.
- GPU adapters may declare an optional fixed-stride opaque CPU-egress lane.
  Moria reuses the GPU-to-CPU handoff/readback lifecycle and shared staging
  accounting, delivers initialized prefixes asynchronously without
  interpreting records, and keeps GPU-to-GPU handoffs on the GPU path.
...
- Extraction records/bytes, live and lifetime volume records, presentation
  ...
  updates/bytes, opaque-egress records/host bytes/receipts (device/staging
  charge shared staging), and
...
- Public query/interest/result/dressing records, scheduled Behavior ABI v2,
  ...
  The public surface for these transports is limited to capability descriptors,
  piece-to-child results, placement-batch results, and opaque egress outcomes.
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, extraction-local commit-group, failure reclamation, and restore/rematerialization |
| Persistent multi-fidelity adapter integration | `behavior`, `gpu`, `volume` | CPU-authored disconnected/overlapping regions, one-time classification, halo continuity, fixed-dispatch bound, and coarse placement batch |
| Opaque GPU-to-CPU adapter egress | `behavior`, `gpu`, `bevy` | Unknown-schema round trip, zero/exact/overflow outcomes, shared staging readback, and failure reclamation |
...
extraction-local commit-group publication. P13 qualifies bounded opaque egress including
shared-staging readback latency and zero/exact/overflow honesty.
