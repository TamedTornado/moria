...
source-bound extraction, a compact placement-stream path through ordinary
per-volume gates, and optional opaque CPU egress through the existing effect
and staging machinery.
...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Atomic component extraction, placement streams, multi-fidelity proof obligations, opaque CPU egress, and Scheduled ABI v2 sections |
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   atomically. Source-bound component extraction is the closed exception that
   publishes one source residual-or-tombstone with its pre-reserved children
   through one extraction-only directory generation swap.
...
- Scheduled behavior uses one active tick per world, a builder-validated
  ...
  revision per affected volume. Scheduled v2 still excludes arbitrary create
  and `BaseContentSource` transport. Source-bound extract-components may
  publish pre-reserved children from one pinned source through an
  extraction-only directory generation swap; placement streams publish
  compacted moves through ordinary per-volume gates; optional opaque egress
  uses the effect-output section and ordinary staging readback.
...
- Checkpoints store stable identities, source lineage plus an exact
  reconstruction fingerprint or minimal derived-extraction provenance,
  placements, and sparse full-brick scars. They never store meshes or
  untouched base bricks.
...
- Extraction records/bytes, live and lifetime volume records, presentation
  ...
  component-extraction child identities/egress retained host bytes/
...
- Public query/interest/result/dressing records, scheduled Behavior ABI v2,
  ...
  with implementation-defined blobs. Scheduled group 0 has exactly six
  bindings; binding 0 may append the child-reservation section, binding 1 may
  append the egress section, and binding 5 is the participant's current
  read-only opaque input and cannot be replaced by a shared-state side channel.
  Scheduled ABI logical 64-bit fields are always explicit low/high
...
- Scheduled effects in v2 are fill, patch, move, retire, placement stream, and
  extract components. Do not add arbitrary scheduled create, a public COW
  world-directory root for ordinary commits, or a `BaseContentSource` wire
  record. Extract components only redistributes samples from one pinned source.
...
P11 qualifies the multi-fidelity proof adapter's opaque-region integration and
placement-stream publication. P12 qualifies atomic component extraction and
its extraction-only directory generation swap. P13 qualifies bounded opaque
