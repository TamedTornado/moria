...
5. A placement change advances the same per-volume revision sequence as a
   ...
   The closed exceptions are a pre-reserved placement stream (ordered
   per-volume placement gates after one GPU validation) and atomic component
   extraction (one extraction-scoped commit of hidden pre-reserved children).
...
- Component extraction publishes source remainder and pre-reserved hidden
  children under one extraction-scoped confirmed-publication commit.
  Placement streams validate once on GPU then advance ordinary per-volume
  placement/revision gates. Per-volume revisions remain consumer freshness
  identities; unrelated proposals keep independent ordered gates and receipts.
- GPU adapters may declare an optional fixed-stride opaque CPU-egress lane as
  a terminal extension of the T28 handoff transport. Moria stages and delivers
  initialized prefixes asynchronously without interpreting records or routing
...
- Checkpoints store stable identities, placements, and sparse full-brick
  ...
  fingerprint; extraction children instead store provenance and ordinary
  full-brick scars over the internal empty base. Checkpoints never store meshes
  or untouched external base bricks.
...
- Extraction records/bytes, live and lifetime volume records, presentation
  ...
  GPU bytes/views/collision scratch/handoff maps and bytes (including terminal
  CPU egress charges)/proposals/double-buffered feedback/factory buffer counts
  and live bytes, component-extraction proposals/children/assignments/child
  bricks/bytes, placement updates/bytes, and asynchronous GPU extension
  registrations/WGSL bytes are separate named bounds. Do not make one pool
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, extraction-scoped commit, failure reclamation, and restore/rematerialization |
