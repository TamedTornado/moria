...
component extraction, one compact placement batch for GPU-resident persistent
...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Atomic component extraction, placement batches, multi-fidelity adapter ownership boundary, opaque CPU egress, and Scheduled ABI v2 |
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   The closed exception is atomic component extraction, which installs one
   source change and its pre-reserved children as one extraction transaction.
   Placement-batch entries still publish through ordinary per-volume move
   gates and do not require cross-volume atomicity.
...
15. Atomic component extraction can publish only samples already owned by one
    ...
    child, or is explicitly reported as removed; readers see either the
    pre-extraction source or the complete post-extraction source-plus-children
    set.
16. Activity regions and fidelity classes are consumer/adapter data.
    Moria transports their opaque current-tick bytes and accepts a bounded
    placement-update batch but never chooses regions, classifies bodies, or
    owns coarse or full simulation.
17. Optional GPU-to-CPU egress is bounded opaque transport on existing
    staging/map infrastructure. Zero records, pending delivery, overflow,
    mapping failure, decode failure, and device loss are distinct, and
    publication never depends on CPU interpretation of the bytes.
...
- Scheduled behavior uses one active tick per world, a builder-validated
  ...
  revision per affected volume. Scheduled ABI v2 adds a bounded placement-
  update batch and source-bound extraction into pre-identified child volumes.
  Arbitrary scheduled create and `BaseContentSource` transport remain
  forbidden.
- Component extraction installs its source change and children as one
  extraction-local transaction on the existing directory-generation and
  per-volume gate machinery. Placement-batch entries publish through ordinary
  per-volume move gates into the existing tick revision vector; cross-volume
  placement atomicity is not required.
- GPU adapters may declare optional bounded opaque CPU egress.
  Moria stages and delivers initialized prefixes asynchronously through the
  existing handoff/staging/map path without interpreting records or routing
  GPU-to-GPU handoffs through the CPU.
...
- Checkpoints store stable identities, placements, and sparse full-brick
  ...
  fingerprint; derived-extraction children store provenance and use an
  internal empty base plus ordinary full-brick scars. Checkpoints never store
  meshes or untouched
...
- Extraction records/bytes, live and lifetime volume records, versioned
  directory roots/nodes/entries/authorities, presentation
  ...
  component-extraction proposals/children/assignments/child bricks/bytes, placement
...
- Scheduled effects are fill, patch, move, retire, placement batch, and
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, extraction-local atomic install, empty-base scar rematerialization, and failure reclamation |
| Multi-fidelity adapter hooks | `behavior`, `gpu`, `volume` | CPU-authored regions via opaque input, proof-adapter classification/continuity, and placement batch through ordinary move gates |
| Opaque GPU-to-CPU adapter egress | `behavior`, `gpu`, `bevy` | Unknown-layout round trip, zero/exact/overflow outcomes, ordered async readback on existing staging paths, and failure reclamation |
...
feasibility gates P1–P12 are blocking physical-adapter receipts for each claimed
...
P11 qualifies atomic component extraction and empty-base scar rematerialization.
P12 qualifies bounded opaque egress including readback latency and
zero/exact/overflow honesty.
