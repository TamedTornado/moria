...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Atomic component extraction, multi-entry directory publication, placement streams, multi-fidelity adapter hooks, opaque CPU egress, and Scheduled ABI v2 |
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   component extraction, both published by one multi-entry directory-generation
   install.
...
- Each component-extraction or placement-stream proposal installs its prepared
  directory entries under one advanced `directory_generation`. Per-volume
  revisions remain consumer freshness identities; unrelated multi-entry
  proposals keep independent ordered gates and receipts.
...
- Extraction records/bytes, live and lifetime volume records, presentation
  ...
  component-extraction children/bytes, placement updates/bytes, opaque-egress
  records/bytes/maps/receipts, and asynchronous GPU extension
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, one multi-entry directory install, failure reclamation, and restore/rematerialization |
