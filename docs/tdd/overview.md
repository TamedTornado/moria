...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Atomic component extraction, bounded placement streams, opaque region input, opaque CPU egress, and Scheduled ABI v2 |
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   The closed multi-entry directory exception is atomic component extraction.
   A placement stream validates a bounded batch and publishes through existing
   independent per-volume gates.
...
- Scheduled behavior uses one active tick per world, a builder-validated
  ...
  stream that uses independent per-volume gates and bounded extraction of
  source-owned samples into pre-identified child volumes under one multi-entry
  directory generation. Arbitrary scheduled create and `BaseContentSource`
...
- Checkpoints store stable identities, placements, and sparse full-brick
  ...
  fingerprint; extraction children store minimal provenance and ordinary
  full-brick scars over an internal empty base. Checkpoints never store meshes
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, one multi-entry directory generation, failure reclamation, and empty-base+scar restore |
| Persistent multi-fidelity adapter integration | `behavior`, `gpu`, `volume` | Opaque CPU-authored disconnected/overlapping regions, proof-adapter continuity, fixed-dispatch bound, and placement stream |
...
multi-entry directory-generation publication. P13 qualifies bounded opaque
egress including readback latency and zero/exact/overflow honesty.
