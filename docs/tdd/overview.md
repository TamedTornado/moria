...
- Public query/interest/result/dressing records, scheduled Behavior ABI v2,
  ...
  blobs. Scheduled ABI v2 group 0 retains exactly six bindings. Binding 5 is
  the participant's current read-only opaque input. Binding 1 contains
  independently bounded and validated proposal/payload, canonical
  child-reservation, and optional opaque-egress subranges; an adapter mutation
  of reservation bytes invalidates its complete output. None may be replaced
  by a shared-state side channel. Scheduled ABI logical 64-bit fields are always explicit low/high
