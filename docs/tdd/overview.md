...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Atomic component extraction, multi-volume directory publication, placement streams, multi-fidelity adapter integration, opaque CPU egress, and Scheduled ABI v2 |
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   The closed exceptions are a pre-reserved placement stream and atomic
   component extraction, both published by one confirmed multi-volume directory generation.
...
- Component-extraction and placement-stream proposals reuse offline prepare
  plus one confirmed world-directory generation swap per proposal. Per-volume
  revisions remain consumer freshness identities; ordinary single-volume commits
  keep existing revision gates; unrelated multi-volume proposals keep
  independent ordered publications and receipts.
...
- Extraction records/bytes, live and lifetime volume records, versioned
  directory entries, presentation
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, one multi-volume directory generation, failure reclamation, and restore/rematerialization |
...
multi-volume directory publication. P13 qualifies bounded opaque egress including
