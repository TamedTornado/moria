...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Atomic component extraction, directory epochs, placement streams, multi-fidelity substrate boundary, opaque CPU egress, and Scheduled ABI v2 |
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   component extraction, both published by one world-directory epoch gate.
...
- Extraction records/bytes, live and lifetime volume records, multi-entry
  directory roots/entries, presentation
  ...
  updates/bytes, opaque-egress records/bytes/maps/receipts,
  and
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, one directory epoch, failure reclamation, and restore/rematerialization |
| Persistent multi-fidelity adapter integration | `behavior`, `gpu`, `volume` | Opaque CPU region input, adapter-owned multi-fidelity proof, fixed-dispatch bound, and compact placement stream |
