...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Atomic component extraction, placement streams, multi-fidelity adapter integration, opaque CPU egress, and Scheduled ABI v2 |
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   A placement stream batches independent per-volume placement gates; atomic
   component extraction is the closed multi-entry directory-generation
   transaction for one source and its reserved children.
...
- Scheduled behavior uses one active tick per world, a builder-validated
  ...
  stream of independent per-volume placement gates and bounded extraction of
...
- Extract-components publishes only its source and reserved children by
  advancing the existing `directory_generation` membership token already
  captured in `SnapshotToken`. Per-volume revisions remain consumer freshness
  identities; placement streams do not share that multi-entry transaction.
- GPU adapters may declare an optional fixed-stride opaque CPU-egress lane.
  Moria stages and delivers initialized prefixes asynchronously on the existing
  handoff/staging/readback path without interpreting records or routing
...
- Checkpoints store stable identities, placements, and sparse full-brick
  ...
  fingerprint; derived-extraction children instead store their pre-reserved
  key, provenance, and complete sparse derived base. Checkpoints never store
  meshes or untouched external base bricks.
...
- Extraction records/bytes, live and lifetime volume records, presentation
  ...
  placement updates/bytes, opaque-egress records/receipts (device/staging/map
  charges reuse handoff and generic staging pools), and asynchronous GPU
  extension registrations/WGSL bytes are separate named bounds. Do not make
  one pool silently own another except the explicit egress reuse of handoff
  and staging accounting.
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, one extraction directory-generation transaction, failure reclamation, and restore/rematerialization |
| Persistent multi-fidelity adapter integration | `behavior`, `gpu`, `volume` | CPU-authored disconnected/overlapping regions, one-time classification, halo continuity, fixed-dispatch bound, and coarse placement stream |
...
readback latency and zero/exact/overflow honesty on the shared handoff/staging
path.
