...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Source-bound component extraction, compact placement updates, CPU-authored activity-region integration, and opaque GPU-to-CPU egress |
...
- Scheduled behavior uses one active tick per world, a builder-validated
  ...
  revision per affected volume. Its only creation-shaped operation is bounded
  source-owned component extraction; arbitrary create and
  `BaseContentSource` transport remain forbidden.
- Existing transaction, placement, capacity, staging, and tick-result
  machinery owns compound source-to-child publication, packed placement
  updates, and optional opaque adapter egress. Activity regions and all
  full/coarse simulation policy remain consumer-owned.
...
- Checkpoints store stable identities, placements, and sparse full-brick
  scars. External volumes store lineage plus a reconstruction fingerprint;
  extracted children store provenance plus complete initial sparse bricks.
  They never store meshes or untouched external base bricks.
...
- Public query/interest/result/dressing records, scheduled Behavior ABI v1,
  ...
  blobs. Scheduled group 0 has exactly eight bindings: binding 5 is current
  read-only opaque input, binding 6 is the read-only candidate-child map, and
  binding 7 is bounded opaque CPU egress. None may be replaced by a
...
- Scheduled effects in v1 are fill, patch, move, retire, placement batch, and
  source-bound component extraction. Do not add arbitrary create,
  consumer-authored child content, debris/gameplay policy, or a
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Exact conservation, GPU-visible final IDs, one compound-transaction gate, and rematerialization |
| Multi-fidelity adapter substrate | `behavior`, `gpu`, `volume` | CPU-authored disconnected/overlapping regions, continuous identity, continued coarse work, and compact placements |
| Opaque GPU-to-CPU egress | `behavior`, `gpu`, `bevy` | Unknown-schema zero/exact/overflow delivery, asynchronous failure, and safe reuse |
...
| Scheduled external behavior seam | `behavior`, `bevy`, `gpu`, `command` | Purpose-built Moria-conforming CPU/GPU adapters proving ingress, isolation, handoffs, feedback, source-bound extraction, compact placements, and opaque egress without behavior-policy ownership |
...
feasibility gates P1–P11 are blocking physical-adapter receipts for each claimed
