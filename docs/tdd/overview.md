...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Atomic component extraction, placement stream, multi-fidelity adapter boundary, opaque CPU egress, and Scheduled ABI v2 |
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   The only closed multi-volume exception is atomic component extraction, which
   publishes one source update and its children under one directory-generation
   gate. A placement stream publishes many independent per-volume placement
   revisions through ordinary gates without host body enumeration.
...
15. Atomic component extraction can publish only samples already owned by one
    ...
    generation or the complete new set.
...
- Scheduled behavior uses one active tick per world, a builder-validated
  ...
  stream and bounded extraction of source-owned samples into pre-identified
  child volumes. Arbitrary scheduled create and `BaseContentSource` transport
  remain forbidden.
- Component extraction publishes source and children under one directory-
  generation gate. Placement streams reuse ordinary independent per-volume
...
- Public query/interest/result/dressing records, scheduled Behavior ABI v2,
  ...
  bounded opaque CPU-egress target. None may be replaced by a shared-state side
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, one directory-generation gate, failure reclamation, and restore/rematerialization |
