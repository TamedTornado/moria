...
| [adapter-substrate-contracts.md](adapter-substrate-contracts.md) | Atomic component extraction, compact placement streams, multi-fidelity substrate hooks, opaque CPU egress, and Scheduled ABI v2 |
...
5. A placement change advances the same per-volume revision sequence as a
   ...
   The closed exceptions are a pre-reserved compact placement stream
   (independent per-volume gates after whole-stream validation) and atomic
   component extraction (one source-bound multi-volume publication for the
   parent and its pre-reserved children only).
...
15. Atomic component extraction can publish only samples already owned by one
    pinned source. Each source sample remains in the source, transfers to one
    child, or is explicitly reported as removed; readers see the old directory
    or the complete new directory.
16. Activity regions and fidelity classes are consumer/adapter data.
    Moria transports their opaque current-tick bytes and bulk-publishes
    placements but never chooses regions, classifies bodies, or owns coarse or
    full simulation.
17. Optional GPU-to-CPU egress is bounded opaque transport.
...
- Scheduled behavior uses one active tick per world, a builder-validated
  ...
  revision per affected volume. Scheduled ABI v2 adds a compact placement
  stream (batched ordinary placement updates) and bounded extraction of
...
- `ExtractComponents` publishes only its source-bound parent/child volume set
  through existing per-volume gates as one confirmed all-or-none transaction.
  `PlacementStream` validates a compact GPU batch and publishes each entry
  through the same independent placement/revision gates as scheduled move.
  Per-volume revisions remain consumer freshness identities; unrelated volumes
  keep independent publication. No world-wide directory-epoch root rewires
  ordinary queries, collision, views, checkpoints, or presentation.
...
- Checkpoints store stable identities, placements, and sparse full-brick
  scars. External volumes store source lineage plus an exact reconstruction
  fingerprint; derived-extraction children instead store provenance and their
  complete sparse derived base. Checkpoints never store meshes or untouched
  external base bricks.
...
- Keep backend/runtime types out of the main facade. The scheduled GPU adapter
  ...
  encoder, participant-filtered read-only export and current consumer input,
  write-only proposal/handoff targets, and read-only prior feedback. It must
...
- Extraction records/bytes, live and lifetime volume records, versioned
  presentation artifact/dirty/job/mesh/instance pools, dressing registrations,
  scheduled behavior registrations/order edges/per-participant input records
  and host/GPU bytes/views/collision scratch/handoff maps and bytes/proposals/
  double-buffered feedback/factory buffer counts and live bytes,
  component-extraction proposals/children/assignments/child bricks/bytes,
  placement updates/bytes, opaque-egress records/device/staging/host
  bytes/maps/receipts, and asynchronous GPU extension registrations/WGSL bytes
  are separate named bounds. Do not make one pool silently own another.
  Extraction and placement reuse the baseline volume directory and
  per-volume gates; they do not introduce world-wide directory-root or radix
  pools.
...
- Public query/interest/result/dressing records, scheduled Behavior ABI v2,
  ...
  blobs. Scheduled ABI v2 group 0 has exactly eight bindings: binding 5 is the
  participant's current read-only opaque input, binding 6 is the read-only
  pre-reserved component-extraction identity map, and binding 7 is the optional bounded
  opaque CPU-egress target. None may be replaced by a shared-state side
  channel. Scheduled ABI logical 64-bit fields are always explicit low/high
...
- Scheduled effects are fill, patch, move, retire, placement stream, and
  extract components. The last operation may create only pre-reserved dynamic
  children whose cells come from one pinned source. Do not add arbitrary
  scheduled create, behavior-defined child content, debris/physics policy, or
  a `BaseContentSource` wire record.
...
- Every state transition and error variant needs a unit or headless-app test.
  Atomic publication, component-extraction matter conservation, child-ID GPU feedback,
  stale preconditions, observation gaps, restore mismatch, egress overflow,
  queue pressure, behavior first-participant input and upload failures,
  activity-region overlap/boundary continuity, continued coarse placement,
  tick ordering/composition, adapter-state nonownership, and device loss
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, source-bound multi-volume publication, failure reclamation, and restore/rematerialization |
| Persistent multi-fidelity adapter integration | `behavior`, `gpu`, `volume` | CPU-authored disconnected/overlapping regions, one-time classification, halo continuity, fixed-dispatch proof harness, and compact placement stream |
| Opaque GPU-to-CPU adapter egress | `behavior`, `gpu`, `bevy` | Unknown-schema round trip, zero/exact/overflow outcomes, ordered async readback, and failure reclamation |
...
P11 qualifies fixed maximum dispatch and compact placement publication for the
multi-fidelity proof harness. P12 qualifies atomic component extraction and
source-bound multi-volume publication. P13 qualifies bounded opaque egress
including readback latency and zero/exact/overflow honesty.
