...
5. A placement change advances the same per-volume revision sequence as a
   ...
   component extraction, both published by one multi-volume
   `directory_generation` advance.
...
- Each component-extraction or placement-stream proposal installs its prepared
  multi-volume directory facts and advances the existing
  `directory_generation` once (`WorldDirectoryEpoch` on the receipt). Per-volume
  revisions remain consumer freshness identities; unrelated multi-volume
  proposals keep independent ordered publications and receipts. Ordinary
  single-volume commits keep the baseline per-volume revision-gate path.
...
| Atomic GPU component extraction | `behavior`, `storage`, `volume`, `persistence`, `gpu` | Source-to-children conservation, GPU child-ID mapping, one multi-volume publication, failure reclamation, and restore/rematerialization |
