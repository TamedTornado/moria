...
WorldPresentationRoot
  ...
  ObjectLayer
    ...
    HorizonObjectCell (stable 64 m cell key/revision)
      HorizonTreeCluster (eligible base cards)
      HorizonDerivedObject (edited current-truth payload or empty tombstone)
  DressingLayer
...
When `WorldEditWrite` accepts a request during rendered frame `F` and a fixed tick later commits batch revision `R`:
...
6. Render extraction copies each revisioned payload/removal, including any active Horizon-cell aggregate repartition and derived-object/tombstone, render-asset preparation creates/frees the corresponding GPU buffers, and the queue stage acknowledges its draw/removal key. Once all primary-focus keys through a reported revision are acknowledged, telemetry emits `EditPrimaryPresentationReady`. After every accepted batch and all affected keys reach the final request revision, it emits `EditReconciliationComplete`.

A 3 m sphere can intersect at most 27 4 m bricks and remains the interactive demo workload. It does not size the substrate queue or define the maximum edit. Progressive operations traverse canonical brick ranges without materializing the full volume; immutable snapshots, shared terrain material, bounded per-frame upload/install budgets, and fair request scheduling apply at every scale. Gate F2 measures the production path on the M4 for the interactive carve, eight colony-style streams, and a 16 m catastrophic carve: admission, staging/commit batches, dirty/object discovery, dependency eligibility, snapshots, terrain/object extraction, seams, dressing remove/install, Bevy mesh/entity installation, GPU accounting, primary readiness, and full render-sub-app reconciliation. If frame pacing, progress/fairness, barrier completeness, or the discovery budget fails, downstream implementation stops under [implementation-plan.md](implementation-plan.md).

Because collision updates per committed batch before mesh installation, a consumer can observe new truth while the corresponding presentation is still pending. The demo freezes its one debug action until primary presentation reaches that request; games may instead display progress or effects. The system never delays committed collision to match rendering because material truth is authoritative, and it never claims full reconciliation from a primary-focus acknowledgement.
...
- Horizon trees use generated card clusters per aligned 64 m cell; the underlying placements, IDs, dependencies, and voxel truth remain separate.
...
The dependency is the extractor-declared union of all object, adjacent terrain/ruin, gradient, material, provenance, and downsampling inputs that can affect that object's emitted primitives. It is represented by a fixed-size conservative AABB and exact lazy membership against the placement's analytic shape plus the shared finite extraction stencil, never by retained coordinates. Only an active object's bounded footprint probes sparse delta bricks. Any matching delta—regardless of base owner—removes the authored form and replaces it with a material-aware mesh retaining only globally extracted primitives tagged with that ID. Further dependency edits revision that mesh. The authored form returns only when the dependency has no delta. Thus an adjacent terrain-only place/dig can occlude or expose the object without leaving a stale GLB or Horizon card, and reverting object cells while such a boundary delta remains cannot restore either. Placed/digged deltas still do not acquire the owner of a neighboring object: regenerated base provenance determines payload routing, including across a single edit sphere.

Near/Middle/Far apply this rule to the per-placement GLB/derived root. Horizon applies the identical eligibility result to a cell group: intact tree IDs contribute one canonical base card to `HorizonTreeCluster`; ineligible IDs are excluded and use one owner-filtered 4 m `HorizonDerivedObject`. A fully erased tree contributes an acknowledged empty tombstone. Tree anchors assign cells deterministically, and cluster/derived ID lists are sorted and disjoint. The group records source revision and request token and installs atomically; stale results cannot overwrite a newer partition. Active relevant edits pin and rebuild the group and include every aggregate buffer create/update/free and derived payload/removal in request reconciliation. Far-to-Horizon transition waits for the complete current partition before retiring Far roots. Eviction removes installed presentation but never deltas; reactivation repartitions from current truth.
...
- a 3 m hillside tunnel carve whose committed cut becomes walkable without a frame hitch and whose old grass and stale surface disappear by terminal reconciliation;
- visible progressive change during a 16 m catastrophic carve, with no frozen frame, stale resurrection, or false completion;
