...
6. Render extraction copies each revisioned payload/removal, including any active Horizon-cell aggregate repartition and derived-object/tombstone, render-asset preparation creates/frees the corresponding GPU buffers, and the queue stage acknowledges its draw/removal key. Only after every barrier key has this render-sub-app acknowledgement does telemetry emit `EditSurfaceReady`. Its `ready_frame` must be no greater than submitted frame `F + 2` for the 3 m operation.

A 3 m sphere can intersect at most 27 4 m bricks; dirty face-neighbor seams enlarge the task set but not authoritative edit cells. The two reserved/preemptive worker lanes, immutable snapshots, shared terrain material, and 16 MiB/frame upload budget are sized for this path. Gate F2 measures the actual full path on the M4 before broad generation: staging/mutation, dirty/object discovery, dependency eligibility, immutable snapshots, terrain/object extraction, seams, dressing remove/install, Bevy mesh/entity installation, GPU upload accounting, and render-sub-app extraction acknowledgement. The stress trial uses F1's maximum-candidate full-forest target, and discovery plus eligibility must consume no more than 1.0 ms. If either the signature or stress trial misses two frames, 33.3 ms frame time, barrier completeness, or the discovery budget, downstream implementation stops and the TDD returns for revision under [implementation-plan.md](implementation-plan.md); the implementation may not weaken the event, workload, forest, resolution, or timing definition.
...
Near/Middle/Far apply this rule to the per-placement GLB/derived root. Horizon applies the identical eligibility result to a cell group: intact tree IDs contribute one canonical base card to `HorizonTreeCluster`; ineligible IDs are excluded from that buffer and contribute one owner-filtered 4 m `HorizonDerivedObject`. A fully erased tree contributes an acknowledged empty tombstone, so it remains excluded without drawing. Tree anchors assign cells deterministically, and cluster/derived ID lists are sorted and disjoint. The group records source revision and request token and installs atomically; stale results cannot overwrite a newer partition. Active relevant edits pin and rebuild the group and include every aggregate buffer create/update/free and derived payload/removal in `EditSurfaceReady`. Far-to-Horizon transition waits for the complete current partition before retiring Far roots. Eviction removes the installed group and GPU buffers but never deltas; reactivation and a post-load active-cell rebuild repartition from current deltas. Only immutable base card descriptors may survive by cell key, preventing an edited or erased tree from reappearing through a stale cluster cache.
