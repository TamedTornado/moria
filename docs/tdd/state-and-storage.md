...
### Extraction-scoped publication and bulk placement

Ordinary snapshots continue to sort live volumes by stable key and read their
revision/placement gates in one ordered compute pass. That capture is not a
cross-volume transaction (T4). Component extraction and placement streams
reuse the baseline unreferenced prepared versions, per-volume publication
gates, pinned snapshots, and directory membership rules rather than a
world-wide directory epoch or four-level radix root.

`ExtractComponents` pre-reserves child live/lifetime identities and builds
source remainder and child bricks only in unreferenced slots. Child candidates
remain hidden—absent from the live directory and from every snapshot, query,
collision, and presentation capture—until one ordered publication dispatch
installs the source revision gate (or retirement) and every selected child
directory entry together under the ordinary confirmed-publication boundary.
No consumer reader, observation, or receipt is submitted behind that commit
until queue completion confirms it. Before confirmation the source still owns
every sample; after confirmation the source remainder and children form one
complete ownership partition. Device loss of an unconfirmed generation
discards an unobservable candidate and never rolls back a confirmed
extraction.

`PlacementStream` validates a GPU-compacted batch once, then advances each
addressed volume's ordinary placement/revision gate in one ordered pass. It
does not install an alternate world-directory root and does not promise
cross-volume atomic placement visibility.
...
A scheduled `PlacementStream` reuses that same per-volume gate: after one GPU
validation of a compacted update batch, an ordered publication pass advances
each addressed volume's placement/revision gate without host body enumeration
or per-object `VolumeCommand::Move` objects. Unrelated volumes retain
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records plus ordered per-volume placement-gate advances |
...
| Scheduled opaque egress (via handoff pools) | charged to handoff maps/bytes | fixed-stride terminal consumer extension of T28 handoff transport; zero/exact/overflow/failure are distinct |
...
component-extraction/placement/handoff-including-egress/opaque-resource/live-factory-buffer-byte/WGSL, and extension-registry
...
- component extraction without confirmed extraction-commit publication releases
...
- placement-stream publication follows the same confirmed per-volume gate
  boundary as ordinary Move;
