...
### Prepared directory-generation swap for component extraction

Baseline snapshots already pin `directory_generation` before resolving volume
revisions and placements. Ordinary single-volume matter and placement commits
continue to publish through their existing per-volume revision/placement gates.

Scheduled component extraction is the closed exception that prepares several
directory entries (source remainder or tombstone plus every published child)
in unreferenced slots and installs them with one `directory_generation` swap.
Readers that pin the old generation observe the complete pre-extraction
membership; readers that pin the new generation observe the complete
post-extraction membership. The coordinator withholds subsequent consumer
readers, observations, and receipts until the swap submission is
queue-confirmed. Loss of an unconfirmed generation therefore discards an
unobservable candidate membership set and reconstructs the still-pinned old
directory; it does not guess whether a consumer-visible commit occurred.

Placement streams do not use a multi-entry generation swap. After GPU
validation they publish through ordinary per-volume placement/revision gates
in one ordered pass, preserving independent-volume revision semantics.
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured GPU bulk placement stream; publishes through ordinary per-volume gates |
| Scheduled component-extraction proposals / children / assignment cells / child bricks / bytes | 16 / 256 / 262,144 / 4,096 / 32 MiB | configured; children fit live/lifetime records and prepared directory-generation swap scratch |
| Scheduled adapter CPU-egress bytes / maps / receipts | 16 MiB / 16 / 64 | configured handoff-shaped GPU-to-CPU terminal; reuses staging/map lifecycle |
...
- component extraction without confirmed directory-generation publication releases
