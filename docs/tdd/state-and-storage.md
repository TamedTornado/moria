...
### Component-extraction publication

Ordinary matter, move, and retire operations continue to publish through the
existing per-volume revision and placement gates described below.
`ExtractComponents` is the closed exception that must install one source change
and every published child without intermediate visibility.
It prepares unreferenced source remainder or tombstone state, child directory
entries, empty-base child content, and full-brick scars, then installs them as
one extraction-local transaction ordered on the existing directory-generation
and gate machinery so readers observe either the complete pre-extraction source
or the complete post-extraction source-plus-children set.
Placement-batch entries do not use that multi-entry install: each publishes
through the ordinary per-volume move gate independently.
No public world-directory epoch type is exposed.

Derived-extraction children carry a provenance record and `source_kind =
DerivedExtraction` so restore can rematerialize empty-base plus scars without a
consumer content source. External volumes retain their ordinary content-source
lineage and reconstruction fingerprint.

The exact component-transfer, child frame, placement-batch, and conservation
protocol is in
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records plus ordinary per-volume move-gate versions |
...
component-extraction/placement/opaque-resource/live-factory-buffer-byte/WGSL, and extension-registry
...
- component extraction without confirmed extraction-transaction publication
  ...
  ownership;
- placement-batch entries follow ordinary per-volume move-gate confirmation;
