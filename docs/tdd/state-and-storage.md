...
keyed by the 16-bit runtime volume index. Slot index zero is null in every
pool; only the explicitly declared authority gate inside a live entry mutates.
...
Tombstones retain volume ID/key and terminal revision; `mode`,
`active_authority_index`, cell/domain/source/provenance facts, and reserved
words are zero. Host/WGSL offset, tag, null-index, finite-value, and
reserved-word tests freeze this internal authority ABI. The Rust `[u8; 16]`
key mirror is four `u32` words in WGSL with identical little-endian wire bytes.
