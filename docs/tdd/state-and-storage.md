...
### Extraction commit-groups and placement batches

Ordinary volume commits continue to use the existing directory and per-volume
revision/placement gates described below. Snapshots still couple each captured
volume's revision with its placement; they do not imply a simultaneous
cross-volume transaction.

`ExtractComponents` is the closed multi-entry exception. It prepares source
remainder (or retirement) and hidden child entries in unreferenced storage,
then publishes them through an extraction-local commit-group gate referenced
only by those prepared records. Old snapshots retain the pre-extraction source
state; new readers resolve the committed source/children group together.
Directory layout details for that gate remain internal.

`PlacementBatch` validates one compact GPU payload and publishes each entry
through the ordinary per-volume move/revision path in a bounded GPU pass.
Cross-volume atomicity is not required for placement.

The exact component-transfer, child frame, placement-batch, conservation, and
commit-group protocol is in
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records published through ordinary per-volume move/revision gates |
...
| Scheduled opaque egress receipts / records / host bytes | 64 / 16,384 / 16 MiB | configured distinct receipts with shared staging/maps for GPU-to-CPU readback; zero/exact/overflow/failure are distinct |
...
- component extraction without confirmed commit-group publication releases
...
- placement-batch publication uses ordinary per-volume revision-gate
  confirmation;
