...
### Component-extraction multi-entry directory transaction

Ordinary commits and placement streams use the baseline per-volume
revision/placement gates below. The only closed multi-entry directory
transaction is scheduled component extraction: Moria prepares unreferenced
directory entries for the source remainder or tombstone and every published
child, then installs one new `directory_generation` so readers observe either
the complete old source or the complete new source/children set. Placement
streams do not use that multi-entry gate. Protocol details are in
...
A scheduled placement stream validates its bounded update batch once, then
publishes each admitted entry through these same independent per-volume
gates. It does not install a multi-entry directory generation.
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records plus independent per-volume gate capacity |
| Scheduled component-extraction proposals / children / assignments / child bricks / bytes | 16 / 256 / 262,144 / 4,096 / 32 MiB | configured and completely reserved before execution, including live/lifetime IDs, pages, scars, transfer, provenance, and one prepared directory generation |
...
- component extraction without confirmed multi-entry directory-generation
  publication releases unpublished child identities after old-generation
  quarantine; confirmed publication remains applied and never rolls back to
  parent-only ownership;
- placement-stream publication follows ordinary independent per-volume gate
  confirmation;
