...
A scheduled placement-stream proposal reuses these same per-volume gates after
batch GPU validation. It does not allocate an alternate world-directory root
for unrelated placement updates. The stream contract is normative in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).

## Component-extraction generation swap

Ordinary single-volume matter and placement commits continue to use the
per-volume revision gates above. Source-bound component extraction is the only
scheduled operation that also advances the world `directory_generation`
already present on `SnapshotToken` and used for offline page-table rebuild.

Before adapter execution, Moria installs prebuilt invisible child directory
entries from reserved identity slots. Construction builds the source residual
and child full-brick scars against the canonical empty internal base in
unreferenced storage. One extraction-only generation swap then atomically
promotes the residual-or-tombstone and every published child into the live
directory generation. Readers that hold the old generation continue to see the
pre-extraction source only. Confirmed publication is required before
observations and receipts; unconfirmed device loss never exposes the candidate
generation. Full reservation, conservation, identity, and failure rules are
normative in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
| Scheduled behavior proposals / payload / affected cells / affected bricks / directory effects / conflict checks / feedback | 1,024 / 64 MiB / 262,144 / 4,096 / 16 / 1,048,576 / 1 MiB | configured and wholly reserved/bounded before adapters run; extraction assignments, placement entries, child bricks, and directory-entry updates charge these same pools |
...
| Scheduled component-extraction child identities | 256 | configured; the only extraction-specific identity headroom beyond ordinary create; live/lifetime volume records cover initial live records plus this reserved maximum |
| Scheduled opaque egress retained host bytes | 16 MiB | configured decoded egress deliveries retained until result drop; device copy and staging maps charge existing staging/map pools |
...
dirty/artifact/instance, behavior input/view/collision/handoff/proposal/
component-extraction-child/egress-host/feedback/
...
- an active scheduled behavior tick without confirmed revision-gate or
  extraction generation-swap completion reports typed no-publication device
  loss; a confirmed publication remains published and never rolls back to
  parent-only ownership; late adapter callbacks, mapped handoffs, feedback,
  and pending egress maps from the old generation are quarantined; old-
  generation consumer-input uploads are also quarantined and cannot reach an
  adapter; the first recovered GPU tick receives typed
  `UnavailablePreviousGeneration` feedback;
- component extraction without confirmed directory-generation publication
  releases unpublished child identities after old-generation quarantine;
  placement-stream entries follow ordinary per-volume gate rules;
