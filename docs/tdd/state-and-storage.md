...
Scheduled placement streams reuse these same per-volume gates after complete
batch prevalidation. They do not install a shared world transaction for
unrelated poses.

## Source-and-children extraction publication

Closed scheduled `ExtractComponents` is the sole multi-volume publication
exception. After all child and source remainder matter is prepared in
unreferenced slots, Moria installs the source remainder-or-tombstone and every
child directory entry under one candidate world `directory_generation` image
and publishes that image with the ordinary confirmed-generation swap used by
directory rebuilds. Readers pin `directory_generation` on
`SnapshotToken` before resolving entries, so they observe either the complete
old source or the complete new source/children set.
Ordinary single-volume matter and placement commits continue to use only their
per-volume revision gates and do not acquire a public world-epoch type or a
four-level copy-on-write directory.

The exact transfer, conservation, identity, and failure protocol is normative
in [adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records; publish through ordinary per-volume gates |
...
| Scheduled opaque egress receipts / records | 64 / 16,384 | configured terminal outcomes; device/staging/host/map capacity charged to handoff pools |
...
component-extraction/placement/egress/opaque-resource/live-factory-buffer-byte/WGSL,
