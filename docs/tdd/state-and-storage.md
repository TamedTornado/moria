...
### Fracture-scoped directory deltas

Ordinary commits continue to use the existing per-volume revision and
placement publication gates. Snapshots still capture
`SnapshotToken.directory_generation` before reading volume gates; that
generation is the baseline structural token already advanced by directory-table
rebuilds and, for this amendment, by confirmed extract-components publication.

`ExtractComponents` is the only operation that must make several directory
entries appear or change together: the source remainder or tombstone plus zero
or more pre-reserved children. It does so with a **fracture-scoped prepared
directory delta**, not a world-wide immutable radix root or a
`WorldDirectoryEpoch` captured by every query, collision traversal, scheduled
view, checkpoint, presentation job, and ordinary commit.

```text
FractureDirectoryDelta {
    expected_directory_generation,
    expected_source_revision,
    source: prepared live remainder or tombstone,
    children: prepared pre-reserved live entries,
    transfer and validation sentinels...
}
```

Construction writes only unreferenced source/child page versions, brick slots,
and delta entries. A separate ordered publication dispatch verifies the
expected generation and source revision, installs every prepared child
directory slot, publishes the source gate, and advances
`directory_generation` by one checked step. The coordinator withholds
consumer readers, observations, and receipts until that submission is
queue-confirmed. Loss of an unconfirmed generation therefore discards an
unobservable candidate delta and reconstructs the still-pinned pre-delta
directory.

Readers that captured the pre-publication generation cannot resolve any child
or source remainder from the delta; readers that captured the post-publication
generation observe the complete source/children set. Unrelated volumes keep
independent publication (T4). Placement streams never allocate a directory
delta: after whole-stream validation they publish each entry through ordinary
per-volume placement gates.
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records published through per-volume placement gates |
| Scheduled component-extraction proposals / children / assignments / child bricks / bytes | 16 / 256 / 262,144 / 4,096 / 32 MiB | configured and completely reserved before execution, including live/lifetime IDs, pages, scars, transfer, provenance, and fracture deltas |
| Scheduled opaque egress bytes | 1 MiB | configured terminal handoff/staging charge; zero/exact/overflow/failure are distinct |
...
- component extraction without confirmed directory-generation publication
  ...
  confirmed publication remains applied and never rolls back to parent-only
  ownership;
- placement-stream publication follows ordinary per-volume gates under the same
  confirmed-submission boundary;
