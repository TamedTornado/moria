...
Snapshots capture a checked nonzero `WorldDirectoryEpoch` together with the
per-volume revision/placement facts they resolve. Ordinary single-volume
matter and move commits keep the existing per-volume authority gate: they
prepare one unreferenced revision/placement version and publish that volume
alone. They do not rebuild a multi-entry directory root.

Scheduled placement streams and extract-components are the closed exceptions.
Each prepares every changed directory entry (and, for extraction, transferred
matter) in unreferenced storage, then installs one complete alternate
directory root under a single epoch gate per proposal. Multiple root-affecting
proposals in a tick form a stable root chain and publish in
participant/proposal order. Failure may reject one proposal while another
valid proposal publishes under its policy, but device loss before confirmation
exposes none of the tick's candidate roots and never part of one proposal.

The multi-entry root is a capacity-bounded directory snapshot: a header naming
the epoch and live count, plus entry records that carry volume ID/key, live or
tombstone state, mode, cell size/domain, source kind
(`External` or `DerivedExtraction`), provenance index when derived, and the
selected revision/placement authority slot. Structural sharing or denser
encodings are implementation choices; they must preserve epoch/all-or-none
visibility and charge conservative root and entry permits at admission.
Public limits therefore name `directory_roots` and `directory_entries`, not a
particular tree fanout.
...
| Directory roots / directory entries (multi-entry publication) | 64 / 131,072 | adapter/config bound; current, prepared, and reader-pinned multi-entry roots and their entry versions |
...
| Scheduled opaque egress maps / receipts / records / bytes | 16 / 64 / 16,384 / 48 MiB | configured independent GPU-to-CPU transport; zero/exact/overflow/failure are distinct |
