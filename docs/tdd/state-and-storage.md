...
## Compound source-to-child publication

GPU component extraction reuses the mutation transaction above. Prepared
source and child authority heads live in reserved, unreferenced slots and name
one `CompoundVolumeTransaction`. Snapshot and directory lookup follows the
previous head while that transaction is prepared; a prepared child therefore
does not exist. After every head, page, scar, and observation is installed and
validated, one ordered compare-exchange changes the transaction from prepared
to committed against the pinned source revision.

That transaction word is the only additional publication gate. Before it,
readers see the old source. After it, readers see the source remainder and all
children. Failure leaves every prepared head invisible and reclaims it after
last use. This extends the existing version-chain lookup and reservation
machinery; it does not add a second versioned world directory. The complete
contract is in
...
| Scheduled behavior opaque egress records / device+staging+host bytes | 16,384 / 48 MiB | configured; exact aggregate capacity is reserved before adapters run |
...
egress/opaque-resource/live-factory-buffer-byte/WGSL, and extension-registry
