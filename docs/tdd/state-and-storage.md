...
### Source-bound multi-volume publication

Ordinary matter, placement, create, and retire continue to use the baseline
per-volume revision/placement gates and the world-scope snapshot token above.
They do not acquire a world-wide directory epoch or four-level radix root.

Scheduled `ExtractComponents` is the only operation that must make several
directory identities visible together: the source remainder or retirement and
every published child. It prepares those identities in unreferenced storage,
then installs only that source-bound set through the existing volume gates in
one ordered, all-or-none publication dispatch under the ordinary
confirmed-publication boundary. Unrelated volumes are not participants in that
gate.

Scheduled `PlacementStream` is a compact GPU batch of ordinary placement
updates. After whole-stream validation it publishes each entry through the
same independent per-volume placement/revision gate used by scheduled move.
It does not install an alternate world directory root or require atomic
visibility across unrelated moved volumes.

The exact component-transfer, child frame, placement-stream, conservation, and
failure protocol is in
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records published through ordinary per-volume placement gates |
...
- component extraction without confirmed multi-volume publication releases
...
- placement-stream publication follows ordinary per-volume gate confirmation;
