...
### Component extraction and placement stream

The only closed multi-volume publication exception is `ExtractComponents`: one
proposal prepares a source remainder-or-tombstone plus zero or more children in
unreferenced storage and installs those directory entries under one
`directory_generation` bump. Readers either see the old source alone or the
complete new set; they never observe a half-published extraction. Ordinary
single-volume matter and placement commits retain the per-volume revision gates
below. A placement stream validates many placement updates in one GPU proposal
and then publishes each update through those ordinary independent per-volume
placement/revision gates—batched for efficiency, not rewrapped as a second
global multi-volume epoch. Exact transfer, conservation, child frame, identity
mapping, and egress contracts live in
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records plus ordinary per-volume placement preparation |
...
- component extraction without confirmed directory-generation publication
  ...
  ownership;
- placement-stream entries that have not completed ordinary per-volume
  placement publication release without advancing those revisions;
