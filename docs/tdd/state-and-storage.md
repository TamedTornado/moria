...
### Multi-volume publication tokens

Ordinary single-volume commits keep the per-volume revision/placement gate
model below. Scheduled `ExtractComponents` and `PlacementStream` proposals are
the only closed operations that install several volume-directory facts
together. They prepare every entry offline, publish all of those facts in one
ordered multi-volume pass, and advance the world's existing
`directory_generation` once as the linearization point. Snapshots already
capture `directory_generation` before resolving volume entries, so a reader
observes either the complete pre-proposal directory or the complete
post-proposal set for that proposal.
...
`directory_generation`. It is not a second directory data structure, a radix
root pool, or a cross-volume gameplay clock. Multiple multi-volume proposals
in one tick publish in participant/proposal order, each with its own
generation advance and independent receipt. Failure may reject one proposal
while another valid proposal publishes under its policy, but device loss
before confirmation exposes none of the tick's candidate multi-volume sets.
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records; multi-volume publication reuses the existing directory generation |
...
- placement-stream publication follows the same multi-volume
  `directory_generation` boundary;
