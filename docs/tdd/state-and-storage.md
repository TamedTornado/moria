...
### Multi-volume directory publication

Ordinary single-volume matter and placement commits keep the existing
per-volume revision/placement gates below. They are not rewritten through a
new directory ABI.

Scheduled placement streams and extract-components are the only closed
operations that change several directory entries together. They reuse the
existing offline-prepare and world-directory generation-swap model already used
for directory table rebuilds: construction writes unreferenced page versions,
directory entries, and child records; a separate ordered publication dispatch
installs one new `directory_generation` that makes the complete prepared entry
set visible together. Readers that acquire a world-scope snapshot capture
`directory_generation` before resolving volume entries, so they observe either
the complete old set or the complete new set for that proposal. Multiple such
proposals in a tick publish in participant/proposal order as independent
confirmed generation installs. Failure may reject one proposal while another
valid proposal publishes under its policy, but device loss before confirmation
exposes none of that proposal's candidate directory and never part of one
proposal.

The coordinator withholds subsequent consumer readers, observations, and
receipts until the generation's submission is queue-confirmed. Loss of an
unconfirmed generation therefore discards an unobservable candidate directory
and reconstructs the still-pinned old generation; it does not guess whether a
consumer-visible commit occurred. Per-volume revisions remain the freshness
identity exposed for later single-volume work. Receipts for the two multi-
volume effects report the published `DirectoryGeneration` so consumers can
correlate that visibility point.
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records plus multi-volume directory publication capacity |
...
- component extraction without confirmed multi-volume directory publication
  ...
  ownership;
- placement-stream publication follows the same directory-generation boundary;
