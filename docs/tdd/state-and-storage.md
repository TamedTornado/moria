...
### Extraction-scoped directory generation

`SnapshotToken.directory_generation` remains the world-directory membership
token already used for page-table rebuild swaps and snapshot capture.
Ordinary single-volume matter and placement commits continue to publish through
per-volume revision/placement gates only; they do not rebuild or replace the
directory for unrelated volumes.

Scheduled `ExtractComponents` is the sole multi-entry directory transaction:
it prepares only the source entry (remainder or retirement) and the reserved
child entries, then publishes them together by advancing
`directory_generation` after queue confirmation. Readers that captured the
prior generation observe the old source membership; readers that capture the
new generation observe the complete source-remainder-or-tombstone plus child
set. No global four-level radix, root/node/entry/authority-version pool, or
mandatory alternate epoch type is introduced, and placement streams do not use
this path.
...
A scheduled placement stream validates a compact GPU batch once, then
publishes each valid entry through this same per-volume gate. It does not
install an alternate directory root or advance `directory_generation`.
...
| Scheduled behavior handoff maps / bytes | 4 / 24 MiB | configured Moria-owned host/device/staging transport for mixed-processor handoffs and terminal consumer-egress staging/map; payload meaning remains consumer-owned |
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records; each entry publishes through ordinary per-volume placement/revision gates |
...
| Scheduled opaque egress receipts / records | 64 / 16,384 | configured terminal consumer outcomes; device/staging/map bytes charge existing handoff and generic staging pools |
...
component-extraction/placement/egress-receipt/opaque-resource/live-factory-buffer-byte/WGSL, and extension-registry
...
- component extraction without confirmed directory-generation publication
  ...
  ownership;
- placement-stream entries follow ordinary per-volume placement/revision
