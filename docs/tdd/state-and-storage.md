...
### Multi-entry directory publication

Ordinary matter and move commits still publish one volume at a time through the
existing per-volume revision/placement gates and
`directory_generation` capture already used by snapshots.

Scheduled placement streams and extract-components are the only closed
operations that prepare several directory entries and install them together.
They reuse that same directory-generation mechanism: build every new entry,
tombstone, placement, and content root in unreferenced storage, then advance
`directory_generation` once per proposal after validation so readers observe
either the complete old set or the complete new set. Multiple such proposals in
one tick publish in participant/proposal order. Failure may reject one proposal
while another valid proposal publishes under its policy, but device loss before
queue confirmation exposes none of the tick's candidate multi-entry installs
and never part of one proposal.

No separate public directory-root, radix-node, or authority-version resource
kind is introduced. Multi-entry publication charges ordinary live/lifetime
volume records, page/brick/scar pools, and the behavior placement /
component-extraction budgets declared below and in
...
| Scheduled placement updates / bytes | 65,536 / 4 MiB | configured compact GPU records; multi-entry directory install charges ordinary directory capacity |
| Scheduled component-extraction children / bytes | 256 / 32 MiB | configured and completely reserved before execution, including live/lifetime IDs, pages, scars, transfer and provenance |
| Scheduled opaque egress maps / receipts / records / bytes | 16 / 64 / 16,384 / 16 MiB | configured independent GPU-to-CPU transport covering device, staging, and host; zero/exact/overflow/failure are distinct |
...
- component extraction without confirmed multi-entry directory publication
  ...
  ownership;
- placement-stream publication follows the same multi-entry directory boundary;
