...
- one live directory entry, permanent lifetime record, `VolumeId`,
  consumer-visible `VolumeKey`, revision/placement gate, observation fact,
...
`VolumeId` uses the ordinary pre-reserved generational live slot—the same
live-directory reservation ordinary create consumes.
`VolumeKey` uses the ordinary lifetime-key reservation path: Moria mints a
fresh stable UUID into a reserved `volume_records` slot before GPU execution.
There is no separate derived-key namespace, candidate ordinal, UUIDv5
derivation, or restore-time key recomputation.
An exact key collision with an already accepted lifetime record retries minting
at most a small fixed bound (`volume_records + 1`); exhausting that bound fails
identity preflight before any adapter runs.
Unpublished keys are reservation candidates, not lifetime tombstones.
They are released without becoming observable if unused or if the proposal
fails.
Once published, the key consumes its lifetime record permanently and is
persisted by value with extraction provenance; restore reloads that stored key.
...
Every possible handle therefore has its final `VolumeId` and `VolumeKey`
before the adapter dispatch.
...
Atomic visibility for extraction reuses the baseline world-directory generation
already captured by every `SnapshotToken.directory_generation`, not a global
replacement of ordinary directory, revision, query, collision, checkpoint, or
presentation paths.
...
6. prepare only the affected directory entries—the updated or retired source
   entry and every published child entry—in unreferenced reservation slots,
   without rebuilding an alternate world-wide directory root; and
7. in a separate ordered publication dispatch, compare the source revision and
   the expected `directory_generation`, install the prepared source and child
   directory entries, advance the ordinary volume revision/placement gates for
   those entries, and publish the next `directory_generation` as the single
   extraction visibility token; then
8. after queue completion, emit observations and resolve the
   component-extraction receipt.

Construction never mutates directory entries visible to readers holding the
prior generation.
The publication pass is ordered after payload construction on the same queue
and uses the same confirmed-publication boundary as ordinary revision gates:
the coordinator withholds subsequent consumer readers that would capture the
new generation, observations, and the extraction receipt until queue
completion confirms the gate submission.
A gate executed in a device generation that is lost before confirmation is
never semantically committed; recovery retains the prior directory generation
and releases unpublished candidates.

Readers that already capture `directory_generation` in `SnapshotToken` observe
either the complete old source membership or the complete new
source-remainder-or-tombstone plus child set for that extraction.
They do not require a new epoch type, radix root pool, or mandatory alternate
reader path.
Ordinary single-volume matter and placement commits continue to use their
existing per-volume MVCC/revision gates and do not become root-affecting
directory-generation transactions.
Page-table rebuilds continue to use the baseline directory-generation swap
already specified for table replacement.
...
When several extract-components proposals are selected in one tick, each is its
own all-or-none directory-generation transaction ordered by participant
schedule order and proposal index; unrelated proposals retain independent
receipts and the approved independent-publication rule for failure policy.
Device loss before the tick submission's confirmation exposes none of that
submission's candidate extractions.
...
- Validation, assignment, conservation, or capacity sentinel failure leaves
  the prior directory generation current and releases unpublished
  IDs/resources after their last GPU use.
...
pub struct PlacementStreamApplied {
    pub command: CommandId,
    pub updates: Vec<(VolumeId, VolumeRevision, RigidPlacement)>,
    pub correlation: Correlation,
}
```

### Why this is not a directory-generation transaction

Multi-fidelity requires bounded, scalable placement freshness so coarse and
full objects keep truthful Moria poses without host body enumeration.
It does **not** require cross-volume placement atomicity.
Independent volumes retain the product's independent-publication rule (T4):
each stream entry publishes through the existing per-volume placement and
revision gate used by ordinary `VolumeCommand::Move` / scheduled move.
There is no alternate-root build, no multi-entry directory-generation swap, and
no root-chain/repair path for placement.

The stream still removes the costs that do not scale:

- no per-object host-authored `VolumeCommand::Move`;
- no per-object ordinary proposal header, queue admission, or host receipt
  object;
- one GPU-compacted batch and one validation pass for the whole stream; and
- one stream-level proposal receipt that reports the exact updated
  `(VolumeId, VolumeRevision, RigidPlacement)` vector after the independent
  gates complete.

Moria reserves the descriptor's complete maximum update records, per-entry
revision/placement outcome slots, observation facts, presentation markers,
outcomes, and GPU scratch before the adapter runs.
Publication walks validated entries in snapshot-index order and advances each
addressed volume's ordinary placement/revision gate by one.
A later independent failure of one volume under an invariant/device-loss path
follows ordinary per-volume recovery; the stream is not a multi-volume atomic
commit.
Whole-stream validation failure before any gate publishes leaves every
addressed volume unchanged.
...
Egress is a terminal consumer-facing channel.
It reuses the existing Moria-owned GPU-to-CPU adapter-handoff staging/map path
and the generic staging/readback/typed-receipt primitives rather than a
parallel non-aliasing map/receipt/record/device/staging/host pool family.
Tick admission charges:

- one tick-local device working range of
  `align4(80 + maximum_bytes)` from the existing behavior handoff/device
  transport accounting (or an equivalent factory-registered adapter buffer
  copied into that handoff path);
- one staging map slot and payload range from the configured generic
  staging / behavior-handoff map and byte pools;
- one host decoded-delivery charge against ordinary receipt host capacity; and
- one typed `Receipt` for the terminal consumer outcome.

Those charges are reserved before execution and released at the ordinary
handoff/staging/receipt milestones.
They must fit the existing pools; registration fails rather than inventing an
independent egress-only transport stack.
GPU-to-GPU consumers continue to use ordered handoffs or adapter-owned factory
buffers; Moria never routes a GPU handoff through this CPU channel.
...
`record_count * record_stride` initialized bytes onto the existing
GPU-to-CPU handoff/staging path used for mixed-processor handoffs and generic
readback.
...
The working device range is reusable only after its copy's last GPU use.
The staging range is reusable only after successful/failed map completion,
mapped-view drop, and unmap—the same milestones as existing handoff and
generic staging reuse.
...
`ResourceLimits` and `ResourceKind` add only the bounds required by the three
transports:
...
There are no independent directory-root, radix-node, entry-version, or
authority-version pools.
There are no independent egress-only device, staging, host, or map pools:
enabled egress capacity is charged against the existing
`behavior_handoff_maps` / `behavior_handoff_bytes` and generic
`staging_maps` / `staging_bytes` limits, and host delivery against ordinary
receipt host capacity, with registration failing when those pools cannot hold
every enabled participant maximum plus ordinary handoff and staging demand.

These charges do not alias proposal, observation, or adapter factory-resource
pools beyond the explicit handoff/staging reuse above.
...
extraction directory-generation publication latency.
...
DerivedExtractionProvenance {
    ...
    piece_handle,
    child_volume_key,         // the pre-reserved published key
    sample_count,
    sample_digest,
}
```
...
Checkpoint format stores the provenance tag, the child's durable `VolumeKey` by
value, and the complete sparse derived base for each child.
There is no world-level derived-key namespace or next-ordinal counter to
persist or recompute.
Later edits are ordinary full-brick scars relative to that stored derived
base.
Restore recreates child identity from the stored key, domain, cell size,
dynamic mode, placement, provenance, and sample bytes without a consumer
content source.
The parent need not still be live.
Cold rematerialization loads the persisted derived base first, then later
scars.
Digest/count mismatch, missing derived chunk, key mismatch against provenance,
or an attempt to attach a consumer source to the child fails restore.
...
1. one real-GPU adapter labels at least three connected pieces in one source,
   ...
   continuity across one extraction directory-generation publication;
2. every candidate identity/resource is reserved before adapter execution,
   binding 6 maps piece handles to final IDs/keys minted through ordinary
   live/lifetime reservation, and the adapter updates its factory-owned body
   table without CPU authority-path readback;
...
4. checkpoint/restore and cold rematerialization reproduce the stored child
   key, provenance, placement, samples, occupancy, and later edits without a
   consumer source and without key recomputation;
...
6. the placement stream publishes compacted full/coarse changes without host
   ...
   per-object ordinary move proposals, while each volume still advances through
   its independent placement/revision gate;
...
8. an adapter-owned egress struct unknown to Moria round-trips byte-exactly for
   zero records, one record, exact capacity, and multiple ticks using the
   existing handoff/staging/readback path; one-over capacity, malformed header,
   cancellation, shutdown, map/decode failure, and device loss produce distinct
   terminal results with no silent bytes or leaked/reused-early buffers.
