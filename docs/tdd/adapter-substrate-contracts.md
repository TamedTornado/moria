...
Enabled egress charges its device, staging, host, and map capacity against the
existing handoff pools (see Opaque GPU-to-CPU egress); it does not introduce a
parallel resource family.
The terminal egress edge's total range is `align4(80 + maximum_bytes)` and is
accounted in those pools even when that total exceeds the ordinary
adapter-to-adapter handoff edge maximum of 1 MiB.
...
Classification algorithms, fidelity class names, and exact proof-adapter
dispatch topology are adapter-owned; Moria enforces only the declared
dispatch/workgroup maxima already present on the engine descriptor.
...
`VolumeKey` is an ordinary pre-reserved durable UUID drawn from the same
identity pool used for consumer-registered volumes: Moria allocates a fresh
key that is free in the world's lifetime registry.
No world-global derived-key namespace and no monotonic candidate-ordinal
lifecycle are required.
Exact registry collisions (vanishingly rare for fresh UUIDs) retry allocation
within the reserved child budget; exhausting unique keys fails identity
preflight before any adapter runs.
...
exists.
If no source sample remains, the same source-and-children transaction
publishes its retirement tombstone at `source_revision + 1`.
Each child begins at revision 1.

### Preparation and atomic publication

Atomic visibility reuses the existing world-directory generation and snapshot
mechanism rather than replacing every ordinary volume commit with a public
epoch type or a four-level copy-on-write directory.
...
6. install prepared page versions and directory entries for the source
   remainder or tombstone and every published child into an unreferenced
   directory image under a candidate `directory_generation`; and
7. in a separate ordered publication dispatch, after payload construction on
   the same queue, compare the expected source revision and active directory
   generation, then publish the candidate directory image and the complete
   source-and-children revision vector as one transaction.

The construction dispatches never mutate a directory image visible to readers.
Readers pin `directory_generation` (already present on `SnapshotToken`) before
resolving volume entries, so they observe either the complete old source or the
complete new source/children set.
Per-volume revision gates remain the freshness identity for later ordinary
matter and placement work; this transaction is the sole multi-volume exception
required for fracture ownership.
The coordinator does not submit any consumer reader behind a candidate
directory image, append its observations, or resolve its receipt until queue
completion confirms that publication submission.
...
reconstructs the retained prior directory generation.
This is the same confirmed-publication boundary used by ordinary revision
gates, not a rollback of a visible commit.
Old directory images, source pages, and slots remain pinned until every prior
snapshot reader and the publication submission complete.
...
- Device loss before gate submission or after submission but before queue
  ...
  can have acquired the candidate directory image; old-generation resources
...
stream before any volume gate is written.
...
Publication does **not** require all unrelated poses to share a world
transaction or directory-generation swap.
After complete prevalidation of the compact batch, Moria publishes each
addressed volume through the existing per-volume placement/revision gate:
each volume independently advances its revision by one when its gate
succeeds.
...
A post-validation gate compare failure for one volume follows the ordinary
scheduled preparation-failure path for that volume under the selected
participant policy; prevalidation already excluded stale or conflicting
entries relative to the pinned frontier.
...
pipeline plus ordinary per-volume gates.
...
The substrate obligations stop at:

- opaque region bytes on binding 5;
- one body classified at most once against the adapter's deterministic region
  union (overlap deduplication is the adapter's responsibility and is proven
  by validation);
- continued adapter-owned coarse work for bodies outside every region;
- continuous ownership across region boundaries (no cross-adapter migration);
- publication of changed Moria placements through `PlacementStream`; and
- declared dispatch and workgroup maxima already present on the engine
  descriptor.

The multi-fidelity algorithm, fidelity class vocabulary, hierarchical-scan
shape, and exact proof dispatch topology live inside the proof adapter and its
validation oracle.
They are not Moria substrate types and do not expand the public ABI beyond
declared resource bounds.
The substrate already charges `maximum_gpu_dispatches` and
`maximum_gpu_workgroups`; the proof adapter must fit within the bounds it
declares, and P11 measures that selection.
...
An enabled GPU participant receives a terminal CPU-consumer egress edge on the
existing outgoing handoff transport (binding 3), not a seventh storage binding
and not a parallel device/staging/host/map accounting family.
Moria reserves one additional handoff edge slot whose successor is the owning
CPU consumer rather than another behavior engine.
Capacity, maps, copies, and lifetime follow the existing GPU-to-CPU handoff
path; the edge carries fixed-stride opaque records plus the semantics below.

Moria supplies a WGSL helper that atomically reserves one fixed-stride record
inside that edge's payload:
...
### Header and handoff layout

The terminal CPU-egress edge reuses the ordinary 32-byte handoff descriptor
and payload range, with an 80-byte record header at the payload base when
enabled:
...
72 correlation_low:u32
76 correlation_high:u32
```

Moria initializes counters, overflow, payload, padding, reserved words, and
correlation from the tick request.
Disabled requires zero stride/capacity/counters and no egress edge capacity.
...
The edge's handoff `written_bytes` equals the validated initialized prefix
length on success, or zero on overflow/failure.
...
GPU-to-CPU handoff staging path.
...
Working, staging, and host permits follow the existing handoff reuse rules:
the working range is reusable only after its copy's last GPU use; the staging
range only after map completion, mapped-view drop, and unmap; decoded host
bytes remain charged until the egress result is dropped or undeliverable
...
Group 0 has exactly seven storage bindings:
...
| 3 | read-write | outgoing handoffs, including the optional terminal CPU-egress edge |
...
Scheduled v2 requires at least seven storage bindings in one shader stage.
...
| `behavior_egress_records` | 16,384 | 1,048,576; aggregate descriptor record maxima fit |

Enabled CPU-egress byte and map capacity is reserved from
`behavior_handoff_bytes` and `behavior_handoff_maps` using the same three-fold
host/device/staging accounting as GPU-to-CPU handoff edges.
Registration fails if every declared adapter-to-adapter edge plus every
enabled egress edge cannot fit those existing pools.
These extraction and placement pools do not alias proposal, handoff, generic
staging, command, observation, or adapter factory-resource pools.
...
bytes, egress overflow count, map/decode failure count, unused
component-extraction reservations, matter-conservation failures, and placement
update count.
...
It carries only the minimal parent/piece identity needed to audit derivation;
it does not introduce a global ordinal lifecycle.
...
Those bricks are dirty authoritative persistence input and use the existing
scar/chunk machinery.
...
derived base for each child through ordinary scar chunks.
The child's ordinary stable `VolumeKey` is persisted like any other volume key.
...
1. one real-GPU adapter labels at least three connected pieces in one source,
   ...
   continuity across one source-and-children directory-generation
   transaction;
...
6. the placement stream publishes compacted full/coarse changes without host
   ...
   per-object ordinary move proposals, using complete prevalidation plus
   ordinary per-volume revision gates;
7. the proof adapter's declared dispatch/workgroup maxima are measured at
   empty, sparse, half, and full active lists and pass P11 on every claimed
   backend family; and
8. an adapter-owned egress struct unknown to Moria round-trips byte-exactly for
   zero records, one record, exact capacity, and multiple ticks over the
   handoff-backed terminal edge; one-over capacity, malformed header,
   cancellation, shutdown, map/decode failure, and device loss produce
   distinct terminal results with no silent bytes or leaked/reused-early
   buffers.
