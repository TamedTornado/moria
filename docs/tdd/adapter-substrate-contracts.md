...
`VolumeKey` is pre-reserved through the existing live/lifetime-record path
used by ordinary `Create`: one permanent lifetime record and one live slot
per possible child.
...
exists.
If no source sample remains, the same extraction commit publishes its
...
5. validate cell conservation, page/slot generations, occupancy summaries,
   directory entry candidates, and every counter/byte sentinel;
6. in a separate ordered publication dispatch, compare the source volume's
   ordinary revision gate, install prepared source pages/scars (or the
   source retirement tombstone), and install every selected child from its
   hidden pre-reserved directory entry into the live directory with its
   initial placement/revision gate; and
7. after queue completion, emit observations and resolve the
   component-extraction receipt.

Child candidates remain hidden until that publication pass: they hold no live
directory membership, are omitted from every snapshot/query/collision/
presentation capture, and cannot be addressed by any effect.
The construction dispatches never mutate a live directory entry or a root
visible to readers.
...
The coordinator does not submit any consumer reader behind the extraction
commit, append its observations, or resolve its receipt until queue completion
confirms that gate's submission. A gate executed in a device generation that
is lost before confirmation is therefore never semantically committed or
externally observable; recovery reconstructs the retained pre-extraction
state. This is the same confirmed-publication boundary used by ordinary
revision gates, not a rollback of a visible commit.
Old source pages and slots remain pinned until every pre-publication reader
and the publication submission complete.

No observable state contains duplicated, ownerless, or half-published matter:
before confirmation the source still owns every sample and children are
invisible; after confirmation the source remainder and children form one
...
This extraction-scoped commit reuses the baseline unreferenced prepared
versions, per-volume publication gates, pinned snapshots, and directory
membership install used by ordinary create/matter publication.
It does **not** introduce a world-wide directory epoch, four-level radix
directory, or migrate ordinary single-volume commits, queries, collision
traversals, presentation jobs, or checkpoint frontiers onto a new root-slot
authority path.
Cross-volume atomic visibility is required only for the pre-reserved source
plus its hidden children in this one proposal; independent volumes retain the
approved per-volume revision model (T4).
...
- Validation, assignment, conservation, or capacity sentinel failure leaves
  the live directory unchanged and releases unpublished IDs/resources after
  their last GPU use.
...
- Device loss before gate submission or after submission but before queue
  ...
  can have observed a candidate child; old-generation resources are
...
After one GPU validation of the compacted batch, the publication pass advances
each addressed volume's existing placement/revision gate in one ordered
pass—the same per-volume gate primitive used by ordinary `Move`.
...
Whole-stream cross-volume atomic visibility is **not** required and is not
provided: unrelated volumes may be observed at mixed revisions under T4.
Device loss before queue confirmation of the ordered gate pass exposes none of
the stream's candidate placement advances; after confirmation, published
gates remain.
...
pipeline and one ordered per-volume gate publication pass.
...
### Transport extension of T28 handoffs

Optional CPU egress is an extension of the existing scheduled opaque handoff
transport (T28), not a second independent subsystem.
A GPU adapter may declare a terminal consumer egress endpoint: fixed-stride
records written by the adapter, delivered asynchronously to the owning
consumer through the same ordered copy/map stages used for GPU-to-CPU handoff
edges.
GPU-to-GPU peer handoffs remain on the existing GPU path and never route
...
pub enum BehaviorEgressFailure {
    ...
    InvalidHeader,
    Transport, // map, decode, cancelled, shutdown, or device-loss terminal
}
...
the adapter-owned bytes mean. `BehaviorEgressFailure::Transport` covers cases
where dispatch/API, map, decode, cancellation, shutdown, or device loss makes
that participant's egress bytes undeliverable or untrustworthy.
...
`required_records`; an attempt beyond `u32::MAX` is reported as overflow with
the saturated required count and delivers no prefix.
...
the outer `device_generation` repeats the generation for device loss.
...
### Ordering, lifetime, and capacity reuse
...
range using the ordinary handoff GPU-to-CPU copy path.
...
Egress device, staging, map, and host-byte permits are reserved from the
existing `behavior_handoff_maps` and `behavior_handoff_bytes` pools together
with peer handoff edges for the same tick.
Declared egress maxima are charged at admission exactly as an additional
terminal GPU-to-CPU handoff of `align4(80 + maximum_bytes)` would be, plus one
map slot and one ordinary receipt permit per enabled egress participant.
There are no independent egress-only capacity pools.
Working, staging, and host bytes release only after their respective GPU-use,
map/view-drop/unmap, and result-drop milestones under the handoff lifecycle.
...
mapping impossible, the receipt receives the explicit transport failure before
the shutdown report completes.
Device loss before map completion yields transport failure, even when
...
`ResourceLimits` and `ResourceKind` add independent fields only where the
baseline pools cannot express the new work:
...
Enabled CPU egress charges its map, device, staging, and host-byte maxima
against the existing `behavior_handoff_maps` and `behavior_handoff_bytes`
pools; registration rejects when peer handoffs plus egress cannot fit those
pools.
Pending egress results use ordinary receipt permits.

These extraction and placement pools do not alias proposal, command,
observation, or adapter factory-resource pools.
Tick admission reserves their complete descriptor maxima before execution.
For every selected extraction it reserves one hidden child directory/lifetime
candidate per published child bound and the ordinary prepared page/brick/scar
versions used by baseline matter publication; for every placement stream it
reserves one placement/revision gate advance per maximum update.
...
Every published child carries a small extraction-provenance record:
...
Its base is the ordinary internal empty base already used when no external
source supplies brick content.
Publication installs one dirty full-brick scar (T8) for every nonempty child
brick; those scars are dirty authoritative persistence input and remain pinned
in retained GPU/scar capacity until a checkpoint is durable.
Before that checkpoint, device loss is `UnrecoverableDirtyState` under the
existing rule.

Checkpoint format stores the provenance tag on the volume record and the
child's scars under the ordinary scar-chunk path.
There is no separate complete derived-base chunk model and no
world-level derived-key namespace or candidate ordinal.
Child `VolumeKey` values are ordinary lifetime keys allocated before
publication and persisted exactly like any other volume UUID.
Restore recreates child identity, domain, cell size, dynamic mode, placement,
provenance, and sample bytes by loading empty base plus scars without a
consumer content source.
The parent need not still be live.
Cold rematerialization is the ordinary base-plus-scar path: empty base, then
scars.
Digest/count mismatch, missing scar chunk, or an attempt to attach a
consumer source to the child fails restore.
Registration of an extraction-derived key as an ordinary consumer-created
volume is rejected.
...
1. one real-GPU adapter labels at least three connected pieces in one source,
   ...
   continuity across the one extraction-scoped publication commit;
...
4. checkpoint/restore and cold rematerialization reproduce child identity,
   ...
   consumer source, using ordinary empty-base-plus-scar reconstruction;
...
6. the placement stream publishes compacted full/coarse changes without host
   ...
   per-object ordinary move proposals, using ordered per-volume placement
   gates rather than a world-directory root;
...
8. an adapter-owned egress struct unknown to Moria round-trips byte-exactly for
   ...
   leaked/reused-early buffers, while peer GPU handoffs remain on the GPU path.
