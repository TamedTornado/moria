...
This file specifies three generic substrate hooks required by human review of
Moria PR 395:

1. atomic extraction of existing authoritative matter into new child volumes;
2. a bounded bulk placement update path for GPU-resident persistent objects
   (so a multi-fidelity adapter can keep Moria placements current without
   per-object host moves); and
3. optional bounded opaque GPU-to-CPU adapter egress.

They do not add physics, damage, weapons, fracture policy, body state,
significance thresholds, activity-region schema, scoring, audio, or gameplay
events to Moria. An adapter may give the byte and proposal records those
meanings; Moria sees only bounded regions of existing samples, placements,
identities, opaque bytes, and publication outcomes.
...
`V1` type names inside v2. There is no runtime v1/v2 negotiation in the initial
implementation: configuration accepts only scheduled ABI version 2.
...
tick. Checked products and sums must fit the adapter maxima and the world
...
occupancy rule exactly. It is published as a dynamic volume.
...
supply a discontinuous initial transform through the component-extraction
proposal. It may publish later placement updates through the generic placement
stream.

The source remains live with one incremented revision when any remainder
exists. If no source sample remains, the same publication installs its
retirement tombstone at `source_revision + 1`. Each child begins at revision 1.

### Preparation and atomic publication

Moria prepares source remainder and child matter in unreferenced storage,
validates conservation and every sentinel, then publishes the complete set in
one ordered gate:

1. validate proposal headers, source snapshot, labels, assignments, and
   dispositions;
2. build new source and child bricks in unreferenced reserved slots and compute
   exact sample digests, domains, and placements;
3. validate cell conservation, page/slot generations, occupancy summaries, and
   directory entries for the source remainder-or-tombstone and every child; and
4. install those prepared directory entries under one `directory_generation`
   bump so no reader can observe a half-published extraction.

This is the only closed exception to independent per-volume publication: for
one extract-components proposal, the source update and all of its children
become visible together, or none of them do. It reuses the existing world
directory generation and per-volume revision/placement records. It does not
introduce a second global transaction clock, a multi-volume placement epoch, or
a redesigned directory topology for ordinary single-volume commits.

Readers that capture a directory generation before the gate see only the old
source. Readers that capture after confirmation see only the complete new set.
...
observable; recovery reconstructs the retained old directory generation. This
is the same confirmed-publication boundary used by ordinary revision gates, not
a rollback of a visible commit. Old pages and slots remain pinned until every
...
addressing its source volume. Two component-extraction proposals for one source
...
- Device loss before gate submission or after submission but before queue
  ...
  can have acquired the candidate entries; old-generation resources are
...
Moria reserves the descriptor's complete maximum update records, per-volume
revision/placement versions, observation facts, presentation markers, outcomes,
and GPU scratch before the adapter runs. After validation, publication reuses
the ordinary independent per-volume placement/revision gates—batched in one
ordered GPU pass for efficiency, not rewrapped as a multi-volume atomic epoch.
Each addressed volume independently advances its revision by one. The result
reports the exact updated `(VolumeId, VolumeRevision)` vector. No ordinary
per-volume `VolumeCommand::Move` object, host body enumeration, or
authority-path CPU readback is created.

```rust
pub struct PlacementStreamApplied {
    pub command: CommandId,
    pub updates: Vec<(VolumeId, VolumeRevision)>,
    pub correlation: Correlation,
}
```

A placement-stream entry overlaps another placement, retire, or
component-extraction effect for the same volume under the existing composition
rules. Unrelated volumes retain independent publication: validation failure can
omit one proposal without discarding another under the selected participant
policy. Device loss before confirmation of a submitted batch exposes none of
its candidate placement revisions.
...
headers, queue operations, and receipts. An adapter must not leave a moved
persistent Moria volume's placement stale: if its consumer-owned state changes
the pose, it emits that pose in the current stream or fails the participant/tick
...
its own schema and rejects invalid input through its ordinary adapter failure.

A conforming multi-fidelity GPU adapter uses one persistent adapter
registration and one consumer-owned body table for the whole world. Geographic
regions are not separate Moria adapters or separate Moria worlds. Each
persistent body has one stable adapter-owned record and, when matter-backed,
one `VolumeId`. Bodies outside every region continue through adapter-owned
coarse simulation; they do not freeze or disappear because Moria lacks a region
type. Overlapping regions form a deterministic union inside the adapter: no
body or matter-backed cell is processed twice because two regions overlap.
Crossing a region boundary must not change body/`VolumeId` ownership; any
promotion/demotion state lives in adapter-owned fields.

Moria's substrate obligations for this capability are exactly:

1. deliver the CPU-authored region bytes through the existing current-input
   path (binding 5);
2. accept changed poses through the placement stream without host body
   enumeration; and
3. refuse to interpret region schema, fidelity class, velocity, force, damage,
   or remote-destruction policy.
...
material effects, handoffs, or opaque CPU egress. Moria does not choose which
outcome applies.

### Fixed maximum dispatch

V2 retains fixed maximum dispatch rather than exposing indirect dispatch. The
proof adapter declares dispatch and workgroup maxima that fit the default
scheduled limits of 256 dispatches and 1,048,576 workgroups. Tick admission
charges those declared maxima. Every compacted pass guards
`global_invocation_id < count`. Inactive lanes still participate in any
workgroup barrier. P11 measures fixed-overdispatch cost at empty, sparse, half,
and full active lists for the multi-fidelity proof adapter and is blocking.
...
Exact classification kernels, halo widths, and active-list recipes belong to
the proof adapter and its validation oracle, not to Moria's public vocabulary.
...
participant that executed. The owning consumer receives the tick disposition and
decides what the adapter-owned bytes mean. `BehaviorEgressFailure::GpuValidation`
is used only when dispatch/API failure makes that participant's egress writes
...
`OperationScope::BehaviorEgress { tick, engine }`. Every transport failure maps
without loss to
...
dropped or undeliverable delivery is discarded. Dropping the public receipt does
not cancel submitted readback; Moria still maps or fails it and reclaims every
permit.
...
impossible, the receipt receives the explicit failure before the shutdown report
completes. Device loss before map completion yields `DeviceLost`, even when
publication was already confirmed; the tick outcome still truthfully reports
that publication. A map or decode failure yields its own terminal status and
quarantines the staging slot until safe unmap/destruction.
...
`total_bytes == align4(80 + maximum_bytes)`. After execution, `overflow != 0` or
`required_records > record_capacity` produces the explicit overflow failure and
no payload delivery.
...
complete descriptor maxima before execution. Component extraction also charges
existing live/lifetime directory, page, brick, scar, and observation pools for
every possible child. Placement stream charges ordinary per-volume
revision/placement preparation for every declared update. Telemetry reports
current, high-water, waiting/rejected, last-use-delayed bytes, overflow count,
map/decode failure count, unused component-extraction reservations,
matter-conservation failures, and placement update count.
...
consumer `BaseContentSource`. Its publication creates an internal derived-content
base consisting of every nonempty child brick plus the provenance record. Those
bricks are dirty authoritative persistence input. They remain pinned in retained
GPU/scar capacity until a checkpoint is durable. Before that checkpoint, device
loss is `UnrecoverableDirtyState` under the existing rule.
...
not still be live. Cold rematerialization loads the persisted derived base first,
then later scars. Digest/count mismatch, missing derived chunk, or an attempt to
attach a consumer source to the child fails restore.

Placement streams persist only the resulting committed per-volume placements and
revisions. Activity-region input, adapter body/fidelity state, coarse/full state,
...
1. one real-GPU adapter labels at least three connected pieces in one source,
   publishes at least two children, retains a source remainder, and proves exact
   coordinate/sample conservation and tolerance-bounded world-box continuity
   across the one extraction publication gate;
...
5. a proof adapter consumes CPU-authored disconnected and overlapping regions,
   ...
   across halo promotion/demotion, has no transform/velocity discontinuity, and
