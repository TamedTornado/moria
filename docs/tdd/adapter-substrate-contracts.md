...
- one complete alternate directory generation covering the source entry or
  tombstone and every child entry for every possible root-affecting extraction
  proposal;
...
`VolumeKey` is preallocated directly as a unique persisted UUID from the
world's ordinary stable-key allocator—the same uniqueness and lifetime rules
as consumer-supplied create keys, except the key is Moria-generated for the
child.
There is no derived-key namespace, UUIDv5 derivation, or persisted candidate
ordinal.
Exact registry collisions retry allocation at most `volume_records + 1` times
per child; exhausting that bound fails identity preflight before any adapter
runs.
...
4. build new source and child bricks in unreferenced reserved slots and mark
   every nonempty transferred brick as an ordinary dirty full-brick scar
   relative to the child's internal empty base;
5. validate cell conservation, page/slot generations, occupancy summaries,
   directory entries, and every counter/byte sentinel;
6. build a complete unreferenced directory generation containing the new
   source entry or tombstone and every child entry;
7. in a separate ordered publication dispatch, compare the active directory
   generation and source revision, then install the new immutable directory
   generation so readers observe either the complete old source or the
   complete new source/children set; and
...
This multi-entry directory transaction is limited to component extraction.
Ordinary single-volume commits and placement streams continue to use the
baseline per-volume revision/placement gates and directory-generation readers
described in [state-and-storage.md](state-and-storage.md).
Readers acquire one directory generation before resolving volume entries,
...
completion confirms that publication.
A generation executed in a device generation that is lost before confirmation
is therefore never semantically committed or externally observable; recovery
reconstructs the retained old directory.
...
- Device loss after confirmed publication preserves the applied extraction.
  Recovery then follows the ordinary dirty-scar rule and may fail with
...
### Independent-volume publication
...
stream before any volume gate runs.
...
revision/placement gate capacity, observation facts, presentation markers,
outcomes, and GPU scratch before the adapter runs.
After validation succeeds, each entry publishes through the existing
independent per-volume placement/revision gate used by ordinary
`VolumeCommand::Move`: one expected-revision compare and one advanced
revision per addressed volume.
There is no multi-entry directory transaction and no single alternate
directory root for placement streams.
...
and proposal index.
Extraction proposals retain the multi-entry directory-generation gate above.
Placement streams validate once, then publish their independent volume gates in
that order under the existing product independent-publication rule: validation
or gate failure can omit one proposal without discarding another under the
selected participant policy.
Device loss before queue confirmation of a tick submission exposes none of its
unconfirmed candidate publications.
Every proposal receipt reports its own outcomes after confirmation.

## CPU-authored activity regions
...
Moria does not define fidelity classes, classification priority, halo width,
or simulation kernels.
Those adapter-owned choices, including any full/transition/coarse lists and
fixed-dispatch measurement, live in the proof adapter and
[validation.md](validation.md).

Substrate obligations that remain here:

- region bytes arrive only through binding 5 as opaque current input;
- changed consumer-owned poses of matter-backed volumes publish through the
  placement stream above, not through per-object host move admission; and
- publication authority does not depend on CPU interpretation of region or
  egress bytes.

Validation must prove that a proof adapter can process disconnected and
overlapping CPU-authored regions once per body, keep continuous body/volume
identity across promotion and demotion, continue coarse motion outside every
full-fidelity region, and keep Moria placements current without host body
enumeration.
...
Egress reuses the baseline opaque handoff/readback and generic staging
infrastructure: device ranges, staging maps, host decode buffers, and last-use
reclamation follow the same permit and completion rules as other bounded
asynchronous readbacks. The egress pools are independently accounted so they
cannot silently borrow proposal, handoff, or command capacity.
...
| `behavior_component_extraction_bytes` | 32 MiB | 256 MiB; covers mapping, payload, transfer, directory-generation preparation, and validation records |
...
Extraction additionally charges ordinary live/lifetime directory records,
page/brick/scar capacity, and one prepared directory generation for the
source/children set; it does not introduce a second general directory-root
pool hierarchy for ordinary commits.
...
Its base is the existing internal empty base.
Publication records every nonempty child brick as an ordinary full-brick scar
relative to that empty base—the same scar mechanism used for edited external
volumes.
Those scars are dirty authoritative persistence input and remain pinned until
a checkpoint is durable.
Before that checkpoint, device loss is `UnrecoverableDirtyState` under the
existing rule.

There is no second derived-base-plus-scar hierarchy, no derived-key namespace,
and no persisted candidate ordinal.
Checkpoint format continues to store volume keys, placements, revisions, and
sparse full-brick scars; extraction children additionally store the minimal
provenance above and an empty-base source tag.
Restore recreates child identity, domain, cell size, dynamic mode, placement,
provenance, and sample bytes by loading the empty base and applying scars.
...
1. one real-GPU adapter labels at least three connected pieces in one source,
   ...
   continuity across the one multi-entry directory-generation gate;
...
4. checkpoint/restore and cold rematerialization reproduce child identity,
   ...
   plus full-brick scars without a consumer source;
5. a proof adapter consumes CPU-authored disconnected and overlapping regions,
   ...
   across fidelity transitions, has no transform/velocity discontinuity, and
   continues coarse motion/destruction-shaped generic effects outside all
   regions—those fidelity classes and dispatch counts are proof/validation
   obligations, not substrate vocabulary;
6. the placement stream validates once, then publishes compacted full/coarse
   changes through independent per-volume gates without host body enumeration,
   stale Moria placement, duplicate volume entry, or per-object ordinary move
   proposals;
7. fixed maximum dispatch for the selected proof capacity is measured at empty,
   sparse, half, and full active lists and passes P11 on every claimed backend
   family; and
