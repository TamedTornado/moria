...
2. bounded bulk placement updates for GPU-resident adapters that keep Moria
   placements fresh without host body enumeration; and
...
Multi-fidelity activity regions remain CPU-authored opaque input. How an
adapter classifies bodies, stores solver state, or schedules its private
kernels is adapter-owned. Moria enforces transport, reservation, publication,
and resource limits; validation proves the required overlap, continuity, and
coarse-motion outcomes for a proof adapter.
...
- one live directory entry, permanent lifetime record, `VolumeId`,
  consumer-visible `VolumeKey`, revision/placement gate slot, observation
  fact, presentation marker, and receipt child record for every possible
  child;
- one source-bound multi-volume publication group per possible extraction
  proposal (source remainder or retirement plus every candidate child), not a
  world-wide alternate directory root;
...
5. validate cell conservation, page/slot generations, occupancy summaries,
   directory/lifetime entries, and every counter/byte sentinel;
6. prepare ordinary per-volume revision/placement directory records for the
   source remainder or retirement and every `PublishChild` identity, still
   unreferenced by any live gate; and
7. in a separate ordered publication dispatch, compare the source volume's
   pinned revision and install **only** that source-bound volume set through
   the existing per-volume publication gates as one all-or-none transaction.

The construction dispatches never mutate a gate visible to readers.
The publication pass is ordered after payload construction on the same queue
and reuses the baseline prepared-transaction pattern: one transaction-wide
status flag, per-volume compare-and-publish of the prepared revision/placement
records for `{source, children}`, and no world-wide root switch.
Children become resolvable live volumes only when that same confirmed
publication installs their gates; provisional binding-6 identities remain
unresolvable until then.

Atomic visibility for external consumers is the confirmed-publication boundary
already used by ordinary revision gates: the coordinator does not submit any
consumer reader behind the candidate gates, append observations, or resolve
the extraction receipt until queue completion confirms the submission.
A generation lost before confirmation never commits; recovery keeps the
still-pinned pre-extraction source and releases unpublished candidates.
Old source pages and slots remain pinned until every prior-revision reader
and the publication submission complete.

No observable state contains duplicated, ownerless, or half-published matter
for the extraction set. Unrelated volumes continue to publish through their
own independent gates and are not forced through a shared directory epoch.
...
This source-bound multi-volume gate is deliberately narrower than a four-level
immutable world-directory root used by queries, collision, scheduled views,
checkpoints, presentation, and ordinary commits. Those paths keep the baseline
`directory_generation` snapshot pin and per-volume revision/placement gates.
Extraction only needs atomic publication of its parent and pre-reserved
children; world-wide root versioning and four additional directory resource
pools are not required for that contract.
...
- Validation, assignment, conservation, or capacity sentinel failure leaves
  the source at its prior revision and releases unpublished IDs/resources after
  their last GPU use.
...
- Device loss before gate submission or after submission but before queue
  ...
  can have observed candidate children; old-generation resources are
...
## Bounded placement stream

### Operation boundary

`PlacementStream` is one bounded scheduled GPU proposal whose payload is a
stable-compacted array of placement updates—the existing scheduled move effect
carried as a compact batch rather than as one host-admitted proposal object
per moved volume:
...
stream before any volume gate publishes.
...
per-volume directory/revision/placement slots for that many moves, observation
facts, presentation markers, outcomes, and GPU scratch before the adapter runs.
It does **not** reserve an alternate world-directory root, a cross-volume root
chain, or a repair path that rewrites a world-wide active-root index.

After validation, the publication pass installs each entry through the same
per-volume placement/revision gate used by ordinary scheduled move: each
...
No ordinary host `VolumeCommand::Move` object, host body enumeration, or
authority-path CPU readback is created.

Unrelated volumes retain independent publication. A validation failure rejects
the stream as a unit so a malformed compact batch cannot partially admit
entries the way a host loop might. Publication of a validated stream does not
require atomic visibility across those unrelated volumes: the authority needs
scalable non-stale placement updates, not a single cross-volume epoch for every
moved ship.
When several placement streams and/or component extractions are selected in one
tick, Moria still orders them by participant schedule order and proposal index;
each proposal keeps its own receipt; device loss before the tick submission is
confirmed exposes none of that submission's candidate gates.

The placement stream is still linear in the number of placements actually
published, which is unavoidable for fresh Moria placements, but removes
per-object host admission, proposal headers, queue operations, and receipts.
...
Geographic regions are not separate Moria adapters or separate Moria worlds and
must not require a cross-adapter migration protocol for a body that crosses
region boundaries.
Moria already supplies:

- one persistent GPU participant registration with opaque factory buffers;
- counted encoder limits for dispatches and workgroups;
- per-participant filtered views from one pinned frontier;
- compact `PlacementStream` publication through ordinary volume gates; and
- the optional opaque egress channel.

A multi-fidelity adapter must therefore keep its own body/solver state,
classify against the CPU-supplied regions without reading the body list back to
the CPU for that classification, avoid processing the same body or its exported
matter twice solely because regions overlap, keep coarse motion running outside
every full-fidelity region, and publish changed Moria placements through the
bounded placement stream (or ordinary scheduled effects) so placements do not
go stale.
Promotion/demotion continuity, velocity fields, damage, and debris policy remain
adapter semantics; Moria does not define them.
Moria does not prescribe a single body-table layout, class enum, scan algorithm,
body-capacity constant, or fixed private dispatch graph as substrate law.

### Qualification-harness proof adapter

The blocking multi-fidelity proof (C12/P11) uses **one** portable harness
implementation that is sufficient to falsify the substrate claims above. That
harness is not the required internal architecture of every conforming adapter.

The selected proof adapter:

1. uploads the CPU-supplied region bytes through binding 5;
2. owns one persistent body table for the whole world, with each matter-backed
   body holding one `VolumeId`;
3. classifies each body once against every declared region in stable input order
   into mutually exclusive harness classes `Full | Halo | Coarse` with priority
   `any Full > any Halo > Coarse`;
4. mark/scans and scatters each body index exactly once into compact lists;
5. runs full, transition, or coarse kernels against those lists;
6. keeps transform/velocity fields bit-identical across harness promotion and
   demotion copies; and
7. compacts changed Moria placements into one `PlacementStream`.

Disconnected and overlapping region fixtures exercise the same table.
Overlaps must process each body once; exported cells of a matter-backed overlap
body are visited once. Bodies outside every region continue through the coarse
pass and may emit placement streams, component extraction, ordinary material
effects, handoffs, or opaque CPU egress. Class names and the region schema
belong to the harness, not Moria's public vocabulary.

### Fixed-dispatch assessment

V2 retains fixed maximum dispatch at the substrate boundary rather than
exposing raw indirect dispatch buffers.
Each GPU participant declares dispatch/workgroup maxima; tick admission charges
those declared maxima.
Whether a given adapter's private kernels fit that bound after active-list
compaction is proven by measurement, not by baking one private pipeline into
the public contract.

For the selected proof capacity of 65,536 persistent bodies and workgroup width
128, the portable harness uses exactly 11 adapter dispatches and at most 3,604
workgroups per tick against declared maxima 16/8,192 (four class-compaction
passes, three fidelity kernels, four placement-compaction passes; each full
list pass uses 512 workgroups). Those descriptor maxima fit the default
scheduled limits of 256 dispatches and 1,048,576 workgroups with headroom.
P11 measures empty, 1%, 50%, and 100% active lists for **this** harness.
Failure of P11 blocks the fixed-dispatch selection for the multi-fidelity claim
and requires a later controlled indirect-dispatch revision; implementation may
not silently expose raw `INDIRECT` buffers.
Other adapters may use fewer or differently structured dispatches provided they
stay within their declared counted-encoder maxima and satisfy the substrate
obligations and validation oracles.
...
`ResourceLimits` and `ResourceKind` add independent fields for the new
transports. They do **not** add world-wide directory-root, radix-node,
entry-version, or authority-version pools; extraction and placement reuse the
baseline volume directory, revision/placement gates, and live/lifetime volume
records.
...
| `behavior_component_extraction_bytes` | 32 MiB | 256 MiB; covers mapping, payload, transfer, multi-volume publication group, and validation records |
...
For every selected extraction it reserves one source-bound multi-volume
publication group covering the source and every candidate child; for every
placement stream it reserves ordinary per-volume placement/revision slots for
the descriptor maximum entries. Shared volume-directory capacity is charged
against existing live/lifetime and directory-effect budgets, not a parallel
radix forest.
...
multi-volume publication latency for extractions.
...
1. one real-GPU adapter labels at least three connected pieces in one source,
   ...
   continuity across one source-bound multi-volume publication; pre-extraction
   readers see only the original source; post-confirmation readers see only
   the complete source remainder and all children;
...
5. a proof adapter consumes CPU-authored disconnected and overlapping regions,
   ...
   regions—without requiring Moria to own that adapter's private tables or
   class names;
6. the placement stream publishes compacted changes through ordinary
   per-volume placement gates without host body enumeration, stale Moria
   placement, duplicate volume entry, per-object host move commands, or a
   world-wide directory-root epoch;
7. fixed maximum dispatch is measured for the multi-fidelity proof harness at
   empty, sparse, half, and full active lists and passes P11 on every claimed
   backend family; and
