...
This file specifies three substrate hooks required by human review of Moria
PR 395:

1. atomic extraction of existing authoritative matter into new child volumes;
2. bulk placement publication that lets one persistent adapter keep Moria
   placements current while it runs multi-fidelity simulation over
   CPU-authored activity regions; and
3. optional bounded opaque GPU-to-CPU adapter egress.

They are transports and publication contracts only. Moria does not implement
physics, damage, weapons, fracture policy, body state, activity-region
semantics, fidelity classes, scoring, audio, or gameplay events. An adapter
may give the byte and proposal records those meanings; Moria sees only bounded
regions of existing samples, placements, identities, opaque bytes, and
publication outcomes.

Scheduled ABI v2 in this file supersedes the six-binding Scheduled ABI v1
...
opaque input contract (binding 5). A multi-fidelity adapter defines and
validates its own input schema within its declared input byte maximum.
...
- one live directory entry, permanent lifetime record, `VolumeId`,
  consumer-visible `VolumeKey`, revision/placement authority slot, observation
  fact, presentation marker, and receipt child record for every possible child;
- one alternate world-directory root for every possible root-affecting
  proposal, covering every changed directory entry;
...
7. in a separate ordered publication dispatch, compare the active root's
   `WorldDirectoryEpoch` and source revision, then install the new immutable
   root with one portable `atomic<u32>` compare-exchange on the active-root
   slot index; and
...
resolving volume entries, so they observe either the complete old source or
the complete new source/children set.
...
Ordinary single-volume matter and move commits keep the existing per-volume
revision/placement authority gate: they prepare one unreferenced authority
version and publish that volume's gate. They do not rebuild a multi-entry
directory root.

Component-extraction and placement-stream commits are the closed exceptions:
they prepare every changed directory entry (and, for extraction, transferred
matter) in unreferenced storage, then install one complete alternate directory
root under a single epoch gate. The epoch is an atomic multi-entry visibility
mechanism, not a cross-volume gameplay clock. It is stored inside the
immutable root; the atomic publication token is the single `u32` active-root
slot index, not a portable 64-bit atomic.

Per-volume revisions remain the freshness identity exposed to consumers.
Directory-epoch exhaustion closes the world to new multi-entry publication.

The directory root is a flat, capacity-bounded table of live and tombstone
entries plus a header that names the epoch and live count. Structural sharing
or denser internal encodings are implementation choices; they must preserve
the public epoch/all-or-none contracts and charge conservative entry and root
permits at admission. Public limits therefore name roots and directory
entries, not a particular tree fanout.
...
GPU-resident adapter's persistent objects under multi-fidelity simulation.
...
## Multi-fidelity simulation: substrate boundary only

Activity regions and fidelity classes are not Moria types.

Required substrate support is exactly:

1. **CPU-authored region input.** Region definitions arrive as the
   participant's ordinary opaque current-tick input (binding 5). The
   CPU/game layer chooses count, shapes, full-fidelity interiors, and any
   transition halo. Moria does not select regions and does not interpret the
   bytes.
2. **One persistent adapter registration.** Geographic regions are not
   separate Moria adapters or separate Moria worlds. Continuous ownership of a
   body that crosses regions is an adapter obligation over one body table and
   one `VolumeId` when matter-backed.
3. **Bulk placement authority.** Changed Moria poses publish through the
   placement stream above, not through a host loop of ordinary move commands.
4. **Fixed maximum dispatch retained.** Scheduled GPU adapters continue to
   declare dispatch/workgroup maxima against the existing
   `behavior_gpu_dispatches` / `behavior_gpu_workgroups` pools. After active-
   list compaction, the portable baseline keeps fixed maximum dispatch rather
   than exposing raw `INDIRECT` buffers. P11 proves that bound for the
   multi-fidelity proof adapter; failure blocks this selection and requires a
   later controlled-dispatch revision.

Everything else—classification, full versus coarse solvers, halo promotion and
demotion, transform/velocity continuity, overlap union, and remote
destruction/debris policy—is adapter-owned. A conforming proof adapter must
demonstrate those properties in validation (C12/P11), including:

- disconnected and overlapping CPU-authored regions;
- each body (and each matter-backed body's exported cells) processed once in
  an overlap;
- continuous identity across boundary crossing and promotion/demotion without
  transform/velocity discontinuity in the adapter oracle; and
- continued coarse motion outside every region with current Moria placements.

Moria does not name fidelity classes, does not prescribe adapter dispatch
graphs, and does not freeze objects merely because they leave every region.
...
| --- | ---: | --- |
| `directory_roots` | 64 desired / 4 minimum | 2,048; current, prepared, and pinned old multi-entry roots |
| `directory_entries` | 131,072 desired / 8,192 minimum | 1,048,576; live, tombstone, prepared, and reader-pinned entry versions for multi-entry roots |
...
| `behavior_egress_bytes` | 48 MiB desired / 3 MiB minimum | 768 MiB and adapter allocation; covers working device, staging, and decoded host ranges for every enabled adapter's `align4(80 + maximum_bytes)` |

These pools do not alias proposal, handoff, generic staging, command,
observation, or adapter factory-resource pools.
Ordinary single-volume authority versions continue to charge the existing
per-volume publication path; multi-entry proposals charge `directory_roots`
and `directory_entries` for every prepared root and changed entry.
...
5. a proof adapter consumes CPU-authored disconnected and overlapping regions
   through opaque input, processes each body once in the overlap, preserves
   one body/volume identity across boundary promotion/demotion, has no
   transform/velocity discontinuity in its oracle, and continues coarse
   motion/destruction-shaped generic effects outside all regions;
...
7. fixed maximum dispatch is measured at empty, sparse, half, and full active
   lists for that proof adapter and passes P11 on every claimed backend
   family; and
