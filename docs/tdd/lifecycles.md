...
owned request
  ...
  -> publishing ordinary revisions or one source-to-children transaction
  -> copying, mapping, and owning terminal opaque egress
...
The closed scheduled effect set is fill, patch, move, retire, placement batch,
and source-bound component extraction. Extraction alone can create
pre-reserved children from existing pinned matter and commits them with the
source under one compound transaction. Arbitrary create and source transport
remain outside the scheduled lifecycle.

Egress copy begins after the adapter dispatch without delaying publication
validation. Publication becomes final before asynchronous mapping, but the
tick receipt waits for every enabled egress result so it returns one coherent
participant outcome. Zero records is ready-empty; overflow, mapping failure,
shutdown, and device loss are explicit. Working, staging, and retained-host
bytes release only at their documented last-use, unmap, and result-drop
milestones.
...
| Scheduled egress overflow/map/loss after publication | Behavior egress | `BehaviorEgressOutcome::Failed` with exact reason | Publication and `revision_changed` stay truthful; no prefix is delivered |
