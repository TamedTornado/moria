...
owned request
  ...
  -> preparing source-bound multi-volume publication groups and placement
     batches when selected
  -> publishing ordinary per-volume gates (extraction installs its source-bound
     set as one all-or-none transaction; placement streams install each entry
     independently after whole-stream validation)
...
placement stream, and extract components. Extract components is the only
operation that publishes several volume identities together, and only for its
source-bound parent/child set through existing gates. Placement stream is a
compact batch of ordinary placement updates with whole-stream validation and
independent per-volume publication. Extract components can publish only
pre-reserved dynamic children made from samples already owned by one pinned
source; arbitrary create and `BaseContentSource` transport remain unavailable.
...
| Scheduled component-extraction validation/conservation/capacity failure | Component-extraction proposal | Typed proposal rejection or tick failure under participant policy | Source remains at its prior revision; no child identity or sample becomes visible |
