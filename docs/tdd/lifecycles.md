...
owned request
  ...
  -> preparing component-extraction and placement-batch work when selected
  -> publishing extraction transactions and ordinary per-volume gates
...
first planner runs. It also reserves every declared placement entry,
component-extraction child identity/transfer/directory/provenance/scar
resource, and opaque-egress record/byte maxima charged against the existing
handoff/staging/map/host pools. Unknown, duplicate, unexpected, missing
required, or over-capacity input rejects synchronously and invokes no
planner/adapter.
...
placement batch, and extract components. Extract components is the closed
multi-entry exception: it installs one source change and its pre-reserved
children as one extraction transaction. Placement-batch entries publish through
ordinary per-volume move gates without cross-volume atomicity. Extract
components can publish only pre-reserved dynamic children made from samples
already owned by one pinned source; arbitrary create and `BaseContentSource`
...
placement-batch lifetime, multi-fidelity adapter ownership boundary, and opaque
egress states are normative in
...
| Scheduled component-extraction validation/conservation/capacity failure | Component-extraction proposal | Typed proposal rejection or tick failure under participant policy | Pre-extraction source remains current; no child identity or sample becomes visible |
...
| Placement-batch invalid/stale/duplicate entry | Placement proposal | Whole batch rejected | No placement/revision in the batch changes |
