...
owned request
  ...
  -> preparing component-extraction/placement multi-entry installs when selected
  -> publishing per-volume gates or one multi-entry directory install per proposal
...
components are the only multi-volume exceptions and use one multi-entry
directory-generation install per proposal. Extract components can publish only
...
| Scheduled component-extraction device loss before/after publication | Behavior tick/world | Typed no-publication loss before confirmed multi-entry install; applied result plus normal dirty-state recovery after it | Never parent/child partial visibility or rollback |
