...
owned request
  ...
  -> preparing component-extraction source-and-children transactions
  -> publishing ordinary per-volume gates and extract-components directory
     transactions
...
component-extraction child identity/transfer/provenance resource, and
opaque-egress receipt/record capacity charged against handoff pools.
Unknown, duplicate, unexpected, missing required, or
...
placement stream, and extract components. Placement stream fully prevalidates
then publishes through existing per-volume revision gates. Extract components
is the sole multi-volume exception: it publishes only pre-reserved dynamic
children made from samples already owned by one pinned source as one
source-and-children directory-generation transaction. Arbitrary create and
`BaseContentSource` transport remain unavailable.
...
placement stream lifetime, multi-fidelity integration, and opaque egress states
are normative in
...
| Scheduled component-extraction validation/conservation/capacity failure | Component-extraction proposal | Typed proposal rejection or tick failure under participant policy | Prior directory generation remains current; no child identity or sample becomes visible |
| Scheduled placement-stream prevalidation failure | Placement-stream proposal | Whole stream rejected before any volume gate | No placement revision advances |
