...
owned request
  ...
  -> publishing per-volume gates or one multi-volume directory_generation advance
...
components are the only multi-volume exceptions and each advances
`directory_generation` once (`WorldDirectoryEpoch` on the receipt). Extract
...
| Scheduled component-extraction validation/conservation/capacity failure | Component-extraction proposal | Typed proposal rejection or tick failure under participant policy | Pre-proposal directory remains current; no child identity or sample becomes visible |
