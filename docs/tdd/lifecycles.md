...
owned request
  ...
  -> preparing component-extraction and placement-stream proposals when selected
  -> publishing ordinary per-volume gates, or one directory-generation gate for
     each selected extract-components proposal
...
multi-volume exception and uses one directory-generation gate per proposal.
Placement streams publish many independent per-volume placement revisions
after whole-stream validation. Extract components can publish only
