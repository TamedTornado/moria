...
owned request
  ...
  -> preparing component-extraction directory generation and placement gates when selected
  -> publishing independent per-volume gates or one multi-entry extraction directory generation
...
component-extraction child identity/transfer/scar/directory-generation/
provenance resource, and opaque-egress record/device/staging/host/map/receipt
resource. Unknown, duplicate,
...
multi-entry directory transaction and installs one directory generation for
the source and children. Placement stream validates a batch and publishes
through independent per-volume gates. Extract components can publish only
