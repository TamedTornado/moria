...
owned request
  ...
  -> publishing per-volume gates or one extraction-local commit-group
...
placement batch, and extract components. Placement batch publishes through
existing per-volume move/revision gates without cross-volume atomicity.
Extract components publish source and children under one extraction-local
commit-group gate and can publish only pre-reserved
...
| Scheduled component-extraction validation/conservation/capacity failure | Component-extraction proposal | Typed proposal rejection or tick failure under participant policy | Old source state remains current; no child identity or sample becomes visible |
