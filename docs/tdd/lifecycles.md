...
owned request
  ...
  -> preparing component-extraction/placement multi-volume directory when selected
  -> publishing per-volume gates or one multi-volume directory generation per root proposal
...
confirmed multi-volume directory generation per proposal. Extract components can publish only pre-reserved
