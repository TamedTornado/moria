...
owned request
  ...
  -> preparing component-extraction/placement candidates when selected
  -> publishing extraction commits and ordered per-volume placement gates
...
(including terminal CPU egress charges), transaction, completion, and
double-buffered feedback capacity before the first planner runs. It also
reserves every declared placement entry and component-extraction child
identity/transfer/directory/provenance resource. Unknown, duplicate,
...
components are the only multi-volume exceptions: extraction uses one
extraction-scoped confirmed commit of hidden pre-reserved children, while a
placement stream advances ordinary per-volume placement gates after one GPU
validation. Extract components can publish only pre-reserved dynamic children
made from samples already owned by one pinned source; arbitrary create and
...
| Scheduled component-extraction validation/conservation/capacity failure | Component-extraction proposal | Typed proposal rejection or tick failure under participant policy | Live directory unchanged; no child identity or sample becomes visible |
