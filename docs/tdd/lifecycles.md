...
owned request
  ...
  -> preparing component-extraction fracture deltas and placement streams when selected
  -> publishing fracture deltas and per-volume placement/revision gates
...
component-extraction child identity/transfer/fracture-delta/provenance
resource, and terminal-egress handoff/staging capacity. Unknown, duplicate,
...
placement stream, and extract components. Placement streams validate once then
publish per volume; extract components install a fracture-scoped directory
delta and advance `directory_generation`. Extract components can publish only
