...
owned request
  ...
  -> preparing component-extraction directory entries or placement gates when selected
  -> publishing per-volume gates or one directory-generation swap per extraction
...
multi-volume membership exception and uses one prepared
`directory_generation` swap per proposal. Placement stream bulk-validates on
GPU and publishes through ordinary per-volume placement gates. Extract
...
| Opaque egress empty/exact output | Egress receipt | Ready with exact prefix, including explicit empty | Publication outcome unchanged |
