...
owned request
  ...
  -> preparing component-extraction and placement-stream batches when selected
  -> publishing per-volume gates, plus one directory-generation transaction per
...
component-extraction child identity/transfer/directory/provenance resource,
and opaque-egress record/receipt capacity plus the handoff/staging charges
those receipts require. Unknown, duplicate, unexpected, missing required, or
...
placement stream, and extract components. A placement stream validates one
compact batch and publishes each entry through ordinary per-volume placement
gates. Extract components is the only multi-entry directory-generation
transaction and can publish only pre-reserved dynamic children made from
samples already owned by one pinned source; arbitrary create and
...
| Placement-stream invalid/stale/duplicate entry | Placement proposal | Whole stream rejected before any volume gate | No placement/revision in the stream changes |
