...
which publishes its own prepared multi-entry directory set under one advanced
`directory_generation`. Multiple such proposals publish in stable
participant/proposal order and keep independent per-proposal receipts; they do
not turn unrelated volumes into one tick-wide transaction.
...
multi-entry directory-publication rules in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
- every possible component-extraction child identity/live/lifetime directory
  record, assignment/transfer byte, provenance record, and ordinary directory
  capacity for each maximum extract-components proposal;
- every compact placement entry and ordinary directory capacity for each GPU
