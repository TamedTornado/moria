...
Source-bound extraction, compact placement, activity-region integration, and
opaque CPU egress are normative in
...
Scheduled v1 excludes arbitrary create because its consumer-owned Rust content
source is a control-plane registration. Its closed `ExtractComponents`
proposal may only transfer samples already owned by one pinned source into
pre-reserved dynamic children. `PlacementBatch` packs ordinary placement
updates without per-object host commands. No Rust `BaseContentSource` or
source descriptor crosses the scheduled effect sink or GPU ABI.
...
| 6 | read-only | pre-reserved component-piece to final-child identity table, or a valid empty header |
| 7 | read-write | optional bounded opaque CPU-egress header and payload, or a valid disabled header |
...
Kind `5` is a placement batch and kind `6` is source-bound component
extraction; their payloads and bounds are defined in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
mismatch.
Arbitrary create remains unrepresentable because a consumer-owned content
source cannot cross a scheduled effect record.
...
- every possible component child identity and existing page/brick/scar/
  directory/result capacity required by the declared extraction maximum;
- every declared placement record and its existing placement transaction
  capacity; and
- exact egress device, staging, host, map, record, and tick-result capacity.
...
vocabulary.
If its consumer wants persistent volumes from existing source matter, the GPU
adapter may use the closed extraction contract; Moria still does not call them
debris or select their significance. Arbitrary new content remains an ordinary
create command, independently admitted outside the scheduled tick.
...
7. bounded changing consumer input reaches the first CPU and first GPU
   ...
   readback; and
8. scheduled fracture/debris claims use only bounded source-owned extraction;
   arbitrary creation and source-object transport remain impossible;
9. CPU-authored regions do not duplicate overlap work, boundary crossings keep
   identity, and compact placements keep coarse-moving volumes current; and
10. opaque egress distinguishes ready-empty, exact delivery, overflow, map
    failure, and device loss without affecting publication authority.
