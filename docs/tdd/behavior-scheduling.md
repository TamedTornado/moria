...
A placement stream validates one compact batch then publishes each entry
through ordinary per-volume placement/revision gates. Component extraction is
the closed multi-entry exception: one source and its reserved children publish
under one extraction-scoped `directory_generation` transaction. Multiple such
extraction proposals keep independent per-proposal receipts in participant/
proposal order; they do not turn unrelated volumes into one tick-wide
transaction.
...
Binding 7 working bytes and its ordered copy/map charge the existing
behavior-handoff and generic staging pools; terminal outcomes charge
`behavior_egress_receipts` / `behavior_egress_records`.
...
extraction directory-generation rules in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
- every possible component-extraction child identity/live/lifetime directory
  record, assignment/transfer/child-brick byte, and provenance record for the
  maximum component-extraction proposal set;
- every compact placement entry and ordinary per-volume placement/revision
  outcome slot for each GPU participant's permitted placement stream; and
- every enabled egress record and receipt slot, plus the handoff/staging map
  and byte charges those deliveries require on the shared transport pools.
...
GPU validation and last use. Egress handoff/staging permits follow the shared
...
before dispatch and the source plus children publish under one
directory-generation transaction.
