...
which publishes its own prepared directory-entry set under one multi-volume
`directory_generation` advance (`WorldDirectoryEpoch` on the receipt). Multiple
such proposals form a stable participant/proposal-order chain and keep
independent per-proposal receipts; they do not turn unrelated volumes into one
...
multi-volume publication rules in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
- every possible component-extraction child identity/live/lifetime directory
  ...
  maximum extraction proposals;
- every compact placement entry for each GPU participant's permitted placement
  stream; multi-volume proposals advance the existing `directory_generation`
  rather than allocating alternate directory roots; and
