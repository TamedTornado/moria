...
The closed exceptions are a placement batch and component extraction.
Component extraction publishes its prepared source and hidden child entries
under one extraction-local commit-group gate. Placement batches validate one
compact GPU payload and publish through existing per-volume move/revision
gates without cross-volume atomicity. Multiple such proposals keep independent
...
extraction-local commit-group rules in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
- every possible component-extraction child identity/live/lifetime directory
  record,
  assignment/transfer/child-brick byte, provenance/authority version, and
  extraction-local commit-group descriptor for every maximum
  component-extraction proposal;
- every compact placement entry and ordinary per-volume move/revision authority
  version for each GPU participant's permitted placement batch; and
- every enabled egress record, host byte, completion, and receipt slot, plus
  shared staging/map capacity for egress readback.
...
GPU validation and last use without consuming lifetime records. Egress
device/staging/host permits follow the existing GPU-to-CPU handoff
...
placement batch, so Moria placements never depend on per-body host commands.
