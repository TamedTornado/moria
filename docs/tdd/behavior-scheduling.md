...
OwnedRequest
  ...
  -> PublishingEffectsOrDirectoryGeneration
  -> Reporting
  -> Complete
```
...
The closed membership exception is component extraction, which publishes its
prepared directory-entry set under one `directory_generation` swap. A
placement stream bulk-validates GPU updates and publishes through ordinary
per-volume placement gates. Multiple such proposals keep independent
...
| 7 | read-write | optional handoff-shaped opaque CPU-egress header and payload, or a valid disabled header |
...
Binding 7 is charged independently to the behavior-egress byte, map, and
receipt pools while reusing the handoff staging/map lifecycle.
...
- every possible component-extraction child identity/live/lifetime directory
  record,
  assignment/transfer/child-brick byte, provenance record, and prepared
  directory-entry slots for one directory-generation swap per maximum
  component-extraction proposal;
- every compact placement entry for each GPU participant's permitted placement
  stream, published through ordinary per-volume placement gates; and
- every enabled egress byte, map, completion, and receipt slot.
...
It uses a transition halo and adapter-owned body state rather than a separate
adapter or world per geographic region. Changed full/coarse poses are compacted
into one placement stream published through ordinary per-volume gates, so Moria
...
before dispatch and the complete source/child membership publishes through one
prepared directory-generation swap.
Arbitrary child content and `BaseContentSource` transport remain forbidden.

An enabled GPU adapter may also write a handoff-shaped opaque CPU-egress
prefix to binding 7. Moria asynchronously stages the initialized prefix and
returns a distinct egress receipt; it never interprets collisions, impacts,
destruction, scoring, audio, or another event meaning. Empty, exact capacity,
overflow, mapping failure, and device loss remain distinct.
