...
Component extraction is the only multi-volume exception: one proposal publishes
its source remainder-or-tombstone and all children under one
`directory_generation` gate. A placement stream validates many placement
updates together, then publishes each through ordinary independent per-volume
placement/revision gates. Unrelated proposals keep independent receipts; they
do not turn unrelated volumes into one tick-wide transaction.
...
- every possible component-extraction child identity/live/lifetime directory
  ...
  maximum component-extraction proposals;
- every compact placement entry and ordinary per-volume placement/revision
  preparation for each GPU participant's permitted placement stream; and
