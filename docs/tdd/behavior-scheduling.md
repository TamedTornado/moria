...
Extraction publishes one source-plus-hidden-children commit under the ordinary
confirmed-publication boundary; a placement stream validates once then advances
ordinary per-volume placement/revision gates in one ordered pass. Multiple such
proposals keep independent per-proposal receipts; they do not turn unrelated
volumes into one tick-wide transaction.
...
extraction-scoped commit rules in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
- every possible component-extraction child identity/live/lifetime directory
  record, assignment/transfer/child-brick byte, and provenance record per
  maximum component-extraction proposal;
- every compact placement entry and one ordinary placement/revision gate
  advance for each GPU participant's permitted placement stream; and
- every enabled egress range/map/receipt, charged against the handoff
  host/device/staging/map pools together with peer edges.
