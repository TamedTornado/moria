...
The closed multi-entry directory exception is component extraction, which
publishes its source remainder or tombstone and every child under one
directory-generation install. A placement stream validates its batch once,
then publishes each entry through the ordinary independent per-volume
placement/revision gates. Multiple such proposals keep independent
...
multi-entry directory-generation rules in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
- every possible component-extraction child identity/live/lifetime directory
  record, assignment/transfer/child-brick/scar byte, minimal provenance, and
  one prepared multi-entry directory generation per maximum component-
  extraction proposal;
- every compact placement entry and independent per-volume placement/revision
  gate capacity for each GPU participant's permitted placement stream; and
...
opaque current input, keeps one persistent body table, and continues simulation
outside every full-fidelity region. Region classification, fidelity classes,
and dispatch counts are proof-adapter obligations described in
[validation.md](validation.md), not substrate types. Changed poses are
