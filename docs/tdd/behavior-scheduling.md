...
OwnedRequest
  ...
  -> PublishingEffectsOrMultiVolumeDirectory
  -> Reporting
  -> Complete
```
...
which publishes its own prepared directory-entry set under one confirmed
world-directory generation swap. Multiple such proposals publish in stable
participant/proposal order and keep independent per-proposal receipts; they
...
multi-volume directory-generation rules in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
- every possible component-extraction child identity/live/lifetime directory
  record,
  assignment/transfer/child-brick byte, provenance record, and directory
  entry capacity for every maximum component-extraction child;
- every compact placement entry and directory/revision capacity for each GPU
  participant's permitted placement stream; and
