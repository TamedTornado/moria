...
Binding 7 is charged independently to the behavior-egress byte, map, receipt,
and record pools.
...
- every possible component-extraction child identity/live/lifetime directory
  record,
  assignment/transfer/child-brick byte, provenance record, and one
...
- every compact placement entry and one alternate directory entry/root for
  each GPU participant's permitted placement stream; and
- every enabled egress record, byte permit, map, completion, and receipt slot.
...
GPU validation and last use. Egress byte permits follow the
