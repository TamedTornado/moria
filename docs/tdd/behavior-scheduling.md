...
The closed exception is component extraction, which installs one source change
and its children as one extraction-local transaction so readers never observe
a partial result. Placement-batch entries still publish through ordinary
per-volume move gates and do not require cross-volume atomicity.
...
GPU adapters additionally receive placement-batch and extract-components
...
| 7 | read-write | optional opaque CPU-egress header and payload, or a valid disabled header |
...
Binding 7 charges the existing handoff, staging, map, and host-result pools.
...
| Byte | Field |
| ---: | --- |
| 0 | kind: unused `0`, fill `1`, patch-runs `2`, move `3`, retire `4`, placement-batch `5`, extract-components `6` |
...
Placement batch uses the v2 64-byte placement-update payload.
...
extraction-transaction rules in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
- every possible component-extraction child identity/live/lifetime directory
  record, assignment/transfer/child-brick byte, provenance record, and scar
  capacity needed by the maximum extraction;
- every compact placement entry and ordinary per-volume move-gate version for
  each GPU participant's permitted placement batch; and
- every enabled egress record/byte maximum charged against the existing
  handoff, staging, map, and host-result pools.
...
GPU validation and last use. Egress staging/host permits follow the same
copy/map/decode/delivery lifetime as GPU-to-CPU handoffs rather than proposal
release.
...
A placement-batch entry overlaps another
...
Union, halo, coarse/full classification, body-table organization, and dispatch
strategy remain adapter-owned. Changed full/coarse poses are compacted into one
placement batch whose entries publish through ordinary per-volume move gates,
so Moria placements never depend on per-body host commands.
...
before dispatch and the complete source/child result publishes as one
extraction transaction. Children persist as empty-base full-brick scars plus
provenance. Arbitrary child content and `BaseContentSource` transport remain
forbidden.

An enabled GPU adapter may also append opaque records to binding 7. Moria
asynchronously stages the initialized prefix through the existing handoff/
staging path and returns a distinct egress receipt; it never interprets
collisions, impacts, destruction, scoring, audio, or another event meaning.
Zero records, exact capacity, overflow, mapping failure, and device loss remain
distinct.
