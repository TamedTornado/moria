...
The closed exceptions are a placement stream and component extraction.
Placement streams validate as one payload then publish each entry through
ordinary per-volume placement gates. Component extraction installs a
fracture-scoped prepared directory delta and advances `directory_generation`
once so readers see the complete old source or complete new source/children
set. Multiple such proposals keep independent per-proposal receipts in
participant/proposal order; they do not turn unrelated volumes into one
tick-wide transaction.
...
| 3 | read-write | packed outgoing opaque-handoff table and zero-initialized payload capacities, including the optional terminal consumer-egress edge |
...
Terminal CPU egress reuses the handoff byte/map budgets.
Component-extraction transfer and fracture-delta scratch charge to
`behavior_component_extraction_bytes`.
...
Extract-components uses the v2 piece/assignment payload and proposal-local
piece handles; final child IDs arrive in prior feedback after publication.
...
- every possible component-extraction child identity/live/lifetime directory
  record, assignment/transfer/child-brick byte, provenance record, and one
  fracture directory delta per maximum component-extraction proposal;
- every compact placement entry for each GPU participant's permitted placement
  stream; and
- every enabled terminal-egress handoff/staging/host byte, map, completion, and
  receipt slot.
...
GPU validation and last use. Terminal-egress handoff/staging/host permits
follow the copy/map/decode/delivery lifetime rather than proposal release.
...
significance nor creates transient debris. Piece handles remain the only
in-dispatch child labels; final child IDs return through prior feedback after
confirmed publication of the fracture-scoped directory delta.
Arbitrary child content and `BaseContentSource` transport remain forbidden.

An enabled GPU adapter may also write an opaque initialized prefix on its
terminal consumer-egress handoff edge. Moria asynchronously stages that prefix
through the existing GPU-to-CPU handoff path and returns a distinct egress
receipt; it never interprets collisions, impacts, destruction, scoring,
audio, or another event meaning. Zero bytes, exact capacity, overflow,
