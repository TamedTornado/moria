...
The closed multi-volume substrate effects are a placement stream and component
extraction. Component extraction publishes only its source-bound parent/child
set through existing per-volume gates as one confirmed all-or-none transaction.
Placement stream validates a compact GPU batch of ordinary placement updates
and then publishes each entry through those same independent volume gates; it
does not install a world-wide alternate directory root. Multiple such
proposals keep independent per-proposal receipts ordered by participant
schedule; they do not turn unrelated volumes into one tick-wide transaction.
...
source-bound multi-volume publication rules in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
- every possible component-extraction child identity/live/lifetime directory
  ...
  source-bound multi-volume publication group per maximum component-extraction
  proposal;
- every compact placement entry and the ordinary per-volume placement/
  revision gate slots for each GPU participant's permitted placement stream;
  and
...
It is a qualification harness, not the required internal shape of every
multi-fidelity adapter: it may use a transition halo and one body table rather
than a separate adapter or world per geographic region. Changed poses are
compacted into one placement stream so Moria placements never depend on
per-body host commands.
...
before dispatch and the source-bound parent/child set publishes atomically
through existing volume gates. Arbitrary child content and
