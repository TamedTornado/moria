...
Placement-stream entries publish through those same per-volume
revision/placement gates after batch validation.
Component extraction is the closed multi-entry exception: one extraction-only
directory generation swap publishes the source residual-or-tombstone together
with every pre-reserved child from that proposal.
Multiple such proposals keep independent per-proposal receipts; they do not
turn unrelated volumes into one tick-wide transaction.
...
| ---: | --- | --- |
| 0 | read-only | this participant's packed stable-view header, volume records, cell records, then optional child-reservation section |
| 1 | read-write | one effect header, fixed proposal records, payload bytes, then optional opaque CPU-egress section |
...
The optional child-reservation section is part of the view allocation.
The optional egress section is part of the effect/output allocation for device
bytes; staging maps charge the ordinary staging pool, and decoded retained
host results charge `behavior_egress_host_bytes`.
...
Placement stream uses the v2 64-byte placement-update payload and publishes
through ordinary per-volume placement gates after batch validation.
Extract-components uses the v2 piece/assignment payload and pre-reserved
child identities from the binding-0 reservation section.
...
extraction-only directory generation-swap rules in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
...
- every possible component-extraction child identity against
  `behavior_component_extraction_children` and the live/lifetime volume-record
  headroom, with assignment cells, child bricks, payload bytes, and
  directory-entry effects charged to the existing proposal/payload/cell/brick/
  directory-effect maxima above;
- every compact placement-stream entry charged as proposal payload plus one
  directory-effect slot per maximum entry; and
- every enabled egress staging map through the ordinary staging/map pools,
  plus retained host-result bytes and receipt slots for decoded deliveries.
...
world per geographic region. Those region-union, halo, coarse-motion, and
fixed-dispatch choices are proof-adapter validation detail, not Moria
substrate architecture. Changed full/coarse poses are compacted into one
placement stream published through ordinary per-volume gates, so Moria
placements never depend on per-body host commands.
...
binding-0 reservation section before dispatch and the source residual plus
children publish through one extraction-only directory generation swap.
Arbitrary child content and `BaseContentSource` transport remain forbidden.

An enabled GPU adapter may also append fixed-stride opaque records to the
binding-1 egress section. Moria asynchronously stages the initialized prefix
through ordinary staging/map machinery and returns a distinct egress receipt;
it never interprets collisions, impacts, destruction, scoring, audio, or
another event meaning. Zero records, exact capacity, overflow, mapping
failure, and device loss remain distinct.
