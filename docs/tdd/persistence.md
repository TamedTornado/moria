...
whole-world checkpoint as any other live volume. It uses an internal empty
base, ordinary full-brick scars for every nonempty brick, and minimal
extraction provenance. It never acquires a consumer `BaseContentSource`.
...
transferred into an extraction child, or imported by restore. A background
compactor may ask the registered base source for that brick and remove the scar
only after byte-for-byte equality is proven at the same reconstruction
fingerprint. Extraction children have no external base source; their
unscarred bricks remain the internal empty base, so every nonempty brick
stays scarred until compacted only if a future equality proof against empty
applies.
...
- a source tag of either:
  - `ExternalBase`, followed by lineage bytes and reconstruction fingerprint; or
  - `ExtractionChild`, followed by minimal provenance
    `(parent_volume_key, parent_revision, extraction_command_id, piece_handle,
    sample_count, sample_digest)` and no external lineage/source descriptor.

Extraction children use the internal empty base. Every nonempty brick is an
ordinary full-brick scar reference in the same scar section used by edited
external volumes. There is no derived-key namespace, candidate ordinal, or
second sparse-derived-base chunk hierarchy.
`ImportAs` changes only the containing world key and preserves all volume
keys, including preallocated extraction-child keys.
...
4. Partition saved live volumes by source tag. Require the current live
   ...
   exactly and reject registration of a tombstoned or `ExtractionChild` key.
   ...
   saved extraction child from its empty base plus ordinary scar references
   and validates its provenance, domain, dynamic mode, sample count, and
   digest. A missing/extra external volume, missing source, attached source
   for an extraction child, or changed fingerprint fails.
...
2. otherwise, for `ExtractionChild`, resolve the internal empty base; for
   `ExternalBase`, request the exact registered base source;
...
Moria does not need to rerun the base for scarred bricks. For external volumes
the fingerprint remains mandatory because every unscarred brick relies on it.
Extraction children rely on scars for all nonempty content.
...
- atomic fracture save/restore with source remainder, multiple child
  identities, provenance, placements, full-brick scars over empty base, later
  child edits, and cold rematerialization without a consumer source;
- same-key and `ImportAs` restore preserve preallocated extraction-child keys
  and a later extraction creates a new noncolliding preallocated key;
- missing/corrupt scar chunk, wrong provenance/sample digest, and an
  attempted source registration for an extraction-child key fail before
  publication;
