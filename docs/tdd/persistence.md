...
provenance and ordinary full-brick scars against a minimal internal empty base.
It never acquires a consumer `BaseContentSource`.
...
- `DerivedExtraction`, followed by parent key/revision, extraction command ID,
  piece handle, and sample count/digest.

Derived children have no external lineage or source descriptor. Their matter is
the ordinary base-plus-scar path with an internal empty base and full-brick
scars for transferred samples. Restore rejects a key/provenance mismatch,
digest/count mismatch, or missing scar before directory publication.
...
4. Partition saved live volumes by source tag. Require the current live
   ...
   saved derived child from provenance plus empty-base and scar records and
   validates its domain, dynamic mode, sample count, and digest. A
...
2. otherwise, for `DerivedExtraction`, resolve the internal empty base; for
...
- atomic fracture save/restore with source remainder, multiple child
  identities, provenance, placements, empty-base full-brick scars, later child
