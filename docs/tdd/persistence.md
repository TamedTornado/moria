...
- `DerivedExtraction`, followed by parent key/revision, extraction command ID,
  piece handle, sample count/digest, and sorted complete
  sparse-derived-base brick references.

Derived children have no external lineage or source descriptor.
Child keys are ordinary lifetime-registry keys. Restore rejects duplicate or
retired keys under the same rules as external volumes. There is no separate
derived-key namespace or candidate-ordinal counter.
