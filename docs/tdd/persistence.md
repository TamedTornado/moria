...
- each volume's domain, cell size, mode, committed revision, and committed
  placement;
- for externally registered volumes: lineage and exact reconstruction
  fingerprint;
- for derived-extraction children: minimal parent/piece provenance and a
  complete sparse derived base persisted through ordinary scar chunks;
...
needs a coherent combined save waits for a `BehaviorTickReceipt`, checkpoints
the returned substrate revisions through Moria, and checkpoints each adapter's
own state through its own store/protocol. Moria does not claim atomic
durability across those stores.
...
Scheduled ABI v2 may publish derived-extraction children atomically with their
source remainder. Those children appear in a later whole-world checkpoint with
ordinary stable keys, minimal provenance, and complete derived-base scar
chunks. Arbitrary scheduled create remains unavailable; ordinary
`VolumeCommand::Create` after a tick is still independently admitted and not
retroactively part of the earlier tick publication.

Checkpoint scope remains `CheckpointScope::WholeWorld`. A successful manifest
contains every live volume at the captured frontier and every known retirement
...
- static/dynamic tag;
- content kind: external source or derived extraction;
- for external volumes: lineage bytes and reconstruction fingerprint;
- for derived-extraction children: minimal parent volume key, parent revision,
  extraction command id, piece handle, sample count, and sample digest, plus
  scar references that reconstruct the complete sparse derived base;
- frontier revision;
...
Derived children use ordinary durable `VolumeKey` values and must rematerialize
without a consumer `BaseContentSource`. Digest/count mismatch or an attempt to
attach an external source to a derived child fails restore.
