...
- `DerivedExtraction`, followed by parent key/revision, extraction command ID,
  piece handle, child volume key (must equal the volume record UUID), sample
  count/digest, and sorted complete sparse-derived-base brick references.

Derived children have no external lineage or source descriptor.
Their durable identity is the pre-reserved `VolumeKey` stored by value in the
volume record and provenance; restore reloads that key and never recomputes it
from a namespace or ordinal. Restore rejects duplicate keys or a
...
- restore reloads each child key by value from the checkpoint; a later
  extraction mints a fresh ordinary lifetime key rather than recomputing one;
- missing/corrupt derived-base chunk, wrong provenance/sample digest, key
  mismatch against provenance, and an attempted source registration for a
  derived key fail before publication;
