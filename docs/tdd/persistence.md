...
provenance, an internal empty base, and ordinary T8 full-brick scars for every
transferred brick. It never acquires a consumer `BaseContentSource`.
...
derived_key_namespace  16 bytes
material_count         u32
volume_count           u32
tombstone_count        u32
chunk_count            u32
...
- `DerivedExtraction`, followed by parent key/revision, extraction command ID,
  proposal slot, piece handle, sample count/digest, and an internal empty-base
  marker.
  Transferred content is represented only as ordinary scar references relative
  to that empty base—there is no parallel complete sparse derived-base section.

Derived children have no external lineage or consumer source descriptor.
Restore must re-derive each UUIDv5 key under `derived_key_namespace` from
parent key, parent revision, proposal slot, and piece handle and reject a
key/provenance mismatch before directory publication.
`ImportAs` changes the active containing world key but preserves this
namespace.
...
4. Partition saved live volumes by source tag. Require the current live
   ...
   saved derived child from its empty base plus ordinary scar chunks and
...
2. otherwise, for `DerivedExtraction`, resolve the closed internal empty base;
   for `ExternalBase`, request the exact registered base source;
...
- atomic fracture save/restore with source remainder, multiple child
  identities, provenance, placements, empty-base-plus-scar content, later child
  edits, and cold rematerialization without a consumer source;
- same-key and `ImportAs` restore preserve the derived-key namespace and a
  later extraction re-derives keys from source context without ordinal state;
- missing/corrupt scar chunk for a derived child, wrong provenance/sample
  digest, and an attempted source registration for a derived key fail before
  publication;
