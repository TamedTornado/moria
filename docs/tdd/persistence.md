...
whole-world checkpoint as any other live volume, with minimal substrate
provenance and complete full-brick scars against the canonical empty internal
base. It never acquires a consumer `BaseContentSource`.
...
- static/dynamic tag;
- source tag `ExternalBase` or `DerivedExtraction`;
- for `ExternalBase`: lineage bytes and reconstruction fingerprint;
- for `DerivedExtraction`: parent key, parent revision, proposal slot, piece
  handle, sample count, and sample digest (no consumer lineage/source);
- frontier revision;
- placement translation/quaternion IEEE-754 bits;
- sorted `(BrickCoord, ChunkDigest, record_index)` scar references.

Derived children store sample truth only as ordinary full-brick scars against
the canonical empty internal base. There is no second sparse derived-base
chunk class, no world `derived_key_namespace`, and no next-candidate ordinal.
Restore re-derives each child `VolumeKey` from parent key, parent revision,
proposal slot, and piece handle, and rejects a key/provenance mismatch.
...
4. Partition saved live volumes by source tag. Require the current live
   ...
   saved derived child directly from its provenance and ordinary scars,
   re-derives its `VolumeKey`, and validates domain, dynamic mode, sample
   count, and digest. A missing/extra external volume, missing source,
   attached source for a derived child, key/provenance mismatch, or changed
   fingerprint fails.
...
2. otherwise, for `DerivedExtraction`, resolve implicit empty against the
   canonical empty internal base; for `ExternalBase`, request the exact
   registered base source;
...
the fingerprint nevertheless remains mandatory because every unscarred brick
relies on it. Derived children have no consumer source; every nonempty brick
must be present as a scar.
...
- atomic fracture save/restore with source remainder, multiple child
  identities re-derived from parent key/revision/proposal slot/piece handle,
  provenance, placements, full-brick scars against the empty internal base,
  later child edits, and cold rematerialization without a consumer source;
- missing/corrupt scar chunk, wrong provenance/sample digest, key mismatch,
  and an attempted source registration for a derived key fail before
  publication;
