...
- `DerivedExtraction`, followed by parent key/revision, extraction command ID,
  piece handle, sample count/digest, and sorted complete sparse-derived-base
  brick references.

Derived children have no external lineage or source descriptor.
Each derived child uses an ordinary published `VolumeKey` under the existing
`volume_records` lifetime rule; there is no separate derived-key namespace or
candidate-ordinal allocator.
`ImportAs` changes the active containing world key under the ordinary restore
rule and does not rewrite subordinate volume keys.
Restore rejects duplicate keys, provenance/sample mismatch, or an attempt to
attach a consumer source to a derived child before directory publication.
...
4. Partition saved live volumes by source tag. Require the current live
   ...
   its provenance, domain, dynamic mode, sample count, and digest. A
...
- atomic fracture save/restore with source remainder, multiple child
  identities, ordinary published keys, provenance, placements, complete
  derived bases, later child edits, and cold rematerialization without a
  consumer source;
- same-key and `ImportAs` restore preserve published derived child keys as
  ordinary lifetime records, and a later extraction reserves noncolliding
  provisional keys before publication;
- missing/corrupt derived-base chunk, wrong provenance/sample digest, and an
