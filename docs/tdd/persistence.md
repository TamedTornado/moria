...
next_derived_candidate_ordinal u64
material_count         u32
volume_count           u32
tombstone_count        u32
chunk_count            u32
...
- `DerivedExtraction`, followed by parent key/revision, extraction command ID,
  candidate ordinal, piece handle, sample count/digest, and sorted complete
  sparse-derived-base brick references.

Derived children have no external lineage or source descriptor.
The manifest's next candidate ordinal must exceed every derived child ordinal
and must reproduce each UUIDv5 key under `derived_key_namespace`.
`ImportAs` changes the active containing world key but preserves this
namespace. Restore rejects ordinal exhaustion, duplicate ordinals/keys, or a
...
- same-key and `ImportAs` restore preserve the derived-key namespace/next
  ordinal and a later extraction creates the next noncolliding key;
