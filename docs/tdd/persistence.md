...
whole-world checkpoint as any other live volume, with a small
extraction-provenance record and ordinary full-brick scars over the internal
empty base. It never acquires a consumer `BaseContentSource`.
...
- `ExtractionChild`, followed by parent key/revision, extraction command ID,
  piece handle, sample count/digest, and the ordinary sorted scar references
  for every nonempty brick (empty base; no consumer lineage).

Extraction children have no external lineage or source descriptor.
Their `VolumeKey` values are ordinary lifetime UUIDs allocated before
publication and stored like any other volume UUID.
`ImportAs` changes the active containing world key but preserves subordinate
volume identities exactly as for external volumes.
Restore rejects a key/provenance/scar mismatch before directory publication.
...
4. Partition saved live volumes by source tag. Require the current live
   ...
   saved extraction child from empty base plus ordinary scar chunks and
   validates its provenance, domain, dynamic mode, sample count, and digest. A
   missing/extra external volume, missing source, attached source for an
   extraction child, or changed fingerprint fails.
...
- atomic fracture save/restore with source remainder, multiple child
  identities, provenance, placements, ordinary scars over empty base, later
  child edits, and cold rematerialization without a consumer source;
- same-key and `ImportAs` restore preserve ordinary extraction-child volume
  UUIDs and a later extraction allocates a fresh noncolliding lifetime key;
