...
Scheduled v1 source-bound extraction can publish derived children whose
complete initial nonempty bricks use the existing dirty-scar path. They carry
substrate provenance rather than a consumer `BaseContentSource`. Activity
input, adapter simulation state, and opaque egress bytes remain ephemeral.
...
- static/dynamic tag;
- source tag: `ExternalBase` with lineage/fingerprint, or
  `DerivedExtraction` with parent key/revision, extraction command, piece
  handle, sample count/digest, and complete initial sparse-brick references;
- frontier revision;
...
4. Require the current live volume registration key set to equal the
   manifest's `ExternalBase` key set and reject registration of a tombstoned
   or derived key. For each external volume, require equal finite domain/cell
   size/mode, lineage, and exact reconstruction fingerprint. Reconstruct each
   derived child from its stored provenance and complete sparse initial
   bricks. A missing/extra external volume, missing source, changed
   fingerprint, corrupt derived brick, or derived sample-digest mismatch
   fails.
...
2. otherwise request the exact registered external source, or resolve
   canonical empty for a `DerivedExtraction` child;
...
- extracted source remainder plus multiple children retain stable identity,
  provenance, placement, exact samples, later scars, and cold
  rematerialization without consumer sources;
- missing/corrupt derived initial bricks, changed provenance/sample digest, or
  consumer registration of a derived key fails before directory publication;
- v1 external-only and v2 derived-child golden fixture readability, plus
  explicit rejection of an unsupported v3 fixture.
