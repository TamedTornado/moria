...
| `behavior_handoff_maps` / `behavior_handoff_bytes: GpuCapacityLimit` | 4 / 24 MiB desired, 3 MiB minimum | 256 maps / 1 GiB and adapter allocation; startup reserves at most three times every declared peer-edge capacity plus every enabled CPU-egress `align4(80 + maximum_bytes)` range for host/device/staging representations, and every individual device binding fits the adapter binding limit; egress reuses these pools rather than independent egress capacity fields |
...
extraction children from empty base plus ordinary scars and provenance and
rejects a missing or corrupt scar/provenance before directory publication.
...
results, placement stream, extraction commit, opaque egress receipt ordering,
