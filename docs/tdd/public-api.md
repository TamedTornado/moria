...
| `behavior_egress_records` / `behavior_egress_bytes` | 16,384 / 48 MiB | 1,048,576 / 768 MiB; bytes charge each enabled adapter's device, staging, and retained-host capacity |
...
pub struct BehaviorEngineDescriptor {
    ...
    pub maximum_component_children: u32,      // zero for CPU
...
pub struct BehaviorCpuEgressDescriptor {
    pub schema: [u8; 16],
    pub record_stride: u32,                 // 0 or aligned 4..=65,536
...
pub struct BehaviorParticipantOutcome {
    ...
    pub egress: BehaviorEgressOutcome,
}
...
pub struct ComponentChildApplied {
    pub piece_handle: u32,
    pub volume: VolumeId,
    pub key: VolumeKey,
    pub revision: VolumeRevision, // one
...
pub struct ComponentExtractionApplied {
    pub command: CommandId,
    pub source: VolumeId,
    pub source_revision: Option<VolumeRevision>, // None when retired
...
Scheduled ABI v1 excludes arbitrary `VolumeCommand::Create` because its
consumer-owned Rust content source is a control-plane registration. The closed
GPU-only `ExtractComponents` proposal transfers existing source samples into
pre-reserved children, and `PlacementBatch` packs bounded ordinary moves.
Neither can transport a `BaseContentSource`, source handle, or source
descriptor. Their identity, publication, failure, and persistence contracts
are normative in
...
consumer-input binding, record/input counts, tick ID, and device
