...
pub struct ResourceLimits {
    ...
    pub behavior_cpu_egress_bytes: u64,
...
16 MiB aggregate opaque CPU egress, 1 MiB per terminal egress edge,
...
candidate children, assignments, child bricks, payload bytes, fracture-delta
capacity, and every associated live/lifetime directory, page, scar,
observation, receipt, and presentation record. Placement registration
checked-sums its 64-byte entries against placement-update/byte pools; each
entry publishes through ordinary per-volume placement gates after whole-stream
validation. Egress registration requires a multiple-of-four
`maximum_cpu_egress_bytes` that fits the participant's handoff and staging
budgets. Terminal egress reuses those handoff/staging pools rather than an
independent egress pool family.
...
children from empty base plus ordinary scars and provenance, and rejects a
missing or corrupt scar chunk before directory publication.
...
pub struct BehaviorEngineDescriptor {
    ...
    pub maximum_cpu_egress_bytes: u64,                     // zero disables
}
...
impl BehaviorGpuTickContext<'_> {
    ...
    pub fn terminal_egress_capacity(&self) -> u32;
...
The Scheduled ABI v2 proposal kinds, component-extraction public results,
placement stream, fracture directory-generation gate, terminal-egress handoff
reuse, resource relationships, and failure outcomes are normative in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
