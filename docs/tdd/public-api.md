...
| `behavior_egress_maps` / `behavior_egress_receipts` / `behavior_egress_bytes: GpuCapacityLimit` | 16 / 64 / 16 MiB desired, 1 MiB minimum | 256 / 4,096 / 256 MiB and adapter allocation; maps cover one active tick's enabled participants, receipts bound retained terminal/pending results, and byte pool covers device+staging+decoded host deliveries for aggregate descriptor maxima |
...
16 MiB opaque CPU-egress payload per adapter maximum,
...
record, including prepared directory-entry slots for one directory-generation
swap. Placement registration checked-sums its 64-byte entries against ordinary
per-volume gate publication capacity. Egress registration checked-sums
`align4(64 + maximum_cpu_egress_bytes)` for every enabled adapter into the
egress byte pool; map/receipt counts cover the enabled adapter count. None of
those pools aliases proposal, handoff edge maxima, generic staging, or
adapter-owned factory bytes.
...
pub struct BehaviorEngineDescriptor {
    ...
    pub maximum_cpu_egress_bytes: u64,      // 0 disables; else 4..=16 MiB, align4
}
...
results, placement stream, directory-generation swap, opaque egress receipt ordering,
