...
| `behavior_handoff_maps` / `behavior_handoff_bytes: GpuCapacityLimit` | 4 / 24 MiB desired, 3 MiB minimum | 256 maps / 1 GiB and adapter allocation; startup reserves at most three times every declared edge capacity for host/device/staging representations, plus every enabled opaque-egress working/map charge that reuses this pool; every individual device binding fits the adapter binding limit |
...
| `behavior_placement_updates` / `behavior_placement_bytes` | 65,536 / 4 MiB | 1,048,576 / 64 MiB; bytes `>= 64 * updates`; complete aggregate descriptor maxima fit; each entry publishes through ordinary per-volume placement/revision gates |
...
| `behavior_egress_receipts` / `behavior_egress_records` | 64 / 16,384 | 4,096 / 1,048,576; receipts bound retained terminal/pending results and aggregate descriptor record maxima fit; device/staging/map charges reuse `behavior_handoff_*` and generic `staging_*` |
...
record. Placement registration checked-sums its 64-byte entries and ordinary
per-volume placement/revision outcome slots. Egress registration requires an
exact `record_stride * maximum_records == maximum_bytes` product and charges
working/map bytes against the existing handoff and generic staging pools so
every enabled adapter maximum fits together with ordinary handoff demand.
Receipt and record counts cover pending and retained terminal egress results.
Those charges do not alias proposal or adapter-owned factory bytes.
...
the declared component-extraction identities/transfers, placement-stream
entries, and egress record/receipt maxima plus the shared handoff/staging
charges those deliveries require. An
...
results, placement stream, extraction directory-generation publication, opaque
egress receipt ordering, resource relationships, and failure outcomes are
normative in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
