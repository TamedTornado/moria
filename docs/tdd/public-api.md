...
pub struct ResourceLimits {
    ...
    pub directory_entries: GpuCapacityLimit,
...
| `directory_roots: GpuCapacityLimit` | 64 desired / 4 minimum | 2,048 and adapter allocation; includes the current multi-entry root, prepared proposal roots, and reader-pinned old roots |
| `directory_entries: GpuCapacityLimit` | 131,072 desired / 8,192 minimum | 1,048,576 and adapter allocation; live, tombstone, prepared, and reader-pinned entry versions used by multi-entry roots |
...
| `behavior_egress_bytes: GpuCapacityLimit` | 48 MiB desired / 3 MiB minimum | 768 MiB and adapter allocation; covers working device, staging, and decoded host ranges for every enabled adapter's `align4(80 + maximum_bytes)` |
...
root and one directory entry version per changed source/child/placement; the
checked aggregate must fit the effective directory pools before adapter
execution. Egress registration requires an
exact `record_stride * maximum_records == maximum_bytes` product. The egress
byte pool checked-sums working, staging, and decoded host ranges for every
enabled adapter's `align4(80 + maximum_bytes)`, and map/receipt counts cover
...
placement-stream entries/directory versions, and egress byte/map/receipt
maxima. An
...
results, placement stream, directory epoch, opaque egress receipt ordering,
