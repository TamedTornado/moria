...
consumer-supplied and persisted; source-bound extraction children receive a
Moria-preallocated, persisted `VolumeKey` defined in
...
associated live/lifetime directory, page, scar, observation, receipt,
presentation record, and one multi-entry directory generation per proposal.
Placement registration checked-sums its 64-byte entries and independent
per-volume placement/revision gate capacity for the declared update maximum.
Egress registration requires an exact
`record_stride * maximum_records == maximum_bytes` product. The device pool
checked-sums `align4(80 + maximum_bytes)` for every enabled adapter; staging
and host pools checked-sum payload maxima, and map/receipt counts cover the
...
the declared component-extraction identities/transfers/scars/directory-
generation preparation, placement-stream entries/per-volume gate capacity, and
egress device/staging/host/map/receipt maxima. An
...
`ExtractionChild` keys may not be registered. Moria reconstructs saved
extraction children from empty base plus ordinary scars and minimal provenance
and rejects a missing or corrupt scar/provenance before directory publication.
...
results, placement stream, multi-entry extraction directory generation, opaque egress receipt ordering,
