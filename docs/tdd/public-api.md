...
| `behavior_egress_receipts` / `behavior_egress_records` | 64 / 16,384 | 4,096 / 1,048,576; receipts bound retained terminal/pending results, and aggregate descriptor record maxima fit |
| `behavior_egress_host_bytes` | 16 MiB | 256 MiB; aggregate exact decoded deliveries fit; device/staging charge shared `staging_maps`/`staging_bytes` via the GPU-to-CPU handoff readback lifecycle |
...
live/lifetime directory, page, scar, observation, receipt, presentation, and
extraction-local commit-group record. Placement registration checked-sums its
64-byte entries and ordinary per-volume move/revision authority capacity; the
checked aggregate must fit effective live/lifetime and authority pools before
adapter execution. Egress registration requires an
exact `record_stride * maximum_records == maximum_bytes` product. Host and
receipt pools checked-sum payload maxima and retained results; device/staging
bytes charge shared staging capacity under the existing GPU-to-CPU handoff
readback lifecycle. Egress host/receipt pools do not alias proposal or
adapter-owned factory bytes; device/staging charge is shared staging, not a
...
the declared component-extraction identities/transfers/commit-group records,
placement-batch entries with ordinary per-volume move/revision versions, and
egress host/receipt maxima plus shared staging/map charge. An
