...
pub struct ResourceLimits {
    ...
    pub behavior_egress_bytes: GpuCapacityLimit,
...
| `behavior_component_extraction_children` / `behavior_component_extraction_bytes` | 256 / 32 MiB | 4,096 / 256 MiB; children also fit reserved live/lifetime directory records and all transfer/scar/page pools |
...
| `behavior_egress_bytes: GpuCapacityLimit` | 16 MiB desired / 1 MiB minimum | 256 MiB and adapter allocation; covers `align4(80 + maximum_bytes)` device ranges, staging payloads, and exact decoded host deliveries for every enabled adapter |
...
candidate children, payload bytes, and every associated live/lifetime
directory, page, scar, observation, receipt, and presentation record.
Placement registration checked-sums its 64-byte entries and ordinary directory
capacity for every updated volume. Multi-entry publication charges those
existing pools; it does not introduce separate public root/node/authority
resource kinds. Egress registration requires an exact
`record_stride * maximum_records == maximum_bytes` product. The egress byte
pool checked-sums `align4(80 + maximum_bytes)` device ranges plus staging and
decoded host deliveries for every enabled adapter; map/receipt counts cover the
enabled adapter count. None of those pools aliases proposal, handoff, generic
staging, or adapter-owned factory bytes.
...
entries, ordinary directory capacity for multi-entry installs, and egress
byte/map/receipt maxima. An
...
results, placement stream, multi-entry directory publication, opaque egress
receipt ordering, resource relationships, and failure outcomes are normative in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
