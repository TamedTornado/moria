...
pub struct BehaviorTickPermit {
    ...
    component-extraction child-identity, placement-stream, and egress
    retained-host maximum
    */
}
...
consumer-supplied and persisted; source-bound derived children receive a
Moria-derived, persisted `VolumeKey` from the parent key, pinned source
revision, proposal slot, and piece handle as defined in
...
| `behavior_view_volumes` / `behavior_view_bricks` / `behavior_view_cells` | 256 / 8,192 / 262,144 | fixed maxima 65,535 / 65,536 / 1,048,576; counts the sum of per-participant filtered exports, not a shared readable union |
...
| `behavior_egress_host_bytes` | 16 MiB | 256 MiB; aggregate exact decoded egress deliveries retained until result drop; device copy and staging maps charge existing staging/map pools |
...
4,096 component-extraction children,
placement-stream and extraction payloads within existing proposal/payload/
cell/brick/directory-effect maxima,
opaque egress records delivered through existing staging/map machinery,
...
Scheduled ABI v2 component-extraction registration checked-sums candidate
children against `behavior_component_extraction_children` and the live/
lifetime volume-record headroom, while assignments, child bricks, payload
bytes, and directory-entry effects must fit the existing proposal/payload/
cell/brick/directory-effect maxima and page/scar pools. Placement registration
checked-sums its 64-byte entries against proposal payload and one directory
effect per entry under ordinary per-volume gates. Egress registration requires
an exact `record_stride * maximum_records == maximum_bytes` product; device
copy and staging maps charge existing staging/map pools, while decoded retained
host results charge `behavior_egress_host_bytes`. None of those charges
aliases handoff or adapter-owned factory bytes.
...
pub struct BehaviorEngineDescriptor {
    ...
    pub maximum_placement_updates: u32,       // zero for CPU; charges proposal/payload/directory-effect maxima
    pub maximum_component_extraction_proposals: u32, // zero for CPU; charges maximum_proposals
    pub maximum_component_extraction_children: u32,  // zero for CPU; identity headroom only
...
pub struct DirectoryGeneration(NonZeroU64);
...
`BaseContentSource`. Child frames, extraction-only directory generation-swap
publication, conservation, failure, and persistence are normative in
...
The Scheduled ABI v2 binding-0 reservation section, binding-1 egress section,
component-extraction public results, placement stream, extraction-only
directory generation swap, opaque egress receipt ordering, resource
relationships, and failure outcomes are normative in
[adapter-substrate-contracts.md](adapter-substrate-contracts.md).
