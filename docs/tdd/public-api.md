...
Moria-generated ordinary durable `VolumeKey` pre-reserved before extraction
(see [adapter-substrate-contracts.md](adapter-substrate-contracts.md)). Numeric
...
| `behavior_handoff_maps` / `behavior_handoff_bytes: GpuCapacityLimit` | 4 / 24 MiB desired, 3 MiB minimum | 256 maps / 1 GiB and adapter allocation; startup reserves at most three times every declared adapter-to-adapter edge plus every enabled terminal CPU-egress edge capacity for host/device/staging representations, and every individual device binding fits the adapter binding limit |
...
| `behavior_egress_receipts` / `behavior_egress_records` | 64 / 16,384 | 4,096 / 1,048,576; receipts bound retained terminal/pending results, and aggregate descriptor record maxima fit; device/staging/host/map bytes charge handoff pools |
...
participant. Structural input rejection uses
...
Scheduled ABI v2 deliberately excludes ordinary `VolumeCommand::Create` because
its consumer-owned Rust content-source object is a control-plane registration,
not a GPU- or stable-view-derived value. Consumers submit ordinary create
through the unchanged ordinary command API.
Its one closed creation operation is `ExtractComponents`: a GPU adapter labels
samples already owned by one pinned source and Moria publishes pre-reserved
dynamic children under one source-and-children directory-generation
transaction. No `BaseContentSource`, source handle, or source descriptor is
transported in the scheduled ABI. Placement streams publish compacted poses
through ordinary per-volume revision gates after complete prevalidation.
Optional opaque CPU egress is a terminal edge on the existing handoff/readback
path. Normative layouts and failure tables are in
...
tables (including any terminal CPU-egress edge), read-only prior feedback, the
current participant's read-only opaque consumer-input binding, read-only
component-extraction reservation map when enabled, record/input counts, tick
ID, and device generation. It contains no raw command encoder, aggregate
cross-participant view, authoritative Moria buffer, or submission method. Dispatch calls
...
GPU ABI v2 supports fill, run patch, move, retire, placement stream, and
extract components against a supplied snapshot index.
