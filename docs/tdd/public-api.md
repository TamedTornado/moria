...
presentation, and source-bound multi-volume publication-group record.
Placement registration checked-sums its 64-byte entries against ordinary
per-volume placement/revision gate capacity for that many moves; it does not
reserve an alternate world-directory root. Egress registration requires an
...
results, placement stream, opaque egress receipt ordering, resource
