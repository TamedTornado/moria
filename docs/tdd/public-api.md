...
record. Placement registration checked-sums its 64-byte entries against
`behavior_placement_updates` and `behavior_placement_bytes`. Multi-volume
proposals reuse the existing volume directory and `directory_generation`; they
do not allocate separate radix root/node/authority pools. Egress registration
requires an exact `record_stride * maximum_records == maximum_bytes` product.
The device pool checked-sums `align4(80 + maximum_bytes)` for every enabled
adapter; staging and host pools checked-sum payload maxima, and map/receipt
counts cover the enabled adapter count. None of those pools aliases proposal,
handoff, generic staging, or adapter-owned factory bytes.
...
results, placement stream, multi-volume publication tokens, opaque egress
