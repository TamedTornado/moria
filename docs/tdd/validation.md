...
- extraction, presentation artifact/dirty/instance, dressing registry,
  behavior handoff maps/bytes including terminal CPU-egress charges, and
...
world-box continuity, and one extraction-scoped visibility commit. Copy
...
after the one extraction-scoped publication commit. Pre-publication
readers see only the original source with children still hidden; post-
publication readers see only the complete source remainder and all children.
This fixture selects zero explicit removals, so no coordinate/sample record is
...
Same-key and import restore preserve ordinary extraction-child volume UUIDs;
the next extraction allocates a fresh noncolliding lifetime key.
...
header/stride/count/reserved fields, cancellation before preparation, skipped
and not-run participants, and transport failure (map, decode, shutdown, or
device loss) each produce their closed result.
...
- P12 failure rejects atomic GPU component extraction or its extraction-scoped
  publication commit.
- P13 failure rejects the opaque CPU-egress handoff extension or its selected
