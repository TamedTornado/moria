...
- extraction and prepared directory-generation swap capacity, presentation
  artifact/dirty/instance, dressing registry, behavior egress maps/receipts/
  bytes, and extension registration/byte-pool defaults, boundaries,
...
- prepared component-extraction directory-entry and reservation-record
  host/WGSL offset, size, tag, finite placement/domain, and zero-reserved
  invariants;
...
- every Scheduled ABI v2 host/WGSL size, alignment, offset, and stride:
  ...
  64-byte egress header, 48-byte reservation record, 32-byte component-piece,
...
- host `[u8; 16]` key fields have the exact same 16 wire bytes as four
  WGSL `u32` words, and egress capacity/written_bytes reject misaligned
  or over-capacity prefixes;
...
- scheduled negative fixtures at the expected validation layer for bad
  ...
  assignments,
  egress written_bytes/status/overflow mismatch, and old-generation
  handoff/feedback/identity/egress reuse.
- scheduled group-0 reflection requires exactly bindings 0..=7 with the
  ...
  pre-reserved child-ID map; binding 7 is read-write only for the bounded
  handoff-shaped opaque egress terminal. A missing/extra/wrong-access binding fails;
...
Moria-conforming: they use only the restricted factory, fixed eight-binding
...
world-box continuity, and one directory-generation visibility gate. Copy
binding-6 piece-handle/final-ID mappings into
...
after the one prepared `directory_generation` swap. Old-generation
...
Published child keys are ordinary lifetime records; the next extraction
reserves noncolliding provisional keys and cannot reuse a saved child key.

Fault injection covers every validation phase, live/lifetime ID exhaustion,
prepared directory-entry capacity exhaustion,
page/brick/scar/byte exhaustion, renderer OOM, unused reservations,
cancellation, shutdown, device loss before and after the swap, and delayed
...
published via ordinary per-volume move gates, with no host body enumeration
and no ordinary host move proposal per object.
...
records are unique and the expected revision vector is exact.

The multi-fidelity proof adapter used for this case may employ body tables,
fidelity lists, and compaction schedules of its own design; those details are
validation evidence, not public substrate constants. Run empty, sparse, half,
and full active sets within the adapter's declared dispatch/workgroup maxima
and the world defaults. Overrun, duplicate classification, stale placement, a
separate adapter/world per region, or transform/velocity discontinuity fails.

### C13. Opaque GPU-to-CPU adapter egress

Register an adapter-owned opaque byte layout whose fields and meaning do not
appear in Moria. Round-trip its initialized prefix exactly for empty, one
payload, multiple ticks, and exact capacity. Results are delivered in tick
order with the request correlation. Empty is a successful zero-byte result,
not pending or unavailable.

One-over capacity must return exact overflow with no prefix. Malformed
header/status/written_bytes/reserved fields,
...
| P11 multi-fidelity placement | One GPU proof adapter owns persistent consumer state covering 16,384 distinct dynamic Moria volumes and receives 32 CPU-authored AABB regions with equal-width halos as opaque input. This gate sets `live_volumes=16,384`, `volume_records=32,768`, and `behavior_view_volumes=16,384`; other limits retain defaults. Fixtures use disconnected, 50%-overlapping, and fully overlapping arrangements. The proof may use body tables, fidelity classes, and compaction of its own design; admission charges only its declared dispatch/workgroup maxima under the fixed-dispatch baseline. Measure empty, sparse, half, and full active sets; every body still runs either transition or coarse work and all 16,384 matter-backed bodies change pose/publish placement per measured tick through one `PlacementStream` and ordinary per-volume gates. | p95 input-upload-to-placement-publication <=33 ms at 50% and <=50 ms at 100%; GPU time <=20 ms when timestamps exist; declared dispatch/workgroup charges are not exceeded; one-time overlap classification, exact continuous boundary oracle, continued coarse changes, exactly 16,384 unique placement entries/revision advances, zero host body enumeration, and every placement pool within its configured limit |
| P12 atomic component extraction | One 32,768-cell/512-brick source contains 64 disjoint connected pieces of 512 cells. Publish 63 dynamic children and retain one source piece. This gate sets `live_volumes=8,192`, `volume_records=8,192`, and both desired/minimum `detailed_bricks` and `dirty_scar_bricks` to 65,536; other limits retain defaults. Pre-reserve all default component-extraction capacity, use binding-6 IDs and provisional ordinary keys, transfer every sample, prepare 64 directory entries, and hold one old-generation reader through the directory-generation swap. Repeat 100 times with newly prepared sources; no presentation work is included. | p95 adapter-output-to-directory-swap <=50 ms and GPU transfer/validate/publish <=20 ms when timestamps exist; exact coordinate/sample conservation, world-box continuity within the specified f32 tolerance, 63 usable GPU child IDs, one old-or-new directory generation, zero authority-path readback, and all unused reservations reclaimed after last use. Live/dirty authoritative counts must rise by exactly the published result; allocator capacity and nonauthority overhead may not grow >1% after warm-up. |
| P13 opaque egress | One GPU adapter declares a 1 MiB opaque egress maximum and emits empty, 64-byte, 512 KiB, 1 MiB, then 1 MiB+64 required prefixes in a repeating sequence. Two staging slots alternate while publication effects remain valid. | p95 GPU-complete-to-decoded ready <=16 ms for <=512 KiB and <=33 ms at exact capacity; byte-exact initialized prefix with tick/correlation and increasing-tick delivery order; empty is ready-empty; exact capacity succeeds; one-over is explicit overflow with zero delivered bytes; egress byte/map high-water stays within config and returns to baseline after result drop |
...
- P12 failure rejects atomic GPU component extraction or its directory-generation
