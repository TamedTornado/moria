...
- extraction, multi-entry directory roots/entries, presentation
...
- multi-entry directory root/entry host/WGSL offset, size, tag, null-index,
  low/high-word, finite placement/domain, and zero-reserved invariants for the
  closed extraction/placement-stream publication path;
...
world-box continuity, and one directory-epoch visibility gate. Copy
...
directory root/entry exhaustion,
...
must process each body once in the overlap; assert a body in the overlap
...
body crosses full-fidelity, transition, and coarse adapter states without
changing body/volume identity or discontinuously changing the adapter-owned
...
records are unique and the expected directory epoch/revision vector is exact.

Run empty, 1%, 50%, and 100% active compact lists at the declared 65,536-body
proof capacity against the adapter's declared dispatch/workgroup maxima within
the default scheduled pools. Overrun, duplicate classification, stale
