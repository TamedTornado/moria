...
- scheduled group-0 reflection requires exactly bindings 0..=7 with the
  ...
  opaque egress target. A missing/extra/wrong-access binding fails;
...
world-box continuity, and one extraction-local atomic visibility boundary.
Copy binding-6 piece-handle/final-ID mappings into factory-owned body state
and publish without CPU authority-path readback. Arbitrary child content and
...
after the extraction-local publication. Pre-extraction readers see only the
original source; post-extraction readers see only the complete source remainder
and all children. This fixture selects zero explicit removals, so no
...
Restore rematerializes children from empty-base plus full-brick scars without a
consumer source, and published child keys consume ordinary lifetime records.
...
### C12. CPU-authored regions and multi-fidelity adapter hooks

The validation proof adapter declares required current input and owns a fixed
test-only region schema carried as opaque binding-5 bytes. Use at least two
disconnected regions, then overlap them. The adapter's own classification
writes one class per persistent body; assert a body in the overlap appears
exactly once. A per-cell diagnostic also proves every exported cell of a
matter-backed overlap body is visited exactly once. One projectile-shaped body
crosses full, halo, coarse, halo, and full classifications without changing
body/volume identity or discontinuously changing the adapter-owned
transform/velocity oracle. Union, halo, coarse/full classification, body-table
organization, and dispatch strategy remain outside Moria's public vocabulary.

Objects outside every region continue changing in the adapter's coarse pass.
Their Moria placements remain current through one compact placement batch whose
entries publish through ordinary per-volume move gates into the tick revision
vector, with no host body enumeration and no ordinary host move command per
object. Cross-volume placement atomicity is not required. Exercise a remote
generic remove/extract proposal as a debris/destruction-shaped outcome while
verifying Moria owns no such meaning. Changed placement records are unique and
the expected revision vector is exact. Stale placement, a separate
adapter/world per region, or transform/velocity discontinuity fails.

### C13. Opaque GPU-to-CPU adapter egress

Register an adapter-owned record layout whose fields and meaning do not appear
in Moria. Round-trip its initialized prefix exactly for zero records, one
record, multiple records/ticks, and exact capacity. Results are delivered in
tick order with the request correlation. Zero is a successful empty result, not
pending or unavailable.

One-over capacity must return exact overflow with no prefix; saturating-counter
overflow is a separate failure. Malformed header/count/reserved fields,
...
mapping/decoding. Working, staging, and host bytes are not reused before their
defined last-use/unmap/drop milestones on the existing handoff/staging path,
and dropping the receipt still reclaims them. Attempts to obtain a raw mapped
view/authority buffer or route a GPU handoff through egress fail.
...
| P11 atomic component extraction | One 32,768-cell/512-brick source contains 64 disjoint connected pieces of 512 cells. Publish 63 dynamic children and retain one source piece. This gate sets `live_volumes=8,192`, `volume_records=8,192`, and both desired/minimum `detailed_bricks` and `dirty_scar_bricks` to 65,536; other limits retain defaults. Pre-reserve all default component-extraction capacity, use binding-6 IDs, transfer every sample into empty-base full-brick scars, and hold one pre-extraction reader through publication. Repeat 100 times with newly prepared sources; no presentation work is included. | p95 adapter-output-to-publication-gate <=50 ms and GPU transfer/validate/publish <=20 ms when timestamps exist; exact coordinate/sample conservation, world-box continuity within the specified f32 tolerance, 63 usable GPU child IDs, no partial source/child visibility, zero authority-path readback, and all unused reservations reclaimed after last use. Live/dirty authoritative counts must rise by exactly the published result; allocator capacity and nonauthority overhead may not grow >1% after warm-up. |
| P12 opaque egress | One GPU adapter declares a 64-byte unknown record, 16,384-record/1 MiB maximum charged against existing handoff/staging pools, and emits 0, 1, 8,192, 16,384, then 16,385 required records in a repeating sequence. Two staging slots alternate while publication effects remain valid. | p95 GPU-complete-to-decoded ready <=16 ms for <=8,192 records and <=33 ms at exact capacity; byte-exact initialized prefix/tick/correlation and increasing-tick delivery order; zero is ready-empty; exact capacity succeeds; one-over is explicit overflow with zero delivered bytes; staging/host high-water stays within shared config and returns to baseline after result drop |
...
- P11 failure rejects atomic GPU component extraction or its extraction-local
  publication and empty-base scar rematerialization path.
- P12 failure rejects the opaque CPU-egress transport or its reuse of existing
  staging/delivery bounds; publication correctness does not convert that
  failure into a pass.
...
- architecture feasibility gates P1–P12;
...
- a P1–P12 pass missing its workload scale, in-flight pressure, percentile,
