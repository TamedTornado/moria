...
- extraction, presentation artifact/dirty/instance, dressing registry, behavior
  placement/component-extraction/egress maps/receipts/records/bytes, and
...
world-box continuity, and one multi-volume `directory_generation` visibility
gate. Copy binding-6 piece-handle/final-ID mappings into
...
after the one multi-volume `directory_generation` publication
(`WorldDirectoryEpoch` on the receipt). Old-generation readers see only the
original source; new readers see only the complete source remainder and all
children. This fixture selects zero explicit removals, so no coordinate/sample
...
disconnected regions, then overlap them. The adapter classifies each
persistent body at most once against the deterministic region union; assert a
body in the overlap appears exactly once. One projectile-shaped body crosses
region boundaries without changing body/volume identity or discontinuously
...
records are unique and the expected multi-volume epoch/revision vector is exact.

Run empty, 1%, 50%, and 100% active compact lists at the declared 65,536-body
capacity against declared maxima 16 dispatches / 8,192 workgroups. Overrun,
...
| P11 multi-fidelity placement | One GPU adapter owns 65,536 persistent records, of which 16,384 are distinct dynamic Moria volumes, and receives 32 CPU-authored AABB regions through opaque input. This gate sets `live_volumes=16,384`, `volume_records=32,768`, and `behavior_view_volumes=16,384`; other limits retain defaults. Fixtures use disconnected, 50%-overlapping, and fully overlapping arrangements. The proof adapter classifies bodies, runs coarse/full work, and compact-publishes placements under declared maxima 16 dispatches / 8,192 workgroups. Measure empty, 1%, 50%, and 100% active lists; every body still runs either transition or coarse work and all 16,384 matter-backed bodies change pose/publish placement per measured tick. | p95 input-upload-to-multi-volume-publication <=33 ms at 50% and <=50 ms at 100%; GPU time <=20 ms when timestamps exist; declared charges are 16/8,192; one-time overlap classification, exact continuous boundary oracle, continued coarse changes, exactly 16,384 unique placement entries/revision advances, zero host body enumeration, and every placement pool within its configured limit |
| P12 atomic component extraction | One 32,768-cell/512-brick source contains 64 disjoint connected pieces of 512 cells. Publish 63 dynamic children and retain one source piece. This gate sets `live_volumes=8,192`, `volume_records=8,192`, and both desired/minimum `detailed_bricks` and `dirty_scar_bricks` to 65,536; other limits retain defaults. Pre-reserve all default component-extraction capacity, use binding-6 IDs, transfer every sample, update 64 directory entries, and hold one old-generation reader through publication. Repeat 100 times with newly prepared sources; no presentation work is included. | p95 adapter-output-to-multi-volume-gate <=50 ms and GPU transfer/validate/publish <=20 ms when timestamps exist; exact coordinate/sample conservation, world-box continuity within the specified f32 tolerance, 63 usable GPU child IDs, one old-or-new directory generation, zero authority-path readback, and all unused reservations reclaimed after last use. Live/dirty authoritative counts must rise by exactly the published result; allocator capacity and nonauthority overhead may not grow >1% after warm-up. |
...
- P12 failure rejects atomic GPU component extraction or its multi-volume
