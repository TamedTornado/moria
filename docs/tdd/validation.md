...
- fracture-directory-delta host/WGSL offset, size, tag, null-index,
  low/high-word, finite placement/domain, and zero-reserved invariants for
  the prepared source and child entries;
...
- every Scheduled ABI v2 host/WGSL size, alignment, offset, and stride:
  the five retained 64-byte headers, 32-byte component-piece, 24-byte
  assignment, 64-byte placement update, 48-byte component-identity feedback,
  the 112-byte volume, 24-byte cell, 128-byte proposal, 20-byte patch run,
  32-byte handoff descriptor, 64-byte feedback participant, and 48-byte
  feedback proposal;
- host `[u8; 16]` key fields have the exact same 16 wire bytes as four
  WGSL `u32` words, and terminal-egress byte capacities reject nonmultiples of
  four;
...
- scheduled negative fixtures at the expected validation layer for bad
  ...
  malformed prior feedback, component-extraction duplicate/empty/unknown-handle
  assignments,
  terminal-egress capacity/overflow mismatch, and old-generation
...
- scheduled group-0 reflection requires exactly bindings 0..=5 with the
  ...
  ingress fails layout validation. Binding 3 may carry the terminal
  consumer-egress edge; binding 4 carries prior feedback including
  piece-handle identity outcomes. A missing/extra/wrong-access binding fails;
...
world-box continuity, and one fracture-scoped directory-generation visibility
gate. Piece handles remain in-dispatch labels; final IDs bind into
factory-owned body state from prior feedback without CPU authority-path
...
directory/lifetime record, fracture delta, page/brick/scar/transfer/provenance
record, proposal, receipt, and byte is reserved. Piece handles remain the only
in-dispatch child labels; after confirmed publication the adapter binds
factory-owned body state from prior-feedback identity records.
...
after the one fracture-scoped `directory_generation` publication. Old-generation
...
from empty base plus T8 scars. Same-key and import restore preserve the
derived-key namespace; the next extraction re-derives keys from source context
without ordinal state.

Fault injection covers every validation phase, live/lifetime ID exhaustion,
fracture-delta/page/brick/scar/byte exhaustion, renderer OOM, unused
reservations, cancellation, shutdown, device loss before and after the gate,
and delayed old-reader reclamation. Every unpublished identity and resource
returns to its pool after last use. Passing a source object or inventing a
sample fails. For a rejected proposal, next-tick GPU feedback makes the proof
adapter remove every provisional body association; none of the failed
candidate `VolumeId`s resolves in a later view or effect.
...
records are unique and the expected per-volume revision vector is exact;
publication uses ordinary placement gates without a cross-volume directory
root.
...
Register an adapter-owned opaque payload whose fields and meaning do not
appear in Moria. Round-trip its initialized bytes exactly for zero bytes, one
prefix, multiple prefixes/ticks, and exact capacity using the terminal
handoff/staging path. Results are delivered in tick order with the request
correlation. Zero is a successful empty result, not pending or unavailable.

One-over capacity must return exact overflow with no prefix. Malformed
handoff header/reserved fields, cancellation before preparation, skipped and
not-run participants, shutdown, map failure, decode failure, and device loss
each produce their closed result. Publication receipts remain independently
truthful before egress mapping/decoding. Handoff working ranges, staging maps,
and host bytes are not reused before their defined last-use/unmap/drop
milestones, and dropping the receipt still reclaims them. Attempts to obtain a
raw mapped view/authority buffer or route a GPU peer handoff through terminal
egress fail.
...
| P11 multi-fidelity placement | One GPU adapter owns 65,536 persistent records, of which 16,384 are distinct dynamic Moria volumes, and receives 32 CPU-authored AABB regions with equal-width halos. This gate sets `live_volumes=16,384`, `volume_records=32,768`, and `behavior_view_volumes=16,384`; other limits retain defaults. Fixtures use disconnected, 50%-overlapping, and fully overlapping arrangements. Classification compaction, three full/halo/coarse proof kernels, and changed-placement compaction use workgroup width 128, exactly 11 adapter dispatches and at most 3,604 workgroups against declared maxima 16/8,192; each maximum-list pass has 512 workgroups. Measure empty, 1%, 50%, and 100% full/halo active lists; every body still runs either transition or coarse work and all 16,384 matter-backed bodies change pose/publish placement per measured tick. | p95 input-upload-to-placement-publication <=33 ms at 50% and <=50 ms at 100%; GPU time <=20 ms when timestamps exist; adapter dispatches/workgroups are exactly 11/at most 3,604 and declared charges are 16/8,192; one-time overlap classification, exact continuous boundary oracle, continued coarse changes, exactly 16,384 unique placement entries/revision advances via per-volume gates, zero host body enumeration, and every placement pool within its configured limit |
| P12 atomic component extraction | One 32,768-cell/512-brick source contains 64 disjoint connected pieces of 512 cells. Publish 63 dynamic children and retain one source piece. This gate sets `live_volumes=8,192`, `volume_records=8,192`, and both desired/minimum `detailed_bricks` and `dirty_scar_bricks` to 65,536; other limits retain defaults. Pre-reserve all default component-extraction capacity, use piece handles during dispatch, transfer every sample, install one fracture directory delta, and hold one old-generation reader through publication. Final child IDs arrive in prior feedback. Repeat 100 times with newly prepared sources; no presentation work is included. | p95 adapter-output-to-directory-gate <=50 ms and GPU transfer/validate/publish <=20 ms when timestamps exist; exact coordinate/sample conservation, world-box continuity within the specified f32 tolerance, 63 usable GPU child IDs via prior feedback, one old-or-new directory generation, zero authority-path readback, and all unused reservations reclaimed after last use. Live/dirty authoritative counts must rise by exactly the published result; allocator capacity and nonauthority overhead may not grow >1% after warm-up. |
| P13 opaque egress | One GPU adapter declares a 1 MiB terminal-egress capacity and emits 0, 64, 512 KiB, 1 MiB, then 1 MiB+64 required bytes in a repeating sequence through the terminal handoff edge. Two staging slots alternate while publication effects remain valid. | p95 GPU-complete-to-decoded ready <=16 ms for <=512 KiB and <=33 ms at exact capacity; byte-exact initialized prefix/tick/correlation and increasing-tick delivery order; zero is ready-empty; exact capacity succeeds; one-over is explicit overflow with zero delivered bytes; handoff/staging/host high-water stays within config and returns to baseline after result drop |
...
- P12 failure rejects atomic GPU component extraction or its fracture-scoped
  directory-generation publication mechanism.
- P13 failure rejects the opaque CPU-egress transport or its selected
  handoff/staging delivery bounds; publication correctness does not convert
