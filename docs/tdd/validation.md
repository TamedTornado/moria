...
- extraction, component-extraction/placement/egress resource pools,
  presentation artifact/dirty/instance, dressing registry, and extension
  registration/byte-pool defaults, boundaries, pressure facts,
...
world-box continuity, and one source-bound multi-volume publication for the
parent and children only. Copy binding-6 piece-handle/final-ID mappings into
...
receipt, source-bound multi-volume publication group, and byte is reserved.
The shader copies the binding-6 piece-handle-to-final-`VolumeId` map into its
own factory-created body state.
...
after the one source-bound multi-volume publication. Readers that pin the
source before confirmation see only the original source; readers after
confirmation see only the complete source remainder and all children, with no
partial parent/child visibility. Unrelated volumes are not participants in that
gate. This fixture selects zero explicit removals, so no coordinate/sample
record is duplicated, absent, or temporarily ownerless.
...
Same-key and import restore preserve the derived-key namespace/next ordinal;
the next extraction cannot reuse a saved child key.
...
disconnected regions, then overlap them. The harness classifies each persistent
body once against the deterministic region union; assert a body in the overlap
appears exactly once. A per-cell diagnostic also proves every exported cell of
a matter-backed overlap body is visited exactly once. One projectile-shaped
body crosses full-fidelity, halo, and coarse regimes without changing
body/volume identity or discontinuously changing the adapter-owned
transform/velocity oracle.
...
published via ordinary per-volume placement gates, with no host body
enumeration and no ordinary host move command per object.
...
P11/C12 may use a harness-private body table, class lists, hierarchical scan,
65,536-body fixture, and exact 11-dispatch/3,604-workgroup pipeline against
declared maxima 16/8,192 at workgroup width 128. Those internals are
qualification evidence for fixed dispatch and compact placement, not substrate
requirements on every multi-fidelity adapter. Overrun of declared maxima,
duplicate classification, stale placement, a separate adapter/world per region,
or transform/velocity discontinuity fails.
...
| P11 multi-fidelity placement | One qualification harness GPU adapter owns 65,536 persistent records, of which 16,384 are distinct dynamic Moria volumes, and receives 32 CPU-authored AABB regions with equal-width halos. This gate sets `live_volumes=16,384`, `volume_records=32,768`, and `behavior_view_volumes=16,384`; other limits retain defaults. Fixtures use disconnected, 50%-overlapping, and fully overlapping arrangements. The harness's private classification/compaction and placement-compaction kernels use workgroup width 128, exactly 11 adapter dispatches and at most 3,604 workgroups against declared maxima 16/8,192; each maximum-list pass has 512 workgroups. Measure empty, 1%, 50%, and 100% full/halo active lists; every body still runs either transition or coarse work and all 16,384 matter-backed bodies change pose/publish placement per measured tick through ordinary per-volume gates. | p95 input-upload-to-placement-publication <=33 ms at 50% and <=50 ms at 100%; GPU time <=20 ms when timestamps exist; harness dispatches/workgroups are exactly 11/at most 3,604 and declared charges are 16/8,192; one-time overlap classification, exact continuous boundary oracle, continued coarse changes, exactly 16,384 unique placement entries/revision advances, zero host body enumeration, no world-directory-root epoch, and every placement pool within its configured limit |
| P12 atomic component extraction | One 32,768-cell/512-brick source contains 64 disjoint connected pieces of 512 cells. Publish 63 dynamic children and retain one source piece. This gate sets `live_volumes=8,192`, `volume_records=8,192`, and both desired/minimum `detailed_bricks` and `dirty_scar_bricks` to 65,536; other limits retain defaults. Pre-reserve all default component-extraction capacity, use binding-6 IDs, transfer every sample, prepare 64 volume directory identities, and hold one pre-publication source reader through confirmation. Repeat 100 times with newly prepared sources; no presentation work is included. | p95 adapter-output-to-volume-gate <=50 ms and GPU transfer/validate/publish <=20 ms when timestamps exist; exact coordinate/sample conservation, world-box continuity within the specified f32 tolerance, 63 usable GPU child IDs, one all-or-none source-bound publication (old source vs complete remainder+children), zero authority-path readback, and all unused reservations reclaimed after last use. Live/dirty authoritative counts must rise by exactly the published result; allocator capacity and nonauthority overhead may not grow >1% after warm-up. |
...
- P12 failure rejects atomic GPU component extraction or its source-bound
  multi-volume publication mechanism.
