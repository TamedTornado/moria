...
- extraction, presentation artifact/dirty/instance, dressing registry, behavior
  component-extraction children/bytes, placement updates/bytes, egress
  maps/receipts/records/bytes, and extension registration/byte-pool defaults,
  boundaries, pressure facts, effective-config reflection, and telemetry
...
world-box continuity, and one multi-entry directory-generation visibility gate. Copy
...
after the one multi-entry directory-generation publication. Readers that hold
the old generation see only the original source; readers that acquire the new
generation see only the complete source remainder and all children. This
fixture selects zero explicit removals, so no coordinate/sample record is
duplicated, absent, or temporarily ownerless. Query, collision, observation,
presentation invalidation, receipt, checkpoint, restore, and cold
rematerialization agree on child identity, placement, inherited cell
size/material samples, provenance, and revision. Same-key and import restore
preserve the derived-key namespace/next ordinal; the next extraction cannot
reuse a saved child key.
...
cancellation, shutdown, device loss before and after confirmed publication, and
delayed old-reader reclamation. Every unpublished identity and resource returns
to its pool after last use. Passing a source object or inventing a sample
fails. For a rejected proposal, next-tick GPU feedback makes the proof adapter
remove every provisional body association; none of the failed candidate
`VolumeId`s resolves in a later view or effect.
...
disconnected regions, then overlap them. Classification, fidelity classes, and
compaction are adapter-owned; assert a body in the overlap appears exactly once
and every exported cell of a matter-backed overlap body is visited exactly
once. One projectile-shaped body crosses full-fidelity, transition, and coarse
regions without changing body/volume identity or discontinuously changing the
adapter-owned transform/velocity oracle.
...
records are unique and the expected directory-generation/revision vector is
exact.

Run empty, 1%, 50%, and 100% active lists at the declared 65,536-body capacity
within the proof adapter's declared fixed dispatch/workgroup maxima. Overrun,
...
| P11 multi-fidelity placement | One GPU adapter owns 65,536 persistent records, of which 16,384 are distinct dynamic Moria volumes, and receives 32 CPU-authored AABB regions with equal-width transition margins. This gate sets `live_volumes=16,384`, `volume_records=32,768`, and `behavior_view_volumes=16,384`; other limits retain defaults. Fixtures use disconnected, 50%-overlapping, and fully overlapping arrangements. Classification, simulation, and placement compaction stay within the adapter's declared fixed dispatch/workgroup maxima (defaults 16/8,192). Measure empty, 1%, 50%, and 100% full-fidelity active lists; every body still runs either transition or coarse work and all 16,384 matter-backed bodies change pose/publish placement per measured tick. | p95 input-upload-to-directory-publication <=33 ms at 50% and <=50 ms at 100%; GPU time <=20 ms when timestamps exist; adapter dispatches/workgroups stay within declared maxima; one-time overlap classification, exact continuous boundary oracle, continued coarse changes, exactly 16,384 unique placement entries/revision advances, zero host body enumeration, and every placement/directory pool within its configured limit |
| P12 atomic component extraction | One 32,768-cell/512-brick source contains 64 disjoint connected pieces of 512 cells. Publish 63 dynamic children and retain one source piece. This gate sets `live_volumes=8,192`, `volume_records=8,192`, and both desired/minimum `detailed_bricks` and `dirty_scar_bricks` to 65,536; other limits retain defaults. Pre-reserve all default component-extraction capacity, use binding-6 IDs, transfer every sample, rebuild 64 directory entries, and hold one old-generation reader through publication. Repeat 100 times with newly prepared sources; no presentation work is included. | p95 adapter-output-to-directory-gate <=50 ms and GPU transfer/validate/publish <=20 ms when timestamps exist; exact coordinate/sample conservation, world-box continuity within the specified f32 tolerance, 63 usable GPU child IDs, one old-or-new directory generation, zero authority-path readback, and all unused reservations reclaimed after last use. Live/dirty authoritative counts must rise by exactly the published result; allocator capacity and nonauthority overhead may not grow >1% after warm-up. |
...
- P12 failure rejects atomic GPU component extraction or its multi-entry
