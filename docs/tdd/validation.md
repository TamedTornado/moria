...
- extraction, presentation artifact/dirty/instance, dressing registry, behavior
  placement/component-extraction/egress-receipt bounds, handoff and generic
  staging charges for egress, and extension registration/byte-pool defaults,
...
world-box continuity, and one extraction directory-generation visibility gate.
Copy binding-6 piece-handle/final-ID mappings into
...
after the one extraction `directory_generation` publication. Readers that
captured the prior generation see only the original source; readers that
capture the new generation see only the complete source remainder and all
children. Ordinary single-volume query/collision paths continue to use their
existing snapshot capture and are not rewired to a separate epoch type. This
fixture selects zero explicit removals, so no coordinate/sample record is
duplicated, absent, or temporarily ownerless.
Query, collision, observation, presentation invalidation, receipt,
checkpoint, restore, and cold rematerialization agree on the stored child key,
placement, inherited cell size/material samples, provenance, and revision.
Restore reloads each child key by value; the next extraction mints a fresh
ordinary lifetime key rather than recomputing one from a namespace or ordinal.
...
Each stream entry still publishes through the ordinary independent per-volume
placement/revision gate. Exercise a remote generic remove/extract proposal as
a debris/destruction-shaped outcome while verifying Moria owns no such meaning.
Changed placement records are unique and the expected per-volume revision
vector is exact.
...
mapping/decoding. Working bytes and shared handoff/staging map slots are not
reused before their defined last-use/unmap/drop milestones, and dropping the
receipt still reclaims them. Attempts to obtain a raw mapped view/authority
buffer or route a GPU handoff through egress fail.
...
- P13 failure rejects the opaque CPU-egress transport or its shared
