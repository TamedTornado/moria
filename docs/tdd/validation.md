...
- extraction, presentation artifact/dirty/instance, dressing registry, behavior
  placement/component-extraction/egress receipt and record pools, and
...
- every Scheduled ABI v2 host/WGSL size, alignment, offset, and stride:
  ...
  24-byte assignment, 64-byte placement update, the 112-byte volume, 24-byte
  cell, 128-byte proposal, 20-byte patch run, 32-byte handoff descriptor,
  64-byte feedback participant, and 48-byte feedback proposal;
...
- scheduled negative fixtures at the expected validation layer for bad
  ...
  assignments, egress stride/count/overflow mismatch, and old-generation
  handoff/feedback/identity/egress reuse.
- scheduled group-0 reflection requires exactly bindings 0..=6 with the
  ...
  ingress fails layout validation. Binding 6 is read-only and contains the
  pre-reserved child-ID map. Optional CPU egress reuses a terminal binding-3
  handoff edge rather than a seventh storage binding. A missing/extra/wrong-
  access binding fails;
...
Moria-conforming: they use only the restricted factory, fixed seven-binding
...
world-box continuity, and one directory-generation visibility transaction.
Copy binding-6 piece-handle/final-ID mappings into factory-owned body state and
publish without CPU authority-path readback. Arbitrary child content and
passing a Rust `BaseContentSource` through either scheduled sink/ABI must
remain impossible.
...
receipt, and byte is reserved. The shader copies the binding-6
piece-handle-to-final-`VolumeId` map into its own factory-created body state.
...
after the one source-and-children directory-generation publication. Prior-
generation readers see only the original source; new readers see only the
complete source remainder plus children. No sample is duplicated, lost, or
ownerless. Explicit `RemoveFromMatter` cells report exact count/digest and
create no Moria debris event. Cancellation, validation failure, capacity
exhaustion, renderer OOM, device loss before and after the gate, shutdown, and
unused child slots leak no capacity and never leave provisional IDs live.
Checkpoint/restore and cold rematerialization reproduce child identity,
minimal provenance, placement, samples, and later scars without a consumer
content source.

### C12. Persistent multi-fidelity regions and placement stream

One persistent GPU adapter owns a body table spanning the world and receives
CPU-authored activity regions only as opaque current input. Fixtures cover
disconnected regions, 50% overlap, full overlap, and a body crossing a halo
boundary. Assert each body is classified once, never processed twice in an
overlap, keeps one identity across promotion/demotion, and shows no
transform/velocity discontinuity under the proof oracle. Bodies outside every
region continue coarse motion and may emit generic effects. Changed poses
publish through one placement stream that fully prevalidates then uses ordinary
per-volume revision gates without host body enumeration or stale Moria
placements. The multi-fidelity algorithm and exact dispatch topology remain
inside the proof adapter; Moria charges only declared dispatch/workgroup
maxima. P11 is blocking.

### C13. Opaque GPU-to-CPU adapter egress

A GPU adapter declares a fixed-stride schema unknown to Moria and a terminal
CPU-consumer egress edge on the existing handoff transport. Round-trip zero
records, one record, exact capacity, and multi-tick ordered delivery with
byte-exact prefixes, schema, tick, and correlation. One-over capacity and the
overflow flag fail with no delivered prefix. Cancellation, shutdown, map/decode
failure, and device loss each produce their closed result.
Publication receipts remain independently truthful before egress
mapping/decoding. Device, working, staging, and host bytes follow handoff reuse
rules and are not reused early; dropping the receipt still reclaims them.
Attempts to obtain a raw mapped view/authority buffer or route a GPU handoff
through egress fail.
...
| P11 multi-fidelity placement | One GPU adapter owns 65,536 persistent records, of which 16,384 are distinct dynamic Moria volumes, and receives 32 CPU-authored AABB regions with equal-width halos. This gate sets `live_volumes=16,384`, `volume_records=32,768`, and `behavior_view_volumes=16,384`; other limits retain defaults. Fixtures use disconnected, 50%-overlapping, and fully overlapping arrangements. The proof adapter stays within its declared dispatch/workgroup maxima and charges those declared maxima at admission. Measure empty, 1%, 50%, and 100% full/halo active lists; every body still runs either transition or coarse work and all 16,384 matter-backed bodies change pose/publish placement per measured tick. | p95 input-upload-to-placement-publication <=33 ms at 50% and <=50 ms at 100%; GPU time <=20 ms when timestamps exist; declared dispatch/workgroup charges fit defaults; one-time overlap classification, exact continuous boundary oracle, continued coarse changes, exactly 16,384 unique placement entries/revision advances, zero host body enumeration, ordinary per-volume gates only, and every placement pool within its configured limit |
| P12 atomic component extraction | One 32,768-cell/512-brick source contains 64 disjoint connected pieces of 512 cells. Publish 63 dynamic children and retain one source piece. This gate sets `live_volumes=8,192`, `volume_records=8,192`, and both desired/minimum `detailed_bricks` and `dirty_scar_bricks` to 65,536; other limits retain defaults. Pre-reserve all default component-extraction capacity, use binding-6 IDs, transfer every sample, and hold one prior-directory-generation reader through publication. Repeat 100 times with newly prepared sources; no presentation work is included. | p95 adapter-output-to-directory-publication <=50 ms and GPU transfer/validate/publish <=20 ms when timestamps exist; exact coordinate/sample conservation, world-box continuity within the specified f32 tolerance, 63 usable GPU child IDs, one old-or-new directory generation, zero authority-path readback, and all unused reservations reclaimed after last use. Live/dirty authoritative counts must rise by exactly the published result; allocator capacity and nonauthority overhead may not grow >1% after warm-up. |
| P13 opaque egress | One GPU adapter declares a 64-byte unknown record, 16,384-record/1 MiB maximum terminal CPU-egress edge on the handoff path, and emits 0, 1, 8,192, 16,384, then 16,385 required records in a repeating sequence. Two handoff staging slots alternate while publication effects remain valid. | p95 GPU-complete-to-decoded ready <=16 ms for <=8,192 records and <=33 ms at exact capacity; byte-exact initialized prefix/schema/tick/correlation and increasing-tick delivery order; zero is ready-empty; exact capacity succeeds; one-over is explicit overflow with zero delivered bytes; handoff device/staging/host high-water stays within config and returns to baseline after result drop |
...
- P11 failure rejects the multi-fidelity proof adapter's declared dispatch
  bounds or compact placement-stream publication for persistent integration.
- P12 failure rejects atomic GPU component extraction or its
  source-and-children directory-generation publication mechanism.
- P13 failure rejects the handoff-backed opaque CPU-egress transport or its
  selected staging/delivery bounds; publication correctness does not convert
