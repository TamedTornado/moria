...
- behavior effect sink ownership: dense/run inputs are borrowed and copied
  ...
  `BaseContentSource` crosses the callback return, and only bounded
  source-owned `ExtractComponents` can create children;
...
- scheduled group-0 reflection requires exactly bindings 0..=7 with the
  ...
  ingress fails layout validation; binding 6 is the read-only final-child map
  and binding 7 is the bounded opaque egress target. The 64-byte reservation
  header, 48-byte reservation record, and 80-byte egress header have exact
  host/WGSL offset, alignment, zero-reserved, and four-word UUID/schema
  fixtures;
...
10. **Scheduled behavior:** run a CPU adapter and two renderer-owned GPU
    ...
    transport and prove both unrepresentable; exercise split only through the
    bounded source-owned extraction contract.
...
For the fracture/debris-shaped GPU variant, publish at least two pre-reserved
children from one source with a remainder. Prove exact matter conservation,
one compound-transaction visibility boundary, initial world-box continuity,
and binding-6 final IDs copied into adapter-owned GPU state without
authority-path readback. Arbitrary child content and passing a Rust
`BaseContentSource` through either scheduled sink/ABI remain impossible.
...
### C11. Adapter substrate capabilities

The real-GPU suite adds three proof adapters:

1. Component extraction labels at least three connected pieces, publishes at
   least two children with a source remainder, and copies final child IDs from
   binding 6 into adapter-owned state. Old readers observe the source before;
   new readers observe the exact remainder/children partition. An independent
   coordinate/sample oracle, placement-continuity oracle, checkpoint/restore,
   and cold rematerialization all agree. Duplicate assignments, every relevant
   exhausted pool, validation failure, cancellation, allocation failure,
   shutdown, and device loss before/after commit prove complete reclamation
   and truthful publication.
2. Multi-fidelity integration receives disconnected and overlapping regions
   only through CPU-authored binding-5 input. It classifies each persistent
   body once, keeps identity and adapter-owned transform/velocity continuous
   through full/halo/coarse transitions, continues coarse changes outside all
   regions, and publishes every changed matter-backed pose in one bounded
   placement batch. Empty, sparse, overlap, and full compact lists stay within
   declared dispatch/workgroup bounds on each claimed backend; failure blocks
   the fixed-dispatch selection.
3. Opaque egress uses an adapter-defined record absent from Moria types. Zero,
   one, multiple ticks, and exact capacity round-trip exact schema/bytes in
   tick results. One-over returns overflow with no prefix. Malformed header,
   participant-not-run, cancellation, map failure, shutdown, and device loss
   remain distinct, preserve independent publication truth, and never reuse a
   working/staging/host range before its last-use milestone.

The reviewer also attempts arbitrary create, source-object transport, raw
device/map access, gameplay-specific schemas, duplicate overlap processing,
stale coarse placements, and GPU-to-GPU routing through CPU egress. Any success
fails C11.
...
| P11 adapter substrate | One GPU adapter owns 4,096 persistent body records, including 1,024 dynamic Moria volumes, and receives eight CPU-authored regions. Empty, disjoint, 50%-overlap, and full-coverage ticks each classify all records once, run full/halo/coarse work within a declared 16-dispatch/8,192-workgroup maximum, and publish 1,024 changed placements. A separate tick extracts 32 equal 512-cell pieces from one source into 31 children and a remainder. Each tick emits an adapter-defined 64-byte egress record for every changed volume (64 KiB). | p95 input-to-publication <=50 ms and publication-to-terminal-egress <=50 ms; dispatch/workgroup use never exceeds the declared bound; overlap processing is exactly once; all 1,024 placements advance once without host enumeration; extraction conserves every coordinate/sample and exposes 31 final GPU child IDs at one compound gate; egress is byte-exact; all configured pools remain within limit and return to baseline after result drop |
...
- P11 failure rejects the fixed-dispatch, compact-placement, compound
  extraction, or opaque-egress mechanism identified by its failing receipt.
...
- contract scenarios C1–C11 where platform capabilities allow loss injection;
- shader validation;
- a fixed report-only workload;
- architecture feasibility gates P1–P11;
...
- a P1–P11 pass missing its workload scale, in-flight pressure, percentile,
