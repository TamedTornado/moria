...
after the one `directory_generation` publication. Old-generation
...
records are unique and the expected placement revision vector is exact.

Run empty, 1%, 50%, and 100% active compact lists at the declared proof
capacity. The proof adapter must stay within its declared dispatch/workgroup
maxima under fixed maximum dispatch; P11 measures that bound. Overrun,
duplicate classification, stale placement, a separate adapter/world per
region, or transform/velocity discontinuity fails.
