...
Startup telemetry separately records object-index build duration, retained index bytes, placement/record count, grid-entry count, maximum dependency bricks observed, and dependency-coordinate allocation bytes for the full checked-in manifest. These values populate `streaming.object_index`. Report validation rejects retained index bytes above 16 MiB, a footprint above 128 bricks, or any nonzero dependency-coordinate allocation. This phase remains inside—not in addition to—the process-to-control-ready `< 5,000 ms` acceptance window.
...
| Idle wilderness sparse | Streaming/startup telemetry + unit/property tests | No voxel arrays/meshes for untouched inactive uniform/procedural bricks; object index <= 16 MiB and no retained dependency-coordinate arrays |
