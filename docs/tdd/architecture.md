...
Bevy RenderApp / Moria render resources
        ...
        +-- component-extraction / placement-stream publication
...
buffers, component-transfer/validation scratch, placement-stream compaction
scratch, and egress staging maps.
...
mapping before adapter execution and publishes source plus children under one
directory-generation gate without CPU authority-path readback. Placement
streams validate compacted GPU records and publish each update through ordinary
independent per-volume placement/revision gates. Opaque egress maps
asynchronously after its ordered GPU copy; effect publication does not wait for
CPU decoding.
...
- component-extraction child reservations fit live/lifetime volume records,
  ...
  ordinary per-volume placement preparation; opaque egress
  record/device/staging/host/map maxima fit every enabled descriptor
  independently of handoff and generic staging;
