...
Bevy RenderApp / Moria render resources
        ...
        +-- extraction-local commit-group and bounded placement-batch publication
        +-- bounded opaque adapter-egress readback (shared staging)
...
buffers, component-transfer/validation scratch, extraction commit-group
descriptors, placement compaction, and shared staging maps used by egress
readback.
...
4. Root `RenderGraph`: execute camera-independent Moria compute in explicit
   ...
   -> behavior_publish_commit_group_and_placement_batches
...
mapping before adapter execution and publishes through an extraction-local
commit-group without CPU authority-path readback. Placement batches validate
one compact GPU payload and publish through existing per-volume move/revision
gates. Opaque egress reuses shared staging readback after its ordered GPU
copy; effect publication does not wait for CPU decoding.
...
- component-extraction child reservations fit live/lifetime volume records, page/brick/
  scar/directory pools, and binding-6 bytes; placement maxima fit ordinary
  per-volume move/revision authority versions; opaque egress record/host/receipt
  maxima fit every enabled descriptor, with device/staging charged to shared
  staging alongside handoff readback;
