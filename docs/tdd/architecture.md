...
buffers, component-transfer/validation scratch, alternate directory roots,
...
- multi-entry directory root and entry pools can hold the current multi-entry
  root, every declared root-affecting proposal's conservative entry charge, and
  configured reader-retained generations;
...
- component-extraction child reservations fit live/lifetime volume records, page/brick/
  ...
  directory-root storage; opaque egress record/byte/map maxima fit every
