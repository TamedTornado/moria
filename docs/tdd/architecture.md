...
Bevy RenderApp / Moria render resources
        ...
        +-- extraction-scoped commit and ordered placement-gate publication
        +-- bounded opaque adapter-egress staging (handoff transport extension)
...
buffers, component-transfer/validation scratch, placement compaction, and
egress staging maps charged through the handoff transport pools.
...
- live/lifetime volume and prepared page/brick/scar pools can hold every
  declared extraction child's hidden candidates and every placement-stream
  gate advance, plus configured reader-retained generations;
- every externally registered volume has a content source and exact
  reconstruction fingerprint; restored extraction children instead have
  validated provenance and ordinary scars over the internal empty base;
...
- component-extraction child reservations fit live/lifetime volume records, page/brick/
  scar/directory pools, and binding-6 bytes; placement maxima fit alternate
