...
Bevy RenderApp / Moria render resources
        ...
        +-- component-extraction transaction install + placement-batch gates
        +-- bounded opaque adapter-egress staging (existing readback path)
...
It also owns generic component-extraction reservations, placement-batch
...
It also owns Scheduled ABI v2 binding-6 identity maps, binding-7 opaque egress
buffers, component-transfer/validation scratch, extraction-transaction
install scratch, placement-batch compaction, and egress copies into the
ordinary staging/map pools.
...
4. Root `RenderGraph`: execute camera-independent Moria compute in explicit
   ...
   -> behavior_publish_extraction_and_placement_gates
...
mapping before adapter execution and publishes through one extraction-local
transaction without CPU authority-path readback. Placement batches validate
and publish compacted GPU records through ordinary per-volume move gates.
Opaque egress maps asynchronously after its ordered GPU copy on the existing
staging path; effect publication does not wait for CPU decoding.
...
- directory and volume-authority pools can hold the current directory, every
  declared extraction child's entries, ordinary move-gate versions for every
  declared placement update, and configured reader-retained generations;
- every externally registered volume has a content source and exact
  ...
  validated provenance and empty-base plus scar rematerialization;
...
- behavior registration/order/per-participant-view/collision/handoff/proposal/
  double-feedback/component-extraction/placement limits can hold every builder
...
- component-extraction child reservations fit live/lifetime volume records,
  page/brick/scar/directory pools, and binding-6 bytes; placement maxima fit
  ordinary per-volume move-gate versions; opaque egress maxima fit the existing
  handoff, staging, map, and host-result pools shared with mixed-processor
  handoffs;
...
requires at least eight storage-buffer bindings in one shader stage for the
