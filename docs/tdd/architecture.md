...
Bevy RenderApp / Moria render resources
        ...
        +-- directory-generation component-extraction publication
...
buffers, component-transfer/validation scratch, prepared directory-entry swap
scratch, placement compaction, and egress staging maps.
...
4. Root `RenderGraph`: execute camera-independent Moria compute in explicit
   ...
   -> behavior_publish_extraction_or_placement_gates
...
mapping before adapter execution and publishes through one prepared
directory-generation swap without CPU authority-path readback. Placement
streams validate on GPU and publish compacted records through ordinary
per-volume placement gates. Opaque egress maps asynchronously after its
ordered GPU copy; effect publication does not wait for CPU decoding.
...
- prepared directory-entry slots can hold every declared component-extraction
  swap's source/child entries and configured reader-retained generations;
...
- component-extraction child reservations fit live/lifetime volume records, page/brick/
  scar/directory pools, and binding-6 bytes; placement maxima fit per-volume
  gate publication; opaque egress byte/map maxima fit every enabled descriptor
  independently of handoff edge maxima while reusing the handoff map lifecycle;
