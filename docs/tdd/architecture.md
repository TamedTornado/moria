...
Bevy RenderApp / Moria render resources
        ...
        +-- extraction generation-swap + placement-stream batch publication
...
It also owns generic component-extraction child-identity reservations,
placement-stream proposal records, and opaque egress receipts. It never owns
component significance, activity-region schemas, fidelity classes, event
schemas, or simulation state.
...
It also owns the binding-0 child-reservation section, binding-1 egress
section, component-transfer/validation scratch, extraction generation-swap
publication, placement-stream batch validation against ordinary volume gates,
and egress staging maps through the existing staging pool.
...
4. Root `RenderGraph`: execute camera-independent Moria compute in explicit
   ...
   -> behavior_prepare_component_extraction
...
Component extraction receives final candidate child IDs in the binding-0
reservation section before adapter execution and publishes through one
extraction-only directory generation swap without CPU authority-path
readback. Placement streams validate as a GPU batch and publish through the
existing per-volume revision/placement gates. Opaque egress maps
asynchronously after its ordered GPU copy through the ordinary staging pool;
effect publication does not wait for CPU decoding.
...
- every externally registered volume has a content source and exact
  ...
  validated empty-base scars and provenance;
...
- live-volume and lifetime volume-record limits cover every initial
  registration plus reserved component-extraction child identity capacity,
  with lifetime records no smaller than live records;
...
- behavior registration/order/per-participant-view/collision/handoff/proposal/
  double-feedback/component-extraction-child/egress-host limits can hold every
...
- component-extraction child reservations fit live/lifetime volume records and
  page/brick/scar pools charged through existing proposal/payload/cell/brick/
  directory-effect maxima; opaque egress retained-host maxima fit every enabled
  descriptor, while device copy and maps fit existing staging/map pools;
