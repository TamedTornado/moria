...
Bevy RenderApp / Moria render resources
        ...
        +-- multi-entry extraction publication + independent placement gates
...
buffers, component-transfer/validation scratch, extraction directory-generation
...
- component-extraction capacity covers every declared child identity, transfer,
  scar, and one multi-entry directory generation; placement capacity covers
  independent per-volume gates for declared update maxima;
- every externally registered volume has a content source and exact
  reconstruction fingerprint; restored extraction children instead have an
  internal empty base, ordinary full-brick scars, and minimal provenance;
