...
Bevy RenderApp / Moria render resources
        ...
        +-- multi-entry component-extraction/placement publication
...
mapping before adapter execution and publishes through one multi-entry
directory-generation install without CPU authority-path readback. Placement
streams validate and publish compacted GPU records through the same multi-entry
mechanism. Opaque egress maps asynchronously after its ordered GPU copy;
...
- component-extraction child reservations fit live/lifetime volume records,
  page/brick/scar pools, and binding-6 bytes; placement maxima fit ordinary
  directory/entry capacity; opaque egress record/byte/map maxima fit every
  enabled descriptor independently of handoff and generic staging;
