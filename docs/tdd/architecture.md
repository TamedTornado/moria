...
Bevy RenderApp / Moria render resources
        ...
        +-- extraction directory-generation publication
        +-- compact placement-stream validation/publication
        +-- bounded opaque adapter-egress via handoff/staging
...
records, and opaque egress receipts that reuse handoff/staging transport. It
never owns component significance, activity-region schemas, fidelity classes,
event schemas, or simulation state.
...
working buffers, component-transfer/validation scratch, placement-stream
compaction, and the ordered handoff/staging copies used for terminal egress.
...
Component extraction receives final candidate child IDs/keys in a read-only GPU
mapping before adapter execution and publishes source plus children through one
extraction-scoped directory-generation transaction without CPU authority-path
readback. Placement streams validate one compact GPU batch and publish each
entry through ordinary per-volume placement/revision gates. Opaque egress maps
asynchronously after its ordered GPU copy on the existing handoff/staging path;
...
- every externally registered volume has a content source and exact
  ...
  validated persisted derived bases, stored keys, and provenance;
...
- component-extraction child reservations fit live/lifetime volume records,
  page/brick/scar pools, and binding-6 bytes; placement maxima fit compact
  update records and ordinary per-volume gate outcomes; opaque egress record
  maxima plus their handoff/staging charges fit every enabled descriptor
  together with ordinary handoff and generic staging demand;
