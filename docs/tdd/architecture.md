...
Bevy RenderApp / Moria render resources
        ...
        +-- fracture-delta component-extraction publication
        +-- placement-stream per-volume publication
        +-- terminal adapter-egress handoff staging
...
It also owns component-transfer/validation scratch, fracture directory deltas,
placement compaction, and terminal egress reuse of handoff staging maps.
...
4. Root `RenderGraph`: execute camera-independent Moria compute in explicit
   ...
   -> behavior_publish_fracture_delta_and_placements
...
Component extraction uses proposal-local piece handles during dispatch and
returns final child IDs through prior feedback after confirmed publication,
without CPU authority-path readback. Placement streams validate compacted GPU
records once and publish each through ordinary per-volume placement gates.
Opaque egress maps asynchronously through the terminal handoff staging path;
...
- fracture directory-delta capacity can hold every declared extract-components
  source plus pre-reserved children under the component-extraction byte bound;
- every externally registered volume has a content source and exact
  ...
  validated empty-base-plus-scar content and provenance;
...
- component-extraction child reservations fit live/lifetime volume records,
  page/brick/scar pools, and fracture-delta bytes; placement maxima fit
  per-volume placement publication; opaque egress maxima fit every enabled
  descriptor inside handoff and staging budgets;
