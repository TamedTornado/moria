...
Bevy RenderApp / Moria render resources
        ...
        +-- source-bound extraction publication and compact placement streams
...
buffers, component-transfer/validation scratch, source-bound multi-volume
publication groups for extraction, placement-stream validation scratch, and
egress staging maps.
...
mapping before adapter execution and publishes its source-bound parent/child
set through existing per-volume gates without CPU authority-path readback.
Placement streams validate compacted GPU move batches and publish each entry
through those same independent volume gates. Opaque egress maps asynchronously
after its ordered GPU copy; effect publication does not wait for CPU decoding.
...
- live/lifetime volume-directory capacity can hold every initial registration
  plus every declared component-extraction child reservation and ordinary
  directory effect bound;
...
- component-extraction child reservations fit live/lifetime volume records,
  page/brick/scar pools, binding-6 bytes, and source-bound multi-volume
  publication groups; placement maxima fit ordinary per-volume placement gate
  slots for the declared entry count; opaque egress record/device/staging/
  host/map maxima fit every enabled descriptor independently of handoff and
  generic staging;
