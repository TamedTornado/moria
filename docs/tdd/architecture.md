...
The module also owns generic component-extraction reservations, packed
placement proposals, and opaque egress results. It does not own activity
regions, fidelity classes, simulation bodies, or event schemas.
...
- every externally registered volume has a content source and exact
  reconstruction fingerprint; restored derived children instead have
  validated persisted provenance and complete initial sparse bricks;
...
- behavior registration/order/per-participant-view/collision/handoff/proposal/
  double-feedback/component-child/placement/egress limits can hold every
