...
Bevy RenderApp / Moria render resources
        ...
        +-- multi-volume component-extraction/placement publication
...
buffers, component-transfer/validation scratch, multi-volume publication
staging for extraction/placement, and egress staging maps.
...
mapping before adapter execution and publishes through one multi-volume
`directory_generation` advance without CPU authority-path readback. Placement
streams validate and publish compacted GPU records through the same multi-volume
linearization. Opaque egress maps asynchronously after its ordered GPU copy;
...
- live/lifetime volume records, pages, bricks, and scars can hold every declared
  component-extraction child reservation and multi-volume publication;
...
- component-extraction child reservations fit live/lifetime volume records, page/brick/
  scar pools, and binding-6 bytes; placement maxima fit the placement pools;
  opaque egress record/device/staging/host/map maxima fit every enabled
  descriptor independently of handoff and generic staging;
