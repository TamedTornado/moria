...
buffers, component-transfer/validation scratch, multi-volume directory
preparation, placement compaction, and egress staging maps.
...
4. Root `RenderGraph`: execute camera-independent Moria compute in explicit
   ...
   -> behavior_publish_multi_volume_directory
...
mapping before adapter execution and publishes through one multi-volume directory
generation without CPU authority-path readback. Placement streams validate and
publish compacted GPU records through the same directory-generation path. Opaque egress
...
- ordinary directory and lifetime capacity can hold the current directory
  plus every declared multi-volume proposal's prepared entries and configured
  reader-retained directory generations;
...
- component-extraction child reservations fit live/lifetime volume records, page/brick/
  ...
  directory/revision capacity for one multi-volume stream; opaque egress record/device/staging/host/map maxima
