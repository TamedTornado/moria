...
Voxel worlds only work when several hard systems agree as explicit contracts: sparse material truth, bounded inspection, mutation admission, streaming lifecycle, collision against matter rather than presentation, physical response for moving matter (gravity, contact, applied force), persistence of world matter plus edits, GPU-resident representation of that matter, and measurable presentation derived from truth. Moria exists so material worlds can be consumed without each game rebuilding those contracts, and without hiding them behind a single privileged demo.

The product’s claim is infrastructural: a consumer can obtain a continuous three-dimensional material world (including movable, damageable voxel volumes such as players and enemies), inspect and mutate it only through supported interfaces, keep authoritative matter GPU-resident for scale and gameplay-enabling work, and trust that what they see, collide with, and push under force is a view of the same authoritative matter.

**World generation is not part of the substrate identity.** How a game fills or seeds sparse material volumes is a consumer- or game-dependent algorithm that runs *on top of* the substrate. The substrate provides storage, query, mutation, streaming, collision and physics seams, persistence seams, and presentation derivation for material truth—not a baked-in procedural generator.
...
- Collision and occupancy truth against voxel matter, not against disposable meshes—including contact between moving voxel entities and other material volumes.
- Physics response for material volumes: gravity, contact resolution for moving entities, and applied force sufficient for explosions, crumbling, and related matter failure—not game rules about *when* those events fire.
...
**Downstream, not this repository** are actual games and game layers: player control, characters, skeletal animation, game-specific presentation, combat rules, AI behavior, economy, building policy, the System / LLM layer, spells, gas pricing, and other gameplay rules—including **which procedural or authored generation pipeline** a game uses. Freeform ship and station games (and their design/combat fiction) remain **future-consumer examples**, not current delivery or validation targets. Compatibility seams may be designed where substrate requirements demand them; those layers are not implemented here.

## Product-level outcomes

These are outcomes the substrate must enable, not a feature inventory or implementation plan:

1. **Truth vs view.** Occupancy, queries, collision, physics response, and persistence run against voxel matter; meshes, dressing, and debug geometry are derived and disposable.
...
7. **Matter physics.** Moving voxel entities participate in collision with other material entities and volumes. Gravity and applied force (for explosions, crumbling, and similar material failure) are substrate capabilities. Integrating a full third-party physics engine may be staged; the product still needs these responses as matter contracts, not as optional demo polish.
...
10. **Measurable substrate quality.** Benchmarks and harnesses can evidence mutation response, streaming, GPU memory behavior, physics/collision honesty, and related contracts without redefining the product as a game. Harness-side generation used for tests or demos does not make generation a substrate requirement.
...
- Pure sandboxes.
- Games whose players and enemies are voxel volumes that move and take damage under the same matter contracts as terrain—including gravity, contact, and force-driven failure.
- A single-captain space trading and combat game in the Escape Velocity mold—starmap, cargo runs, factions, missions—where the ship is a freeform material volume rather than a hull with abstract slots. Reactors, conduits, weapons, and life support occupy real space; power routes are visible geometry; a hit destroys a specific thing rather than a health number. Ship design is conversational with an engineer who proposes layouts and benches them under load; ships wear, fail under stress, and leave wrecks as real salvageable geometry. Debris from fights the player never saw still floats when they arrive. This paragraph is **explicitly nonbinding future-consumer context**: it motivates everywhere mutation, first-class volumetric depth through multi-deck interiors, GPU-resident matter at combat and design scale, matter physics under force, and truth-vs-view so damage and salvage stay honest—without importing its fiction, UI, mission systems, or freeform-hull *delivery* into current Moria scope.
...
- Matter physics—entity-to-entity and entity-to-volume collision for moving voxel matter, gravity, and applied force for explosions and crumbling—is a binding product capability. Immediate third-party physics-engine integration is not required to state the need; the responses are.
...
5. **Matter physics as substrate capability (affirmed).** Moving voxel entities collide with other entities and volumes; gravity and force-driven failure (explosions, crumbling) are required product capabilities even if engine integration is staged.
...
| **Q7.** Is matter physics (collision for moving entities, gravity, force for explosions/crumbling) in product scope? | **Yes.** Moving voxel entities must collide with other entities; gravity and applied force for explosions and crumbling are required. Full physics-engine integration may be staged, but the capability is product-level, not optional polish. |

## Open questions (identity)

None currently open that would change product identity, purpose, or boundary. Engineering and milestone sequencing (which physics responses land first, how gravity is parameterized for non-planetary volumes, which crumbling model is selected) remain design/TDD concerns, not vision-identity blockers.

Older planning docs that still title the effort as “Product One — The Walkable World” should be read as superseded on identity by the boundary and this vision once approved. Engineering docs that still list deterministic generation under the public facade describe current implementation or harness practice; this vision treats generation as **not** substrate product identity per Q6. Seed language that listed structural integrity / cave-ins or rigid conversion as non-current remains superseded for **force-driven crumbling and entity physics** by Q7; full fluid CA, fire/growth CA, and tree-felling remain non-goals until explicitly selected.
