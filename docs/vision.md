...
- A consumer can create and identify a world, request bounded regions, observe readiness and streaming lifecycle, query authoritative material truth (including registered objects that participate in queries without becoming game entities), submit bounded edits, and persist material deltas.
- Generation is deterministic for the same versioned parameters and seed. Mutation is admitted through a bounded command API and committed atomically. Failures are typed and observable on the public surface.
...
- Correctness: deterministic generation for the same versioned parameters and seed; atomic commit of admitted mutations; persistence restores authoritative material state; derived meshes and diagnostics are never authoritative.
- Isolation: external consumers, including validation, use only the public surface; no privileged world path.
- Streaming and failure: resident work is bounded; lifecycle is observable; background results carry generation identities; failures are typed and observable to public consumers.
...
- Concrete public API shapes, identity encodings, command schemas, and readiness/lifecycle representations.
...
- Delivery depth and sequencing of substrate capabilities within the current product.
...
- `docs/seeds/substrate-interface-reference.md` — Supporting public-surface outcomes (identity, query/readiness, mutation admission, streaming states, delta persistence, registered objects, diagnostics) without adding deliverables or expanding product scope.
