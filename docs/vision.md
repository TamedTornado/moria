...
**Moria** is a reusable, GPU-resident voxel-world substrate, delivered as a Rust crate or a small family of tightly scoped Rust crates. Downstream games and tools consume it as a library. Moria is the world-and-matter engine layer—not a game, and not a presentation shell.

## Purpose

Moria exists so multiple game styles can share one material-world foundation: a surface that reads as a normal outdoor world while remaining fully mutable voxel truth; continuous deep underground play space; and public mutation, query, and extension surfaces that keep game rules, content, and policy above the substrate. The world must behave as coherent matter, not only look like terrain, and it must stand alone with no LLM or game-policy dependency.
...
- A clean public integration surface so external games use the same verbs, queries, events, and extension paths as hand authoring and any in-repo consumer.

**Adjacent / not product identity**

- A walkable-world executable is a **required adjacent first proof**. It validates generation, streaming, meshing, editing, collision, persistence, and performance, and is the project’s first downloadable and benchmark deliverable. It is not a game layer and does not define product identity. It must consume the substrate only through public interfaces available to an external game. Character, controller, camera, demo content, route, presentation, workloads, platforms, and numeric acceptance gates remain consumer-owned.
- The actual game (or games) is a separate downstream consumer and is not part of this repository.

**Out of this product**

- Game rules; System/LLM features; spells; gas/pricing policy; combat; AI; and building/gameplay layers (UX, controllers, authored content, presentation policy, work orders, designation modes, and game-specific modes).
...
1. **Normal-looking material world** — Surface terrain, water, vegetation, and geology read as a natural world; the voxel grid is authoritative matter, not the intended aesthetic. Rendered surfaces and non-identity clutter are views of that truth and stay coherent under edit.

2. **Coherent world behavior** — Interactable matter supports burn/break/growth coherence, granular settle, wetness, and fire ecology. Time and seasons drive light, growth, and snow; weather drives wetness, water levels, ignition, and fire ecology. Weather and time are not required to drive breaking or granular settling. Delivery depth beyond the first-proof slice is design sequencing; those outcome families are not cancelled by a thinner first slice.

3. **Mutable everywhere, including deep Z** — Any material can be destroyed, moved, or placed through depth. Caves, strata, ore, and underground volume are first-class content space, not a skybox floor. Dig and place are first-class substrate verbs.
...
Future **consumers** (not current product): a System-driven ARPG, a fortress/colony game, a Moria-style descent, and pure sandbox tools. They motivate substrate outcomes—mutable geology, deep Z, matter verbs, integrity and fluids, coherent ambient/matter behavior, POI/structure metadata, extensible definitions—but own gameplay, content, controllers, characters, presentation, and policy.

The walkable-world first proof is an adjacent validation consumer, not a future game. Its fixed outcome slice is bound above; character, camera, demo postcard content, scripted route, and performance gates stay consumer-owned.
...
- Making render meshes, dressing, or diagnostics authoritative over matter.
...
- GPU-resident world/matter operation is part of the product promise (mechanism detail deferred).
...
None. The seeds agree that the current product is the reusable substrate; the walkable world is an adjacent first proof, not a competing product identity; and future games remain consumers above the crate boundary.
...
- **docs/seeds/project-boundary.md** — Binds product identity to the reusable Rust substrate, excludes the actual game and higher game layers, and requires public-interface-only harness consumption without making the harness product identity.
- **docs/seeds/product-one-seed.md** — Fixes the walkable first proof’s outcome slice (natural continuous material world, dig/place proof, full geology-generation slice) and the wgpu/WGSL portability constraint without redefining product identity or importing demo content, controllers, or machine-specific gates into the substrate.
- **docs/seeds/voxel-world-substrate.md** — Authorizes substrate outcome families: natural look over voxel truth, mutability and deep Z, geology generation, coherent matter behavior with separated ambient couplings, matter services, public layering, seed-backed regeneration plus exact edit-delta restoration, and delta-scoped cross-mode reuse. Future genres and System clients remain above the substrate.
