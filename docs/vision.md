...
**Moria** is a reusable **Rust voxel-world substrate**: a library consumed through public crate interfaces. This repository delivers that substrate for multiple downstream games and tools. It is not a game, demo title, or content package.

## Purpose

Give independent game and tool authors a shared world foundation they can integrate without forking world internals. Callers create and drive generated voxel worlds through a public surface so generation, residency, mutation, material truth, and persistence stay coherent and reusable across consumers.
...
Consumers must not reach into storage, meshing, or scheduler internals. There is no privileged integration path for in-repo or out-of-repo callers.

## Required product outcomes

- **Public multi-consumer surface.** Games and tools integrate solely through public crate interfaces that remain useful to more than one downstream consumer.
...
Moria’s enabling implication is only that a later game can generate, stream, query, mutate, extract surface, and persist a voxel world through the public substrate without privileged internals. No gameplay or content from Product One is in current scope.
...
- Delivery form is a **Rust** library with **public crate interfaces**
...
- Streaming **bounds resident work**, exposes **lifecycle states**, and uses **generation identities** against stale application
- Failures are **typed and observable** to public consumers
- Adjacent validation, when present, uses **exactly the public interfaces** and reports performance with **machine identity**
...
- **`docs/seeds/mixed-project-brief.md`:** Authoritative current product identity, public boundary, correctness and validation commitments, non-goals, and Product One as future-consumer context only.
