...
| **product-one-seed.md** | Product statement and non-goals for the first slice; dig/place as proof not gameplay; layer slice (generation full, matter partial, script/API sliver); performance targets and platform constraints; milestone intent; what Product One “buys.” **Binding validation constraints** (not optional harness flavor): one curated generated region sized so sparsity and streaming are real; generation must produce a proof-point composition covering natural surface, forest/objects, static water bodies, cliffs/strata, walkable cave depth, underground material honesty, micro objects, and a stamp/POI ruin exercise; continuous third-person traversal that exercises surface-to-underground Z. Detailed catalogs (exact dimensions, material list, species counts, trailer beats, character fantasy) stay in the seed — not restated here as product content. |
