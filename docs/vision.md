...
- A walkable third-person validation harness that proves continuous 3D traversal (including surface-to-underground), public-path dig/place, streaming, persistence of that slice, and performance under measurement—using public APIs only. The same executable is the intended public, downloadable, product-shaped demonstration for the audience; it remains a non-privileged harness, not product identity and not a game. Controllers, character, route, seed dressing, presentation, platforms, and numeric gates are harness-owned proof detail, not substrate identity.
...
7. **Public product-shaped demonstration** — The walkable validation executable is also intended as a public, downloadable, product-shaped demonstration for the audience (credibility artifact, not a shipped game). It remains a non-privileged harness that uses only public APIs; it is neither product identity nor a game.
...
| **`docs/seeds/product-one-seed.md`** | Binds the first delivery slice: curated generated region under real sparsity; dig/place as mutability proof; generation full / matter partial (static water, no CA/flow/integrity/felling); seed-plus-deltas exact restore; walkable third-person harness as proof vehicle and public product-shaped downloadable demo for the audience; portable wgpu/WGSL pressure. Concrete set dressing, material lists, milestone order, and machine numbers are harness/validation detail—capability pressure only, not product identity. |
