...
**Moria** is a reusable, GPU-resident **voxel-world substrate**, delivered as a Rust crate (or a small family of tightly scoped crates). **Product One includes** a walkable third-person **validation executable**—a product-shaped demo harness for terrain generation, streaming, meshing, editing, collision, persistence, and performance. That executable is a **separate public-API consumer**, not part of the substrate: it must use the same interfaces available to any external game, with no privileged or game-specific world paths.
...
| Walkable third-person validation executable (Product One; public-API consumer only) | Building UI, blueprints-as-gameplay, mechanisms-as-gameplay |
...
The seeds agree on **current product identity** (substrate plus Product One’s walkable third-person validation executable as a public-API consumer; game elsewhere). No ambiguity requires blocking this vision. Residual items for later human/design decisions (not product-identity blockers):
...
| **project-boundary.md** | Locks product identity to the reusable substrate; forbids implementing the game and higher layers here; requires public-API consumption by the walkable executable and a workspace consumer boundary. (Seed wording is “may include”; human review pins Product One as including that executable.) |
| **product-one-seed.md** | Defines the first product-shaped proof: one generated natural region, third-person traversal of continuous 3D (including underground), dig/place as demo proof, explicit non-goals, performance credibility targets, and the slice of generation / matter / API layers that ship first—including the walkable validation executable as the demo vehicle. Concrete seed-world set dressing (species counts, ruin stamp, material list, milestone order) is **demo content and validation criteria**, not imported here as product scope. |
