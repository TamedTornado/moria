...
Moria exists so multiple downstream consumers can rely on one shared voxel-world foundation—deterministic generation, material truth, bounded edit and streaming behavior, surface extraction, persistence, and diagnostics for lifecycle and bounded work—without reimplementing that foundation or depending on game-specific systems.

## Product boundary

**In product (Moria):** the reusable substrate and its public crate surface: world creation and identity, deterministic seed-based generation, sparse voxel world material storage, bounded region request and readiness, bounded authoritative material queries (including registered objects that can participate in queries without becoming game entities), bounded mutation, bounded streaming with observable lifecycle, surface extraction, persistence that records authoritative material deltas and restores the same authoritative material state, and diagnostics that report lifecycle and bounded work without exposing mutable internal handles. External consumers integrate only through that public surface and must not reach storage, meshing, or scheduler internals.
...
- Consumers create and identify worlds (identity combining format version, generation parameters, and seed), request bounded regions, observe readiness, and query bounded authoritative material observations through public crate interfaces only; registered objects may participate in those queries without becoming game entities.
...
- Persistence records authoritative material deltas rather than derived meshes, and restores the same authoritative material state; derived meshes and diagnostics never become authoritative world state.
- The substrate provides surface extraction and diagnostics that report lifecycle and bounded work without exposing mutable internal handles, usable by external consumers through the public boundary.
...
- Authority model: persistence records authoritative material deltas rather than derived meshes, and restores the same authoritative material state; derived meshes and diagnostics never become authoritative. The seeds do not establish that every non-material observation is categorically non-authoritative.
...
- `docs/seeds/substrate-interface-reference.md` — Supporting vocabulary refining world identity composition, bounded authoritative query observations, query participation without entity-hood, mutations, streaming states, persistence recording of deltas vs meshes, and diagnostics that report lifecycle and bounded work without mutable internal handles; does not add deliverables or expand scope.
