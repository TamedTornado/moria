...
rules, states, content boundaries, behavior, failure behavior, validation
experience, and source-derived design rationale. It stays above technical
implementation detail: it does not choose exact crate splits, algorithms,
storage formats, APIs, deployment topology, or delivery milestones. Product
form—**a Rust and Bevy library (or small family of tightly scoped crates) for
crate consumers**—is part of approved product identity, not a deferred
implementation choice.

**Authority.** `docs/product-vision.md` is the complete product boundary. This
design preserves its requirements and non-goals in meaning. Subordinate sources
(project boundary, GPU-resident architecture note, voxel-world architecture
reference, Product One validation seed, excluded System-pivot notice) contribute
compatible substance only when they align with that vision. They do not reopen
identity, expand excluded game scope, or redefine “done” from harness content.
...
- Homogeneous emptiness and solid remain cheap; detail cost concentrates on the
...
- Material identity is more than a render color: occupancy and material class
  drive inspection, collision truth, presentation derivation, and physics-facing
  supportable fields.

**Seed-compatible altitude.** Subordinate architecture material motivates
treating cells as material + occupancy (or density) so digs can leave partial
cuts that still read as matter, and so presentation can rebuild from truth.
Exact cell packing, bit widths, brick dimensions, and spatial-index layouts are
**not** product identity; they remain technical choices constrained by sparsity,
GPU residency, and honesty requirements.
...
- Content-injection seams may supply how detail is *filled* when first needed;
  the substrate owns the materialization lifecycle, not the fill algorithm.
...
- Supportable material fields relevant to physics guests when those bindings are
  exposed (strength, gravity parameters, force-related fields, and related).
...
- Mutation is a first-class product proof—not optional scenery decoration
  outside the material world.

**Seed-compatible altitude.** Validation tradition treats dig/place as the
honesty proof: a representative carve must leave real openings, honest cut faces,
and rematerialized presentation from truth—not a painted hole in a mesh.
Specific brush shapes, radii, and debug key bindings are harness particulars.

**Not owned:** building policy, construction UI, blueprints as gameplay, dig as
progression, gas pricing of verbs, or game-mode rules about who may edit what.
...
- Multiple focus anchors remain expressible in principle (a fortress-scale
  consumer may pin interest while another focus wanders); exact ring policy is
  technical design.

**Seed-compatible altitude.** Subordinate sources describe distance bands (render
detail, active simulation guests, aggregate/cold storage). The product requires
bounded residency and observable lifecycle; it does not freeze a particular
ring table as identity.
...
- Simulation outcomes that should change matter re-enter through admitted
...
- A consumer may use a reproducible base-world strategy so only scars need
  durable storage; that strategy is guest content, not substrate product.

**Seed-compatible altitude.** The durable idea “truth = base strategy + edit
deltas” is retained at product altitude. Object journals, multi-slot UX, save
versioning policy, and cross-mode fortress reuse loops are consumer/game or
later-scope concerns unless later selected.
...
- Presentation may distinguish organic cuts from sharp constructed faces where
  materials demand it; the product requires honesty after mutation, not a
  particular meshing algorithm.
...
- **Dressing vs matter-backed objects** remains a product distinction at
  consumer altitude: scatter dressing (grass, ground clutter) is a pure function
  of supporting matter and vanishes when that matter is removed; matter-backed
  objects (trees, boulders, stamps) participate as registered assemblies rather
  than as disconnected scenery.

**Not owned:** tree felling, rigid-body conversion of vegetation, growth systems,
fire, granular settling, or other future consumer simulation concepts.
...
- Stamps, prefabs, and material palettes may be injected as consumer content
  through the same contract posture; no particular content pack is substrate
  law.
...
14. **Dressing tracks matter.** Surface clutter derived from supporting material
    must not outlive or contradict the matter it is anchored to.
...
- Supportable material fields for physics guests (strength, gravity parameters,
  force, related).
- Static bodies of water as material occupancy and surface presentation when a
  consumer provides them (no flow simulation required).
...
A walkable natural-world proof shape (curated region, forest, ruin, dig-as-demo)
may be used by a validation consumer. Its content, controls, milestones,
performance tables, and curated generation pipeline remain **context for what
the substrate might support**, not the definition of the product.
...
### 10.5 Illustrative harness proof points (non-binding content)

Subordinate Product One material describes one *possible* way a validation
consumer might make claims undeniable. These are **illustrative**, not substrate
requirements:

- A continuous journey that exercises vertical depth (surface to underground
  material volume).
- A dig that carves a traversable opening with honest cut faces mid-scene.
- Object density (vegetation/micro-objects) that stresses registration without
  desyncing from matter.
- A region large enough that full raw-voxel residency would be unreasonable.
- Scripted flythrough / mutation workloads that report metrics with machine
  profile.

None of the above define product identity or mandatory completion criteria.
...
## 13. Seed reconciliation

Subordinate sources informed this design. They are not equal authorities. When
they conflict with the approved vision, the vision wins. This section states
what the design **selects**, what the approved boundary **excludes**, and what is
**deliberately deferred** for technical design or human decision.

### 13.1 Source authority order (subordinate)

1. Approved product vision (`docs/product-vision.md`) — binding.
2. Project boundary seed — reusable substrate as product; harnesses public-API
   only; game/System layers out of scope.
3. GPU-resident substrate note — supporting principles for sparse residency,
   command/query/event boundary, no privileged storage access, derived meshes
   never truth.
4. Voxel-world substrate reference — architecture context for domain presentation,
   everywhere mutation, deep Z, layering, object/dressing split; game examples
   and future extensions are nonbinding unless selected.
5. Product One seed — validation / consumer example motivating dig honesty,
   collision against truth, sparse streaming, scar persistence, measurable
   quality; harness particulars are not product identity.
6. System-substrate pivot notice — **excluded source**; contributes no product
   requirements. Useful substrate-only principles live in the GPU-resident note.

### 13.2 Selected into this design (compatible substance)

| Concept | Where it lands |
| --- | --- |
| Substrate, not game; public consumer contract shared by harnesses and games | §§1–2, 5, 10 |
| Truth vs view; meshes/dressing disposable | §§3.10, 5, 8.1 |
| Sparse homogeneous volume + interesting shell pays cost | §§3.1, 3.2, 3.5, 8.3 |
| Lazy materialization on demand | §3.2 |
| Dig/place (and related) as sole mutation path; honesty of cut faces | §§3.4, 4.3, 10 |
| Collision/occupancy against matter, not meshes | §§3.6, 4.5, 8.7 |
| Streaming lifecycle / bounded residency | §§3.5, 4.4 |
| Persistence as scars without full dumps; base strategy is consumer-owned | §§3.9, 4.8, 8.8 |
| Object registration + dressing that tracks supporting matter | §§3.11, 4.9 |
| Content injection seams; no baked-in generator | §§3.12, 2.1 |
| GPU-resident matter + async command/query boundary | §§3.13, 8.9 |
| Physics-ready bindings without owning an engine | §§3.7, 4.6, 8.7 |
| Dynamic material volumes under same truth contracts | §§3.8, 4.7, 8.6 |
| Volume-general contracts (ships/stations motivate, not deliver) | §§1.1, 2.3, 7.3 |
| Measurable quality with machine context | §10 |
| Optional walkable / physics proofs as adjacent artifacts | §§2.2, 10.2 |

### 13.3 Excluded by approved boundary (do not import)

| Seed / older concept | Exclusion |
| --- | --- |
| Product One as product identity (“the walkable world”) | Superseded; walkable harness is adjacent only |
| Curated region postcard, third-person controller, demo routes, content palettes as requirements | Harness particulars only |
| Deterministic / procedural geology pipeline as substrate product | Generation is consumer/harness-owned |
| Baked-in physics engine, PBD particles, rigid conversion of trees | Physics engine and rigid felling are adjacent/future |
| Full fluid CA, fire, wetness, growth, granular settle as current product | Explicit non-goals; format hooks may exist later |
| Structural integrity, span tables, cave-in simulation | Future consumer / plug-in unless later selected |
| Building UI, blueprints as gameplay, mechanisms as game entities | Downstream game scope |
| Nav graphs, multiplayer services, weather, seasons | Not current substrate product |
| System / LLM, spells, gas, combat, AI, agent labor | Excluded layers |
| Freeform ship/station delivery and validation as current work | Future-consumer examples only |
| Web / wasm as target platform | Explicit non-goal |
| Machine-specific performance tables as substrate identity | Harness/TDD acceptance concerns |
| System-substrate pivot document as requirements | Excluded source notice |

### 13.4 Deferred for technical design (preserve alternatives; do not select here)

These constraints and options are useful for the TDD and must not be silently
elevated into product identity by this design:

- Exact crate splits within the Rust/Bevy library family.
- Spatial index shape, brick dimensions, voxel edge length, cell payload packing.
- Meshing / presentation techniques (raw debug voxels, smooth isosurfaces, hybrid
  sharp/organic extraction, distant LOD/impostors)—product requires honesty and
  domain-coherent presentation, not a named algorithm.
- CPU vs GPU ownership of particular passes (allowed to move without changing the
  consumer contract).
- Streaming ring tables, activation distances, and multi-anchor pinning policy.
- Persistence encoding of deltas, compression, and object journals beyond the
  product rule “scars without full dumps; views not authority.”
- Minimum material palette for a harness scene vs open material registry design.
- Numeric performance thresholds, resolution targets, and acceptance scenes.
- Platform portability mechanisms and graphics-backend choices.
- Engineering constraints from subordinate seeds that are not product identity
  (for example 32-bit GPU-visible counters/atomics as a portability constraint
  candidate)—preserve for TDD justification; do not treat as vision-level
  identity unless later made binding.

### 13.5 Deferred for human decision

See §14. Reclassifying substrate vs consumer ownership always requires an
...
This design is synthesized under the approved `docs/product-vision.md`, using
compatible substance from subordinate sources listed in §13 (project boundary,
GPU-resident architecture note, voxel-world architecture reference, Product One
validation seed, and the excluded System-pivot notice). Seed documents and
superseded Product One walkable-world planning language were not treated as
authoritative product boundary. Exact crate splits, algorithms, storage layouts,
numeric acceptance thresholds, demo routes, and task breakdowns remain outside
this document’s scope. The product’s form as a Rust and Bevy library (or small
family of tightly scoped crates) for crate consumers is approved identity and is
stated above; it is not deferred as mere implementation technology.
