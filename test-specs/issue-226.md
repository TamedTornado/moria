# Issue 226 — Wire acquired assets/shaders/vegetation.wgsl
...
- Install exactly `assets/shaders/vegetation.wgsl` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates portable 32-bit-only WGSL extension consuming per-instance transform/variation for shared assets.
...
- Independently mutate the fixture for: missing entry/binding, 64-bit atomic/backend-specific feature, per-instance material assumption or parse failure. Each mutation must fail the precise schema/format/budget rule.
