# Issue 203 — Wire acquired assets/config/presentation.ron
...
- Install exactly `assets/config/presentation.ron` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates RON presentation config containing exactly documented streaming/rendering/player/camera/mutation/acceptance defaults and valid ranges.
...
- Independently mutate the fixture for: unknown/missing field, wrong band/hysteresis/budget/player/camera/edit/acceptance value or invalid range. Each mutation must fail the precise schema/format/budget rule.
