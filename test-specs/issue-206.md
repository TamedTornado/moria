# Issue 206 — Wire acquired assets/materials/terrain_albedo.ktx2
...
- Install exactly `assets/materials/terrain_albedo.ktx2` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates Basis Universal KTX2 sRGB array, 14 layers, 1024x1024 authored layer size, complete matching mips, neutral air/water slots in canonical layer order.
...
- Independently mutate the fixture for: wrong color space/payload/dimensions/layers/mips/order, nonneutral reserved slots or corrupt KTX2. Each mutation must fail the precise schema/format/budget rule.
