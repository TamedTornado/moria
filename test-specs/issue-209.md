# Issue 209 — Wire acquired assets/materials/water_normal.ktx2
...
- Install exactly `assets/materials/water_normal.ktx2` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates cross-platform Basis Universal KTX2 linear water-normal texture with complete mip chain and registry dimensions.
...
- Independently mutate the fixture for: sRGB, missing mips, non-Basis/backend-incompatible payload or corrupt texture. Each mutation must fail the precise schema/format/budget rule.
