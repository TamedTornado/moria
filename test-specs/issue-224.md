# Issue 224 — Wire acquired assets/shaders/terrain.wgsl
...
- Install exactly `assets/shaders/terrain.wgsl` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates portable WGSL Bevy material extension with declared entry points consuming position/normal plus four 32-bit/u8-compatible material IDs/weights and shared arrays; no 64-bit atomic/backend-only feature.
...
- Independently mutate the fixture for: missing entry/binding, wrong vertex contract, 64-bit atomic/type, vendor/backend-specific assumption or parse/pipeline failure. Each mutation must fail the precise schema/format/budget rule.
