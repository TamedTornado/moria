# Issue 201 — Wire acquired assets/config/product_one_region.ron
...
- Install exactly `assets/config/product_one_region.ron` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates RON RegionConfig: normative seed; x/z [-500,500) m, y [-128,128) m; 0.25 m voxels/4 m bricks; authoritative feature/object constraints; deny unknown fields.
...
- Independently mutate the fixture for: unknown/missing field, altered bounds/voxel scale/seed, noncanonical authoritative value. Each mutation must fail the precise schema/format/budget rule.
