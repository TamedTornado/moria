# Issue 218 — Wire acquired assets/vegetation/grass_cluster.glb
...
- Install exactly `assets/vegetation/grass_cluster.glb` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates binary glTF support-anchored cluster with finite required attributes and shared batched dressing mesh/material handles within registry budget.
...
- Independently mutate the fixture for: wrong support origin/bounds, malformed attribute/index, unique material or unusable per-instance-only layout. Each mutation must fail the precise schema/format/budget rule.
