# Issue 205 — Wire acquired assets/materials/materials.ron
...
- Install exactly `assets/materials/materials.ron` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates RON registry with exactly IDs 0..13 in canonical key order/classes/hardness/granular/surface/layers; state defaults zero and immutable Product One collision classes.
...
- Independently mutate the fixture for: duplicate/missing/reordered/unknown ID or key, changed air/water/solid class, invalid hardness/layer/state default. Each mutation must fail the precise schema/format/budget rule.
