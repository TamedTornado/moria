# Issue 212 — Wire acquired assets/vegetation/birch_far.glb
...
- Install exactly `assets/vegetation/birch_far.glb` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates binary glTF matching birch origin/bounds conventions, shared material keys and <=500 triangles.
...
- Independently mutate the fixture for: origin/bounds/LOD mismatch, unique material, malformed attributes or 501 triangles. Each mutation must fail the precise schema/format/budget rule.
