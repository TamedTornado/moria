# Issue 210 — Wire acquired assets/vegetation/birch_near.glb
...
- Install exactly `assets/vegetation/birch_near.glb` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates binary glTF 2.0 metres/Y-up/+Z-forward, applied transforms, support-center origin, finite indexed attributes/UV0/tangents as needed, birch analytic bounds/shared materials, <=12000 triangles.
...
- Independently mutate the fixture for: wrong axes/units/origin/bounds, missing/nonfinite attribute, invalid index, unique material or 12001 triangles. Each mutation must fail the precise schema/format/budget rule.
