# Issue 223 — Wire acquired assets/player/explorer.glb
...
- Install exactly `assets/player/explorer.glb` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates binary glTF capsule-proportioned explorer with skeleton and exactly required named Idle/Run/Sprint/Jump/Fall/Paddle clips, support convention and <=40000 triangles.
...
- Independently mutate the fixture for: missing/renamed clip, absent skeleton, wrong support/bounds, malformed animation/attributes or 40001 triangles. Each mutation must fail the precise schema/format/budget rule.
