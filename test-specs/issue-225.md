# Issue 225 — Wire acquired assets/shaders/water.wgsl
...
- Install exactly `assets/shaders/water.wgsl` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates portable 32-bit-only WGSL extension consuming static water geometry and presentation-only time; no truth write/storage simulation.
...
- Independently mutate the fixture for: missing entry/binding, 64-bit atomic/backend-specific feature, truth/storage/flow side effect or parse failure. Each mutation must fail the precise schema/format/budget rule.
