# Issue 208 — Wire acquired assets/materials/terrain_orm.ktx2
...
- Install exactly `assets/materials/terrain_orm.ktx2` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates Basis Universal KTX2 linear 14-layer ORM (R AO, G roughness, B metallic) array with dimensions/order/mips exactly matching terrain_albedo.
...
- Independently mutate the fixture for: wrong color space/channel semantics/payload/layers/mips/order or mismatch with albedo/normal. Each mutation must fail the precise schema/format/budget rule.
