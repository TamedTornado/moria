# Issue 202 — Wire acquired assets/stamps/ruin_p1.ron
...
- Install exactly `assets/stamps/ruin_p1.ron` at that unchanged runtime path; the issue writes no sibling content or either production registry.
- The loader validates RON SparseVoxelStamp: sorted nonoverlapping in-bounds cut_stone/explicit-air runs, valid pivot/palette, Y quarter turns, stair_bottom/stair_top/entrance tags, identity digest.
...
- Independently mutate the fixture for: overlapping/unsorted/out-of-bounds run, illegal palette material/tag/orientation, bad digest or untraversable stair. Each mutation must fail the precise schema/format/budget rule.
